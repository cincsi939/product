...
<?php
	...
	header( "location:../../usermanager/login.php");
		...
		$sql = " SELECT 
						...
						eq_child.eq_care_name AS eq_care_name,
						eq_child.eq_care_lastname AS eq_care_lastname,
						eq_child.eq_Relationship AS eq_Relationship
					FROM eq_child
    ...
    $(".ui-datepicker, select").css('font-size', 14) 
		...
		$('.presStatus').click(function(){
			if($('input[name=presStatus]:radio:checked').val()=='1'){
				$('#eq_care_name').attr('disabled','disabled');
				$('#eq_care_lastname').attr('disabled','disabled');
				$('#eq_Relationship').attr('disabled','disabled');
				$('#eq_care_name').val("");
				$('#eq_care_lastname').val("");
				$('#eq_Relationship').val("");
				
			}else{
				$('#eq_care_name').attr('disabled',false);
				$('#eq_care_lastname').attr('disabled',false);
				$('#eq_Relationship').attr('disabled',false);
			}
		});
...
function saveData(){
	...
	var eq_care_name = $('#eq_care_name').val();
	var eq_care_lastname = $('#eq_care_lastname').val();
	var eq_Relationship = $('#eq_Relationship').val();
	...
	if(presStatus=="2" && eq_care_name==""){
		alert('กรูณาระบุชื่อผู้เลี้ยงดูบุตร');
		return false;
	}
	if(presStatus=="2" && eq_care_lastname==""){
		alert('กรูณาระบุนามสกุลผู้เลี้ยงดูบุตร');
		return false;
	}
	if(presStatus=="2" && eq_Relationship==""){
		alert('กรูณาระบุความเกี่ยวข้อง');
		return false;
	}
	...
	if(stayPlace=="2" && address6==""){
		...
		var url = '../main_exc/dr_doc1_child_exc.php?from='+from+'&idcard_mother='+idcard_mother+'&idcard='+idcard+'&prename='+prename+'&firstname='+firstname+'&surname='+surname+'&birthDate='+birthDate+'&bdoc_no='+bdoc_no+'&bdoc_staff='+bdoc_staff+'&docDate='+docDate+'&birthPlace='+birthPlace+'&presStatus='+presStatus+'&comment='+comment+'&stayPlace='+stayPlace+'&address1='+address1+'&address2='+address2+'&address8='+address8+'&address9='+address9+'&address3='+address3+'&address4='+address4+'&address5='+address5+'&address6='+address6+'&address7='+address7+'&eq_postcode='+eq_postcode+'&eq_mobilephone='+eq_mobilephone+'&eq_care_name='+eq_care_name+'&eq_care_lastname='+eq_care_lastname+'&eq_Relationship='+eq_Relationship+'&eq_id='+eq_id+'&eqc_id='+eqc_id;
		//alert(url);
    ...
    <tr>
     ...
     <tr>
    	<td align="left">
            ...
            <input type="radio" name="presStatus" id="presStatus_2" class="presStatus" value="2" <?php echo $check2;?>>&nbsp;ให้ผู้อื่นเลี้ยงดู (ชื่อ-สกุลผู้เลี้ยงดู) <span class="style1"> *</span>&nbsp;
			ชื่อ&nbsp;
			<input type="text" id="eq_care_name" name="eq_care_name" <?php echo $disabled; ?> value="<?php echo $row['eq_care_name'];?>" style="width:170px;" onkeypress='return chaonly()'> 
			นามสกุล&nbsp;
			<input type="text" id="eq_care_lastname" name="eq_care_lastname" <?php echo $disabled; ?> value="<?php echo $row['eq_care_lastname'];?>" style="width:170px;" onkeypress='return chaonly()'>
			เกี่ยวข้องเป็น&nbsp;
			<input type="text" id="eq_Relationship" name="eq_Relationship" <?php echo $disabled; ?> value="<?php echo $row['eq_Relationship'];?>" style="width:170px;" onkeypress='return chaonly()' size="10">
        </td>
    </tr>
     <tr>
    	<td align="left">
