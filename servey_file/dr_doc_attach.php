<?php
	...
	if($_GET['doc_type']=='APPV'){
		...
		$maxFileUploadError = ' maxFileCountErrorStr:"สามารถอัพโหลดได้แค่เพียง 1 ไฟล์เท่านั้น", ';
		$divHeight = '220';
		$req_id = $_GET['eq_id'];
	}
		...
		$.get("main/multi_upload_file/ajax_get_file_upload.php?req_id="+req_id+"&doc_id="+doc_id+"&doc_type=<?php echo $_GET['doc_type'];?>",function(data){
			...
			if( objLen > 0 ){
				...
				for( var i=0;i<obj.length;i++){
					...
					str += "<div style='margin-bottom:5px; font-size:14px !important;' id='file_"+req_id+"_"+obj[i]['doc_id']+"_"+obj[i]['doc_no']+"'><a href='"+obj[i]['full_path_name']+"' target='_blank' title='ดาวน์โหลด'><img src='../img/pdf.gif' align='absmiddle'/> "+(i+1)+".) "+obj[i]['caption']+"</a>&nbsp;&nbsp;<span onclick=\"editCaption('"+obj[i]['doc_id']+"','"+obj[i]['doc_no']+"')\" style='cursor:pointer;'><img src='../img/b_edit.png' title='แก้ไขคำอธิบาย' align='absmiddle' width='16px' height='16px'/></span>&nbsp;&nbsp;<span onclick=\"deleteFile('"+obj[i]['doc_id']+"','"+obj[i]['doc_no']+"','"+obj[i]['file_name']+"','"+obj[i]['caption']+"')\" style='cursor:pointer;'><img src='../img/p_del.gif' align='absmiddle' width='16px' height='16px' title='ลบ'/></span></div><div style='margin-bottom:5px; display:none;' id='edit_"+req_id+"_"+obj[i]['doc_id']+"_"+obj[i]['doc_no']+"'><input type='text' id='inputCaption_"+req_id+"_"+obj[i]['doc_id']+"_"+obj[i]['doc_no']+"' value='"+obj[i]['caption']+"'>&nbsp;&nbsp;<span onclick=\"saveCaption('"+obj[i]['doc_id']+"','"+obj[i]['doc_no']+"','"+obj[i]['caption']+"')\" style='cursor:pointer;'><img src='../img/save.gif' title='บันทึกการแก้ไข' align='absmiddle' width='16px' height='16px'/></span>&nbsp;&nbsp;<span onclick=\"cancelEdit('"+obj[i]['doc_id']+"','"+obj[i]['doc_no']+"')\" style='cursor:pointer;'><img src='../img/b_drop.png' title='ยกเลิกการแก้ไข' align='absmiddle' width='16px' height='16px'/></span></div>";
				}
			}else{
...
<body style="margin:0">
  <div id="accordion" style="width:85%;">
		...
		?>
            <h3><font  style="font-weight:bold; float:left;">เอกสารแนบ :: <?php echo $value['doc_type_name'];?></font>&nbsp;<?php echo $mark;?>&nbsp;</h3>
