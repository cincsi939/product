<?php
/**
 * @comment ฟอร์มหน้าการแสดงข้อมูลรายการรับเรื่อง
 * @projectCode 
 * @tor 
 * @package core
...
WHERE
	...
	$rsUser = mysql_fetch_assoc($QueryUser);
															if($rsUser['user_ID'] == ''){
																?>
																<a href="JavaScript:if(confirm('ไม่สามารถดำเนินการได้เนื่องจากไม่มีข้อมูลผู้ใช้บริการ ยืนยันการเพิ่มข้อูลผู้ใช้บริการ')==true){window.location='form_users.php?id_received=<?=$rs['sv_received_ID']?>';}">ยังไม่ได้ส่งเรื่อง</a>
																<?php
															}else{
																?>
																<a href="form_get_send.php?id_received=<?=$rs['sv_received_ID']?>&crisiscenter=<?=$rsCrisiscenter['groupname']?>&send=send">
																	...
																	$rsGetsend = mysql_fetch_assoc($getsendQuery);
																		...
																		if($rsGetsend['type_getsend'] == '0'){
																			$print = 'ยังไม่ได้ส่งเรื่อง';

																		}
																		if($rsGetsend['type_getsend'] == '1'){
																			$print = 'ส่งต่อเรื่องแล้ว';
														...
														<td colspan="2">
															...
															if($rsTransfer['status_trans'] == 'N' || $rsTransfer['status_trans'] == ''){
																?>
																<a href="JavaScript:if(confirm('ยืนยันการลบรหัสรับแจ้ง <?=$rs['sv_received_Label']?>')==true){window.location='delete.php?id_received=<?=$rs['sv_received_ID']?>';} "><img src="../../images/img_oscc/file_delete.png" align="absmiddle"  width="24" border="0"></a>
																<?
															}else{
																?>
																<a href="JavaScript:alert('รหัสรับแจ้ง <?=$rs['sv_received_Label']?> ไม่อนุญาติให้ลบข้อมูล') "><img src="../../images/img_oscc/file_delete.png" align="absmiddle"  width="24" border="0"></a>
