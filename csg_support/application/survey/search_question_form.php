...
/*	 if($_POST) 
	 {
		...
		if($keyword != "" ){
			...
			if(count($search)<=1){
				$search = trim($keyword);
				$where = " AND (eq_idcard like '%".$keyword."%' OR eq_firstname like '%".$keyword."%' 
						OR eq_lastname like '%".$keyword."%')";
				...
				//$replace = array();		
					...
					if($i==0){
						$where =" AND (eq_idcard like '%".$search[$i]."%' OR eq_firstname like '%".$search[$i]."%' 
						OR eq_lastname like '%".$search[$i]."%'    ";
						//$where2 = "AND (tbl2_name LIKE '%".$search[$i]."%' OR tbl2_idcard LIKE '%".$search[$i]."%' ";
					}else{
						$where .= " OR eq_idcard like '%".$search[$i]."%' OR eq_firstname like '%".$search[$i]."%' 
						OR eq_lastname like '%".$search[$i]."%'    ";
			...
			$sql ="SELECT
							...
							eq_person.eq_code_district,
							eq_person.eq_relation,
							eq_person.eq_age,
							eq_person.eq_code_gender,
							eq_person.eq_career,
							eq_person.eq_salary,
							eq_person.eq_parish,
							eq_person.eq_district,
							eq_person.eq_code_relation,
							eq_person.eq_partner_id
					FROM `eq_person`
					WHERE 1=1 $where
	";
	//echo $sql;
	...
	while($rs = mysql_fetch_assoc($query)){
		...
		$arr[trim($rs["eq_firstname"])." ".trim($rs["eq_lastname"])]["id_card"] = $rs["eq_idcard"];
		$arr[trim($rs["eq_firstname"])." ".trim($rs["eq_lastname"])]["name"] = $rs["eq_prename"].$rs["eq_firstname"]." ".$rs["eq_lastname"];
		$arr[trim($rs["eq_firstname"])." ".trim($rs["eq_lastname"])]["age"] = $rs["eq_age"];
		$arr[trim($rs["eq_firstname"])." ".trim($rs["eq_lastname"])]["sex"] = $rs["eq_code_gender"];
...
//		}
		if($rs["eq_career"] != ""){
				$arr[trim($rs["eq_firstname"])." ".trim($rs["eq_lastname"])]["occupation"] = $rs["eq_career"];
		}else{
				$arr[trim($rs["eq_firstname"])." ".trim($rs["eq_lastname"])]["occupation"] = "ไม่ระบุ";
		}
		$arr[trim($rs["eq_firstname"])." ".trim($rs["eq_lastname"])]["salary"] = $rs["eq_salary"];
		$arr[trim($rs["eq_firstname"])." ".trim($rs["eq_lastname"])]["district"] = $rs["eq_district"];
		$arr[trim($rs["eq_firstname"])." ".trim($rs["eq_lastname"])]["subdistrict"] = $rs["eq_parish"];
		$arr[trim($rs["eq_firstname"])." ".trim($rs["eq_lastname"])]["status"] = $rs["eq_relation"];
	//	$arr[trim($rs["question_firstname"])." ".trim($rs["question_lastname"])]["question_id"] = $rs["eq_code_relation"];
		$arr[trim($rs["eq_firstname"])." ".trim($rs["eq_lastname"])]["question_main"] =  $rs["eq_code_relation"];
		$arr[trim($rs["eq_firstname"])." ".trim($rs["eq_lastname"])]["partner"] = $rs["eq_partner_id"];
...
//";*/
//$sql_area = "SELECT eq_person.eq_idcard,eq_person.eq_parish,eq_person.eq_district FROM `eq_person` WHERE `eq_idcard` GROUP BY eq_idcard";
...
//		//echo "a";
//		$tambon[$rs_area["eq_idcard"]] = $rs_area["eq_parish"];
//		$aumphur[$rs_area["eq_idcard"]] = $rs_area["eq_district"];
//		
...
//		$arr[$rs2["tbl2_name"]]["district"] = $aumphur[$rs2["tbl2_idcard"]];
//		$arr[$rs2["tbl2_name"]]["subdistrict"] = $tambon[$rs2["tbl2_idcard"]];
//		if($rs2["tbl2_type"] == 1){
      ...
      <TABLE width="100%" style="border:1px #E2EEF3 solid;" id="search_key">
                            <TR>
                                    ...
                                    <TD align="right" valign="top">จำนวนที่ค้นพบ&nbsp;
                                            <?php 
											$sql_count_mem = "select sum(exsum_tb2) as count_mem from exsum_district_all";
  ...
  <tr>
    ...
    <thead>
      <tr  class="style7">
        ...
        <th width="121" class="hd_report"  data-sort="string" >เลขประจำตัวประชาชน <a href=""><img src="http://61.19.255.77/reportbuilder/img/<?php echo showIconOrder('id_card', $_GET['key_arr'], $_GET['sort']);?>"></a></th>
        <th width="170" class="hd_report" data-sort="string">ชื่อ-นามสกุล <a href=""><img src="http://61.19.255.77/reportbuilder/img/<?php echo showIconOrder('name', $_GET['key_arr'], $_GET['sort']);?>"></a></th>
        <th width="150" class="hd_report" data-sort="string">สถานะ <a href=""><img src="http://61.19.255.77/reportbuilder/img/<?php echo showIconOrder('status', $_GET['key_arr'], $_GET['sort']);?>"></a></th>
        <th width="70" class="hd_report" data-sort="float">อายุ(ปี) <a href=""><img src="http://61.19.255.77/reportbuilder/img/<?php echo showIconOrder('age', $_GET['key_arr'], $_GET['sort']);?>"></a></th>
        <th width="70" class="hd_report" data-sort="string">เพศ <a href=""><img src="http://61.19.255.77/reportbuilder/img/<?php echo showIconOrder('sex', $_GET['key_arr'], $_GET['sort']);?>"></a></th>
        <th width="120" class="hd_report" data-sort="string">อาชีพ <a href=""><img src="http://61.19.255.77/reportbuilder/img/<?php echo showIconOrder('occuaption', $_GET['key_arr'], $_GET['sort']);?>"></a></th>
        <th width="100" class="hd_report"  data-sort="string">รายได้ต่อปี (บาท) <a href=""><img src="http://61.19.255.77/reportbuilder/img/<?php echo showIconOrder('salary', $_GET['key_arr'], $_GET['sort']);?>"></a></th>
        <th width="63" class="hd_report" data-sort="string">อำเภอ <a href=""><img src="http://61.19.255.77/reportbuilder/img/<?php echo showIconOrder('district', $_GET['key_arr'], $_GET['sort']);?>"></a></th>
        <th width="58" class="hd_report" data-sort="string">ตำบล <a href=""><img src="http://61.19.255.77/reportbuilder/img/<?php echo showIconOrder('subdistrict', $_GET['key_arr'], $_GET['sort']);?>"></a></th>
       <!-- <th>จัดการ</th>-->
        </tr>
        </thead>
        <tbody>
...
WHERE 	question_detail_1.question_id='".$row['question_main']."' ";
	...
	$id_card_link = $row['partner']."&frame=form2";	
