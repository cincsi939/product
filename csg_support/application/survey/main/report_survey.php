 ...
 */
 ...
 $arrBGSearchIcon = array(	
	...
	function amountNM($report_id='', $type_id='' , $area_type='district'){
		if($type_id == 1){
			$sql_type = "sum(view_NM2.amount_child) AS num";
		}else if($type_id == 2){
			$sql_type = "sum(view_NM2.amount_young) AS num";
		}else if($type_id == 3){	
			$sql_type = "sum(view_NM2.amount_work) AS num";
		}else if($type_id == 4){	
			$sql_type = "sum(view_NM2.amount_old) AS num";	
		}else if($type_id == 5){	
			$sql_type = "sum(view_NM2.amount_cripple) AS num";		
		}
		if($area_type == 'district'){
			$area_type = 'district_id';
		}else if($area_type == 'subdistrict'){
			$area_type = 'subdistrict_id';
		}
		$sql = " SELECT
						count(view_NM2.id),
						sum(view_NM2.amount_total),
					  	{$sql_type}
					FROM
						view_NM2 
					WHERE view_NM2.{$area_type} ='{$report_id}'
						GROUP BY view_NM2.{$area_type}
					 ";
    ...
    <td width="60%">
		...
		$rowExHall = mysql_fetch_assoc($queryExHall);
          ...
          <tr>
            ...
            <td align="center" style="<?php echo $arr_style[$intExs%2];?>">
			<?php echo number_format(amountNM($rd['areaid'], $_GET['report_type'],$area_type));?>
            </td>
          </tr>
          <?php 
		  	$sumExH_all += $rowExH_all['num'];
		   	$sum_exh += $rowExH['num'];
		  	$sum_exp += amountNM($rd['areaid'], $_GET['report_type'], $area_type);
		  }
...
<thead>
  ...
  <tr style="background-color:#EEE;">
    ...
    <td align="center">
	<?php 
	//echo number_format($sum_question_num);
	echo number_format($sum_exp);
	?>
    </td>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
