...
$center = ($focus != '')?$focus:'14.482553,101.040344';
$width = ($_GET['width'] != '')?$_GET['width']:'';
$height = ($_GET['height'] != '')?$_GET['height']:'';
$emblem = ($_GET['emblem'] != '')?$_GET['emblem']:'y';
$arrDataCategories = explode("|", $categories);
$arrDataDatatarget = explode("|", $datatarget);
$arrDataDataset = explode("|", $dataset);
$maxData = @max($arrDataDataset);
$arrSetRate = setRateData($maxData);
...
var xml = [];
  function initialize() {
	...
	//var historicalOverlay = new google.maps.LatLngBounds(new google.maps.LatLng(-45.79987680242331,-67.35605667386972),new google.maps.LatLng(65.1054527203257, 200.9879833748254));
    map = new google.maps.Map(document.getElementById("map_canvas"), myOptions);
	//historicalOverlayBG = new google.maps.GroundOverlay('service/images/map_bg.png', imageBoundsBG);
			...
			downloadUrl(url, function(data) {
				...
				all_polygonMap[url] = Array();
			 	...
			 	for (var i = 0; i < markers.length; i++) {
					...
					all_markers[url][i] = createMarker(markers[i].getAttribute("name"), markers[i].getAttribute("identify"), markers[i].getAttribute("icon"), latlng,1,1);
...
<table border="0" cellspacing="0" cellpadding="0" style="" class="mapTitle">
        <tr  style="background-color:#333; color:#FFF;">
                 <td align="center" colspan="2">สัญลักษณ์</td>
         </tr>
         <tr style="display:none;">
                 <td width="45" align="center">
                 <img src="images/google_maps_marker.png"  width="30"/>
                 </td>
                 <td align="left">จุดเขตพื้นที่การปกครอง</td>
         </tr>
         <tr >
                 <td align="center">
...
<tr>
	<td>
    <input type="checkbox" id="xml_bg" name="xml_bg" value="xml.map_service_bg.php"  checked="checked"  onClick="addlayerXML(this);" />
	</td>
</tr>
<?php
foreach($arrDataCategories as $k=>$cate){
?>
<tr>
    <td>
<input type="checkbox" id="xml<?php echo $cate;?>" name="xml<?php echo $cate;?>" value="xml.map_service.php?ac=<?php echo $cate;?>&datatarget=<?php echo $arrDataDatatarget[$k];?>&data=<?php echo $arrDataDataset[$k];?>&max_data=<?php echo $maxData;?>"  checked="checked"  onClick="addlayerXML(this);" />
</td>
  </tr>  
<?php
}
