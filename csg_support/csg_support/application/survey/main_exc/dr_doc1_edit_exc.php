 ...
 */
 ...
 <?php
	...
	foreach( $VAR as $key => $val){
		...
		";*/
		$sqlVAR="
			UPDATE ".DB_DATA.".eq_var_data SET
			`siteid`='1',
			`number_action`='1', 
			form_id='".$trees."',
			`pin`='".$PIN."',
			...
			`user_id`='".$_SESSION['username']."' ,
			pin_idcard = '".$id_card."'
			WHERE  eq_id = '".$_POST['eq_id']."' AND vid='".$key."' 
		";
	...
	&& in_array($extension, $allowedExts)) {
	  if ($_FILES["v378"]["error"] > 0) {
		...
		//echo '<img src="'.$data.'" />';
                ...
                file_put_contents('../../../../repo_opp/profile/' . $nameNew , $data);
				$con->delete('eq_var_data','pin = '.$PIN.' AND eq_id = "'.$_POST['eq_id'].'" AND vid="378" AND form_id=1');
		$sqlVAR="
			INSERT INTO ".DB_DATA.".eq_var_data SET
			`siteid`='1',
	...
	//$con->delete('eq_person',"eq_idcard = ".$PIN);
	...
	//echo '<META HTTP-EQUIV="REFRESH" CONTENT="0;URL=../question_form.php?frame=dr_doc2&id='.$PIN.'">';
