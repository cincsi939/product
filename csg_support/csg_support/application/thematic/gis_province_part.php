...
#========= Set Config ============#
if($mapid != ''){
	$wherePart = " AND `partid` ='".$mapid."' ";
}else{
	$wherePart = "";
}
$strSQL = "SELECT ccDigi FROM ".$tbl_name." WHERE `ccType` = 'Changwat' ".$wherePart;
...
if($wherePart!=''){
	$strSqlPart = "SELECT partid AS ccDigi, g_point FROM `area_part` WHERE partid!='' ".$wherePart;
