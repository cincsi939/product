...
<body>
<script >
	//ฟังก์ชัน resize iframe บน report builder
	...
	function tooltipBox(x,y,text){
		...
		$("tooltip").style.left = x-x_tooltip+'px'; //ตำแหน่ง x อ้างอิงจาก style.left
		$("tooltip").style.top = y_tooltip+'px';//ตำแหน่ง y อ้างอิงจาก style.top
	...
	<?php // @modify Phada Woodtikarn 12/09/2014 เพิ่ม function Link ?>
...
#tooltip{
	...
	padding:5px;
	height:auto;
	width:250px;/*กำหนดความกว้างสูงสุด*/
	text-align:center;
...
if($level_area == 'province'){
    ...
    <?php
		...
		// @modify Phada Woodtikarn 11/09/2014 รูป map ใหม่
	...
	// @modify Phada Woodtikarn 12/09/2014 เพิ่มการจางสี
	...
	while($row_GYR = mysql_fetch_assoc($query_GYR)){
		...
		if($row_GYR['report_parish'] == 'อำเภอเมืองตราด'){
			$arr[$row_GYR['report_id']]['data'] = $row_GYR['report_parish']."<br>จำนวนเด็กแรกเกิดที่ขึ้นทะเบียน : 303 คน";
		}elseif($row_GYR['report_parish'] == 'อำเภอคลองใหญ่'){
			$arr[$row_GYR['report_id']]['data'] = $row_GYR['report_parish']."<br>จำนวนเด็กแรกเกิดที่ขึ้นทะเบียน : 32 คน";
		}elseif($row_GYR['report_parish'] == 'อำเภอเขาสมิง'){
			$arr[$row_GYR['report_id']]['data'] = $row_GYR['report_parish']."<br>จำนวนเด็กแรกเกิดที่ขึ้นทะเบียน : 159 คน";
		}elseif($row_GYR['report_parish'] == 'อำเภอบ่อไร่'){
			$arr[$row_GYR['report_id']]['data'] = $row_GYR['report_parish']."<br>จำนวนเด็กแรกเกิดที่ขึ้นทะเบียน : 122 คน";
		}elseif($row_GYR['report_parish'] == 'อำเภอแหลมงอบ'){
			$arr[$row_GYR['report_id']]['data'] = $row_GYR['report_parish']."<br>จำนวนเด็กแรกเกิดที่ขึ้นทะเบียน : 87 คน";
		}elseif($row_GYR['report_parish'] == 'อำเภอเกาะกูด'){
			$arr[$row_GYR['report_id']]['data'] = $row_GYR['report_parish']."<br>จำนวนเด็กแรกเกิดที่ขึ้นทะเบียน : 11 คน";
		}elseif($row_GYR['report_parish'] == 'อำเภอเกาะช้าง'){
			$arr[$row_GYR['report_id']]['data'] = $row_GYR['report_parish']."<br>จำนวนเด็กแรกเกิดที่ขึ้นทะเบียน : 2 คน";
		}
