...
for($type=1;$type<=5;$type++){
	...
	$sqlProblem = 'SELECT problem_detail, problem_type, problem_level FROM question_problem WHERE problem_type = '.$type.';';
	$resultProblem = mysql_query($sqlProblem);
	if($resultProblem){
		while($row = mysql_fetch_assoc($resultProblem)){
			$createSQL[] = '`problem_'.$row['problem_level'].'` int(2) NOT NULL DEFAULT "0" COMMENT "'.$row['problem_detail'].'",';
			$checkFiled['problem'][$row['problem_type']][$row['problem_level']] = 'problem_'.$row['problem_level'];
		};
	}
	unset($sqlProblem);
	unset($resultProblem);
	...
	//start question_help
	$sqlHelp = 'SELECT help_detail, help_type, help_level FROM question_help WHERE help_type = '.$type.';';
	$resultHelp = mysql_query($sqlHelp);
	if($resultHelp){
		while($row = mysql_fetch_assoc($resultHelp)){
			$createSQL[] = '`help_'.$row['help_level'].'` int(2) NOT NULL DEFAULT "0" COMMENT "'.$row['help_detail'].'",';
			$checkFiled['help'][$row['help_type']][$row['help_level']] = 'help_'.$row['help_level'];
		};
	};
	unset($sqlHelp);
	unset($resultHelp);
	...
	//start Create question_tbl2_Type
	$sql = 'CREATE TABLE `question_tbl2_Type'.$type.'`(';
	$sql .= ' `tbl2_id` varchar(20) NOT NULL COMMENT "รหัส",';
	foreach($createSQL as $value){
		$sql .= ' '.$value;
	};
	$sql .= ' PRIMARY KEY (`tbl2_id`)';
	$sql .= ' ) ENGINE=InnoDB DEFAULT CHARSET=tis620 ROW_FORMAT=COMPACT;';
	mysql_query('DROP TABLE IF EXISTS `question_tbl2_Type'.$type.'`;');
	$statusCreateTable = mysql_query($sql);
	//end Create question_tbl2_Type
	
	//start query
	$sqlTbl2 = 'SELECT tbl2_id, tbl2_type, tbl2_problem, tbl2_help FROM question_tbl2 WHERE tbl2_type = '.$type.';';
	$resultTbl2 = mysql_query($sqlTbl2);
	if($resultTbl2){
		$statusI = true;
		$i=0;
		while($row = mysql_fetch_assoc($resultTbl2)){
			$sql = 'INSERT INTO `question_tbl2_Type'.$type.'`';
			$filed = '';
			$result = '';
