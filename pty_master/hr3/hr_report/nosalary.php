              ...
              <table width="90%" border="0" cellspacing="2" cellpadding="0" align="center">
                <tr> 
				  ...
				  $d1=explode("-",$rs[fromdate]); 
				   ?>
                      วันที่
				  <select name="from_day"  id="from_day"  onChange="check_date('from_day','from_month','from_year'); withCalcutate();"></select>

				  
				เดือน
				<select name="from_month" id="from_month" onChange="check_date('from_day','from_month','from_year'); withCalcutate();"></select>

				
				ปี พ.ศ.
				<select name="from_year"  id="from_year" onChange="check_date('from_day','from_month','from_year'); withCalcutate();"></select>
          
<script>
	create_calendar('from_day','from_month','from_year','<?=$d1[2]?>','<?=$d1[1]?>','<?=$d1[0]?>');
</script><b>&nbsp;&nbsp;&nbsp;จำนวน&nbsp;&nbsp;<span id="showDifferent">0</span>&nbsp;&nbsp;วัน</b></td>
                </tr>
				<tr> 
                  ...
                  <td align="left" valign="middle" width="616"> 
					 ...
					 ?>
                     วันที่
				                       <select name="to_day"  id="to_day"  onChange="check_date('to_day','to_month','to_year'); withCalcutate();">
                    </select>
เดือน
<select name="to_month" id="to_month" onChange="check_date('to_day','to_month','to_year'); withCalcutate();">
</select>
ปี พ.ศ.
<select name="to_year"  id="to_year" onChange="check_date('to_day','to_month','to_year'); withCalcutate();">
</select>
<script>
...
function withCalcutate(){
	setTimeout(calulateDate(),500);	
}

function calulateDate(){
	var date1 = $('#from_day').val();
	var month1 = $('#from_month').val();
	var year1 = $('#from_year').val();
	var date2 = $('#to_day').val();
	var month2 = $('#to_month').val();
	var year2 = $('#to_year').val();
	
	if(date1 > 0 && month1 > 0 && year1 > 0 && date2 > 0 && month2 > 0 && year2 > 0){
		var d1 = new Date(year2,month2,date2); //todate
		var d2 = new Date(year1,month1,date1); // fromdate
		var msPerDay = 1000*60*60*24;
		$('#showDifferent').html(((d1 - d2) / msPerDay).toFixed(0));
	}
}
</script>