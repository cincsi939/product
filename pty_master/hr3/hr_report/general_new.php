...
if($prename_id == ""){
		...
		$prename_id = $rs_pre_id[PN_CODE];
				...
				$sql = "update general set prename_th=trim('$prename_th'),prename_id=trim('$prename_id'),name_th=trim('$name_th'),surname_th=trim('$surname_th'),prename_en=trim('$prename_en'),name_en=trim('$name_en'),surname_en=trim('$surname_en'),nationality='$nationality',citizenship='$citizenship',persontype='$persontype',royalid='$royalid',taxid='$taxid',sex='$sex',gender_id='$gender_id',blood='$blood',blood_id='$blood_id',position='$position',radub_past='$radub_past',marital_status_id='$marital_status_id',region='$region',home='$home',begindate='$begindate',begindate_label='$begindate_label',startposition='$startposition',birthday='$birthday',contact_add='$contact_add',subminis='$subminis',minis='$minis',subminis_now_label='$subminis_now_label',endday='$enddate',father_occ='$father_occ',mother_occ='$mother_occ',marry_occ='$marry_occ', contact_tel='$contact_tel' ,noteref='$noteref',secondname_th=trim('$secondname_th') ,secondname_en =trim('$secondname_en') , idcard='$idcard', marry_prename=trim('$marry_prename'), marry_name=trim('$marry_name'), marry_surname=trim('$marry_surname'), father_prename=trim('$father_prename'), father_name=trim('$father_name'), father_surname=trim('$father_surname'), mother_prename=trim('$mother_prename'), mother_name=trim('$mother_name'), mother_surname=trim('$mother_surname'),marry_kp7='$marry_kp7',marital_kp7_status_id='$marital_kp7_status_id',label_persontype2now='$label_persontype2now',status_gpf='$status_gpf' $up_startdate $start_date_manual  $up_birthday_label $up_retire_label $up_startdate_label  where id ='$id' ;";
...
if(strcall==1){
	$('#birthd_but').click(function(){popUpCalendar(document.getElementById("birthd"), document.getElementById("birthd"), "dd/mm/yyyy" ,'check_values_label("birthd","birthday_label","values","date","chk_date1");')});
		...
		$('#birthd_but').click(function(){popUpCalendar(document.getElementById("birthd"), document.getElementById("birthd"), "dd/mm/yyyy" ,'ajax_showdatethai();')});
	}
...
?>
									  ...
									  if(intval($rs_gm[id])>0){
										...
										$input_plus="<input name='sex' id='sex' type='hidden' value='$sex_select'>";
									...
									?>
                                      ...
                                      <td height="25" colspan="3">
                                       ...
                                       <input class="index2" style="FONT-SIZE: 11px; WIDTH: 80px" name="birthd_but" id="birthd_but"  type="button" value="วัน เดือน ปี" call="<?=$c_a?>" >
  ...
  <td colspan="3">
    ...
    <?
		...
		while($rs_gpf = mysql_fetch_assoc($result_gpf)){
			if($rs[status_gpf] == $rs_gpf[runid]){ $sel_gpf = " selected='selected'";}else{ $sel_gpf = "";}
			echo "<option value='$rs_gpf[runid]' $sel_gpf>$rs_gpf[gpf_detail]</option>";
...
-->เบอร์โทร
                                      ...
                                      <input name="contact_tel" type="text"   value="<?=$rs[contact_tel]?>" size="35" >                                      </td>
                                    </tr>
...
?>
                                      ...
                                      <input name="label_persontype2now" type="text" value="<?=$rs[label_persontype2now]?>" size="30" maxlength="255"></td>
...
?>
                                    ...
                                    <tr>
                                      ...
                                      <td width="44%"><select name="work" id="work"  style="width:300; color:green" disabled="disabled" > 
...
<?
						...
						$sql_area = "SELECT count(staffid) as num1 FROM keystaff WHERE  site_area='6402' and status_permit='YES' group by site_area";
						$result_area = mysql_db_query($dbnameuse,$sql_area);
...
?>									  
                                        ...
                                        <select name="schoolid" style="width:300; color:green" <?=$dis_lock?>  >       
					...
					//	}	######	if($rs[schoolid] != ""){					
						...
						while($urs=mysql_fetch_array($uresult,MYSQL_ASSOC)){
							if($urs[id]==$rs[schoolid]){
								echo "<option value='$urs[id]' SELECTED>โรงเรียน$urs[office]</option>";
							}else{
								echo "<option value='$urs[id]'>โรงเรียน$urs[office]</option>";
							} ## if($urs[id]==$rs[schoolid]){ 
						} ## while($urs=mysql_fetch_array($uresult,MYSQL_ASSOC)){
					 ...
					 ?>
                                   ...
                                   // echo $rs[minis_now]."<br>".$rs[subminis_now];
                                      ...
                                      <select name="minis_now"  style="width:300; color:green"  onChange="getorg(this.value)"  disabled="disabled">
                                        <? 
									   $id_org="";
									  $select1  = mysql_query("select runid, ministry as ORG_NAME  from $dbnamemaster.hr_addministry where OL_CODE='01' order by ministry;");
									  
									  while ($rselect1=mysql_fetch_array($select1,MYSQL_ASSOC)){
                                      $sel="";
									  if($rs[minis_now]==""){
											if(trim($rselect1[ORG_NAME])=="กระทรวงศึกษาธิการ"){
												$sel="selected";
												$id_org=$rselect1[runid];
												}  
									  }else{
										if(trim($rs[minis_now])==trim($rselect1[ORG_NAME]))//ถ้า 
										{
										  $sel="selected";
										  $id_org=$rselect1[runid];
										}  
...
#### ชื่อเดือนภาษาไทย	
$month_nameth = array("01"=>"มกราคม","02"=> "กุมภาพันธ์","03"=> "มีนาคม", "04"=>"เมษายน","05"=> "พฤษภาคม", "06"=>"มิถุนายน", "07"=>"กรกฏาคม", "08"=>"สิงหาคม","09"=> "กันยายน", "10"=>"ตุลาคม","11"=> "พฤษจิกายน", "12"=>"ธันวาคม");
