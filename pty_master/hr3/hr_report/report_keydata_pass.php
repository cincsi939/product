...
<table width="99%" border="0" align="center" cellpadding="0" cellspacing="0">
  <tr>
    <td bgcolor="#F5F5F5"><table width="100%" border="0" cellspacing="1" cellpadding="3" class="tbl3">
      <tr>
        <td colspan="6" align="left" bgcolor="#CCCCCC"><strong>รายชื่อข้าราชการครูและบุคลากรทางการศึกษาที่ทำการบันทึกข้อมูลเสร็จแล้ว<?=$secname?>
        </strong></td>
        </tr>
      <tr>
        ...
        <td width="26%" align="center" bgcolor="#CCCCCC"><strong>ตำแหน่ง</strong></td>
        <td width="29%" align="center" bgcolor="#CCCCCC"><strong>หน่วยงานสังกัด</strong></td>
...
Inner Join ".DB_USERENTRY.".view_kp7approve as t2 ON t1.idcard = t2.idcard AND t1.siteid = t2.siteid AND t1.profile_id=t2.profile_id
Left Join ".DB_MASTER.".hr_addposition_now t3 ON t1.position_now = t3.position
WHERE
...
(status_numfile='1' and status_check_file='YES' and mainpage ='0' and status_file='1' and status_id_false='0')) and t1.profile_id =  '$profile_id'  and t1.siteid='$xsiteid' $conv
group by t2.idcard
order by orderbyschoolid,t3.orderby asc";
//echo $sql1;
$result1 = mysql_db_query($dbnameuse,$sql1) or die(mysql_error()."<hr>$sql1<hr>LINE__".__LINE__);
$i=0;
$prefix_url = "http://".APPHOST;
while($rss = mysql_fetch_assoc($result1)){
	 if ($i++ %  2){ $bg = "#F0F0F0";}else{$bg = "#FFFFFF";}
	 	 $urlpath = "../../edubkk_kp7file/$rss[siteid]/" .$rss[idcard] . ".pdf"  ;
		 $linkfile = $prefix_url."/edubkk_kp7file/$rss[siteid]/" . $rss[idcard] . ".pdf"  ;
	 if(is_file($urlpath)){
			$pdf= " <a href=\"$linkfile\" target='_blank'><img src='../images_sys/gnome-mime-application-pdf.png' alt='ก.พ.7 สำเนาจากต้นฉบับ' width='16' height='16' border='0'></a> " ;  
	}else{
