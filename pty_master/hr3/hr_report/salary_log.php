...
echo"
  ...
  /*      $str=setorder_type($select_type,$txt_money,$select_school,$select_school_label,$select_school_name);     
	...
	$sql	.= " pos_name, runno,instruct,label_date,label_salary,label_dateorder,label_radub,label_noposition,note,level_id,position_id,system_type) VALUES ('$id', '$date', '$pid_text', '$noposition', '$hr_addradub_text', '$salary_st', '$upgrade',  '$noorder', ";
	$sql	.= " '$ch_position', '$ch_radub','$ch_salary','$pls','$dateorder','$pos_name','$runno','$instruct','$label_date','$label_salary','$label_dateorder','$label_radub','$label_noposition','$txt_note','$hr_addradub','$hr_addposition','$elec_paper') ";			
//echo $sql."<hr>";
	//echo $sql."<br>";
...
// check confirm การบันทึกข้อมูลเงินเดือนในกรณีที่ข้อมูลเงินเดือนไม่ตรงตามผังเพื่อเก็บ log
function check_confirmsalary(){
var flag_error = "N";
var year=document.post.salary_year.value;	
var month=document.post.salary_month.value;	
var money=document.post.salary.value;
...
return false;
if(err_salary == 1){
	if(confirm("เงินเดือนไม่ถูกต้องตรงตามผังคุณต้องการที่จะทำการบันทึกหรือไม่?")){
		alert('submit :: '+err_salary);
	}
}else{
		alert('submit :: '+err_salary);	
}
  ...
  <table width="100%" border="0" cellspacing="3" cellpadding="2" align="center">
            ...
            <td>
                    ...
                    <table width="580" border="0" cellpadding="1" cellspacing="2" bgcolor="#DEDFDE">        
                            <tr>
                                    <td>
                                   			<input type="checkbox" name="elec_paper" value="SYSTEM_ELEC" id="elec_paper" <?=($rs[system_type]=="SYSTEM_ELEC")?'checked="checked"':''?> <?=$disb1?>><?=$txt_data?> 
                                    </td>
                            </tr>
                    </table>
            ...
            <div class="tabbertab">
              ...
              <table width="100%" cellspacing="1" cellpadding="2" align="center" bgcolor="#808080">
                ...
                <tr bgcolor="#f8f8f8">
                  ...
                  <td width="70%"><input type="text" name="noposition" id="noposition" size="25" value="<?=$rs[noposition]?>" onKeyPress="return noSpChar(event)" onBlur="onobjChange(); Check_numberPosition('on_blur');"></td>
                </tr>
                <tr bgcolor="#f8f8f8">
...
?>
  ...
  <tr valign="middle"> 
    ...
    <input type="hidden" NAME="remark" VALUE="<?=$remark?>"><input type="button" name="cancle" value="ยกเลิก" onClick="location.href='salary.php?action=edit&YY=<?=$YY?>&viewall=<?=$v?>' "></td>
  </tr>
  </table>
  </form>
  <?
