...
class activity_log{
    ...
    function save_log($idcard="0000000000000",$name="",$sername="",$server="",$activity="",$temp="",$expire="",$admin_id='',$admin_name='',$admin_sername='',$request_type='',$insertid=''){
		$samp_code = $this->GetRandomString(13);
		$this->scode = $samp_code;
		$this->idcardnm = $idcard;

		if ($request_type  > 90000  ){ 
			$childdb = STR_PREFIX_DB. $_SESSION[secid] ; 
			$sql = " SELECT   comment   FROM   $childdb.req_print_kp7   WHERE  runid  = '$insertid' "; 
			$result = @mysql_query($sql);
			$rs = @mysql_fetch_assoc($result) ; 
			$xxcomment = $rs[comment] ; 
		}else{  $xxcomment = "";  }

 		if ($xxcomment == ""){			$xxcomment = $sql ; 		}

		$sql = "INSERT INTO $this->system_db.`activity_log` (`sampling_code`,`id_card`,`owner_name`,`owner_sername`,`server_id`,`activity`,`date`,`temp_file`,`expire_date`,`admin_id`,`admin_name`,`admin_sername`, user_sessionid	 , request_type ,	 use_for) VALUES ('$samp_code','$idcard','$name','$sername','$server','$activity',NOW(),'$temp','$expire','$admin_id','$admin_name','$admin_sername', '$_SESSION[id]' , '$request_type' , '$xxcomment')";
#user_sessionid				<====== $_SESSION[id]  
