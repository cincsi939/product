...
function get_level_salary($radub,$salary,$radub_id){
 global    $year ,$xxx;
     if($radub=="ทักษะพิเศษ" or $radub=="ชำนาญงาน" or $radub=="เชี่ยวชาญ" or $radub=="อาวุโส" or $radub=="ปฏิบัติงาน" or $radub=="ทรงคุณวุติ" or   $radub=="ชำนาญการ" or  $radub=="ชำนาญการพิเศษ" or $radub=="ปฏิบัติการ"  ){
        ...
        Inner Join tbl_salary_level ON tbl_salary_math_radub.salary_radub_id = tbl_salary_level.salary_radub_id
        Inner Join tbl_salary_radub ON tbl_salary_level.salary_radub_id = tbl_salary_radub.salary_radub_id
        where  tbl_salary_math_radub.radub_id='$radub_id'  and tbl_salary_level.money='$salary' and  tbl_salary_radub.profile_id in(SELECT profile_id FROM tbl_salary_profile Where  active_status='1' and yy='$year')";
        $xxx=$sql_level;
...
function get_runid_viataya_id($radub){
    $sql="SELECT  runid,    radub       FROM hr_addradub  where level_id='$radub'";
    $result=mysql_db_query(DB_MASTER,$sql);
    $row=mysql_fetch_array($result); 
    return $row[runid];
}
#=========================================================================   
$_REQUEST["radub_id"]  ="4252127";
$_REQUEST["radub"]="5";
$_REQUEST["salary"]="9560"; 
 
$_REQUEST[xidcard] ="3360100987810"; 
$_REQUEST[xsiteid] ="4702"; 
$_REQUEST[year] ="2538"; 
$_REQUEST[month] ="10"; 
$_REQUEST[day] ="01"; 
 
 
$radub_id =   $_REQUEST["radub_id"]  ;  
$radub2 =   $_REQUEST["radub"]  ; 
$salary2 = str_replace(',','',  $_REQUEST["salary"] ) ;  
//$radub2_id= get_runid_viataya($radub2); 
$radub2_id= get_runid_viataya_id($radub_id); 
...
$date_sel="$year-$month-$day";
  $sql="SELECT   `date`,  radub, salary ,level_id FROM salary where id='$xidcard' and date(`date`) < date('$date_sel')order by  date(`date`) desc  limit  1  ";
