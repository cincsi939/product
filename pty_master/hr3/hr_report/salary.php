<?php  ######################  start Header ########################
/**
* @comment ฟอร์มบันทึกตำแหน่งและอัตราเงินเดือน
...
function savevitaya($salary_id,$idcard,$vataya_id,$date_start,$noorder,$dateorder){
  ...
  if($vataya_id==""){   //เลือกไม่ระบุ  จะลบข้อมูล
    ...
    if($rs_COUNT[COUNTN]>0){
     ...
     $resultx1 = mysql_db_query($dbname," SELECT  *  FROM  vitaya_stat  WHERE   id ='$idcard' and remark='$noorder' "); 
     ...
     if($numrow>0){
         $sql="update vitaya_stat set 
		  ...
		  remark='$noorder' where id='$idcard' and remark='$noorder'";
     }else{
...
$resultx1 = mysql_db_query($dbname," SELECT  remark  FROM  vitaya_stat  WHERE    id ='$id'  AND  remark = '$noorder_old' ");
	...
	$sql_name=mysql_db_query($dbname,"select * from vitaya_stat WHERE id='$id' and remark = '$hr_vitaya_text' ");
...
$name = trim($row["office"]." [".$row_aumpur['ccName']." ".$row_tamboon['ccName']."] : ". str_replace("สำนักงานเขต", "สนข.", $row["secname"]));
return $name;
}
?>
...
if($row[schoolid]==$row[secid]){
    ...
    $def_schoolname=$row[office].":". str_replace("สำนักงานเขต","สนข.",$row[secname]) ;	
	}
...
?>
  ...
  <thead>
  <tr bgcolor="#A3B2CC" align="center">
  <th height="27" colspan="8" bgcolor="#2C2C9E"><span class="style1">การเลื่อนขั้นตำแหน่ง(แสดงข้อมูลทั้งหมด)</span></th>
  </tr>
  <tr bgcolor="#A3B2CC" align="center" style="font-weight:bold;">
...
<table width="100%" border="0" cellspacing="2" cellpadding="2" align="center" bgcolor="black">
  ...
  <tr bgcolor="#A3B2CC" align="center">
  <td height="27" colspan="7" bgcolor="#2C2C9E"><span class="style1">การเลื่อนขั้นตำแหน่ง(แสดงข้อมูลช่วงปี)</span></td>
  </tr>
  <tr bgcolor="#A3B2CC" align="center" style="font-weight:bold;">
...
<script>
  ...
  <tr>
    ...
    <td width="787" align="left" valign="top"><table width="580" border="0" cellpadding="1" cellspacing="2" bgcolor="#DEDFDE">
        <tr>
          <td><span class="style8">ให้นำข้อมูลที่อยู่ในช่องตำแหน่งใน ก.พ. 7 มากรอก เช่น อาจารย์1 โรงเรียน xxxxx สนข. xxxx เป็นต้น </span></td>
        </tr>
        <tr>
            ...
            <div class="tabbertab">
              ...
              <table width="100%" cellspacing="1" cellpadding="2" align="center" bgcolor="#808080">
                ...
                <tr bgcolor="#f8f8f8">
                  ...
                  <td width="70%"><input type="text" name="noposition" id="noposition" size="25" value="<?=$rs[noposition]?>"  onBlur="onobjChange(); Check_numberPosition('on_blur');"></td><!--onKeyPress="return noSpChar3(event)"-->
                </tr>
                <tr bgcolor="#f8f8f8">
