...
$module_code 		= "add_refdoc"; 
$process_id			= "add_refdoc";
$VERSION 				= "9.91";
$BypassAPP 			= true;
...
clearBrowserCache();
#@modify Suwat.k เพิ่มพารามิเตอร์สำหรับแนบเอกสารหลักฐาน
$siteid = trim(str_replace(STR_PREFIX_DB,"",$_SESSION[temp_dbsite]));
$dbsite = STR_PREFIX_DB.$siteid;

$path_checklist = "../../../../".PATH_KP7_REFDOC_FILE."/"; #edubkk_checklist_kp7file
$path_kp7 = "../../../../".PATH_KP7_REFDOC_FILE."/"; #edubkk_kp7file
$url_file = $path_kp7.$siteid."/".$id."R.pdf";
if(is_file($url_file)){
	$img_pdf = 	"<a href='$url_file' target='_blank'><img src=\"../../../images_sys/pdf_ref.png\" title=\"แสดงเอกสารหลักฐาน\" width=\"16\" height=\"16\" border=\"0\"></a>";
}else{
	$img_pdf = "";	
}
	...
	if($_SERVER['REQUEST_METHOD'] == "POST"){
		...
		//echo $siteid;die;
			...
			if($kp7file_name != ""){
				...
				$page_num = CopyFileToSite($kp7file,$kp7file_name,$id,$path_source,$path_dest,$conF1,"R");// upload ไฟล์
				$arrp1 = explode("_",$page_num);

				#echo '<pre>';print_r($arrp1);
				#echo $kp7file_name.','.$kp7file_name.','.$id.','.$path_source.','.$path_dest.','.$conF1.''.$page_num;
				#die;
				if($arrp1[0] == "x"){
				
					$xstaff_key = $_SESSION[session_staffid];
					$xip = GET_IPADDRESS();
					$sql_log = "insert into log_upload_kp7false(idcard,siteid,server_ip,user_upload,page_false,time_upload,type_doc)values('$id','$siteid','$xip','$xstaff_key','".$arrp1[1]."',NOW(),2);";
					mysql_db_query($dbnamemaster,$sql_log);

					echo"
                <script language=\"javascript\">
                alert(\" ไม่สามารถนำเข้าได้เนื่องจากจำนวนแผ่นที่นำเข้าน้อยกว่าจำนวนแผ่นก่อนหน้า\\n \");
                </script><meta http-equiv='refresh' content='0;URL=add_refdoc.php?id=".$id."&action=edit'>
           ";
				}else{
					...
					if($conF1 == "1"){
						$xstaff_key = $_SESSION[session_staffid];
						$xip = GET_IPADDRESS();
						$sql_log = "insert into log_upload_kp7false(idcard,siteid,server_ip,user_upload,page_false,time_upload,status_conf,type_doc)values('$id','$siteid','$xip','$xstaff_key','".$arrp1[1]."',NOW(),'1',2)";
						mysql_db_query($dbnamemaster,$sql_log);	
					}
				...
				#### update checklist
				$sql_update = "UPDATE tbl_checklist_kp7 SET  num_ref='$page_num',num_actual='$page_num' WHERE idcard='$id' and profile_id='$rsc[profile_id]' and siteid='$rsc[siteid]' ";
				
				mysql_db_query($dbname_temp,$sql_update) or die(mysql_error()."$sql_update<br>LINE__".__LINE__);
				//echo $sql_update;die;
				add_log("ข้อมูลไฟล์เอกสารหลักฐาน",$id,"add",$menu_id);	
				
				echo"
                <script language=\"javascript\">
                alert(\" อัปโหลดเอกสารหลักฐานเรียบร้อยแล้ว \\n \");
                </script>
				<meta http-equiv='refresh' content='0;URL=add_refdoc.php?id=".$id."&action=edit'>
            ";
...
<title>Upload ไฟล์เอกสารหลักฐาน</title>
<style type="text/css">
<!--
...
$(document).ready(function(e) {
	$('#submit').click(function(e) {
        var txt = $('#kp7file').val();
		if(txt==''){
			alert('กรุณาเลือกไฟล์ เอกสารหลักฐาน');
			$('#kp7file').focus();
			return false;
		}
    });
...
<tr bgcolor="#dddddd">
    <td height="20"><strong>แนบเอกสารหลักฐาน</strong></td>
</tr>	
<tr bgcolor="#ffffff">
       ...
       <table width="100%" border="0" cellspacing="0" cellpadding="0">
         <tr>
           <td><table width="100%" border="0" cellspacing="1" cellpadding="3">
             <tr>
               <td width="25%" align="right">เลือกไฟล์เอกสารหลักฐาน:</td>
			 ...
			 #@modify Suwat.k 06/08/2015 ตรวจสอบเงื่อนไขแสดงกรณีมีข้อมูลใน checklist เท่านั้น
			  if($arrp1[profilename] != ""){
				?>
             <tr>
               <td align="right">จัดทำข้อมูล ณ วันที่ :</td>
