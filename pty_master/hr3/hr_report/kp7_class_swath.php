<?
require_once('../../../../thai_word/thaiwordwrap.class.php');
...
function alignstr($strin, $cutlength) {
		$wordwrap= new thaiwordwrap($strin,$cutlength);
		
		return $wordwrap->Arrayword;
}
...
function cfooter($Gety1=-1){
		...
		*/
		...
		$this->Cell(194,6,"๑๔. ชื่อ  $rs[prename_th] $rs[name_th] $rs[surname_th]    ระดับ  $rs[radub]   $strMinisNow   กรม  $subminis_now ",0,0,'L');
}	// end function cfooter

	function Footer()
	{
		
	$this->SetFillColor(0,0,0);
		global $yy,$date_now,$monthname,$mm_now,$activitylog_barcode,$preview_status,$id,$xsiteid, $activity_id, $expire_datesql;
		...
		if($activity_id == 'E0001'){
			$expire_date = $this->ShowDateThai($expire_datesql);
		}
		else{
