...
class FPDF_Protection extends FPDF 
{
	...
	var $xsiteid="";
	var $dbsite;
	var  $status_keydata=1; // คือสถานะการคีย์ข้อมูลเรียบร้อยแล้ว
	...
	function date_salary($idcard,$xsiteid){
		...
		$this->date_datakp7=$this->ShowDateThai($datekey);
	...
	function datechecklist($idcard,$xsiteid){
		global $dbname_temp,$status_keydata;
	...
	function ShowDateThai($xdate){
		if($xdate != "" and $xdate != "0000-00-00"){
			...
			if($arrd[0] > 2500){
					...
					return intval($arrd[2])." ".$this->month_th[intval($arrd[1])]." ".($arrd[0]+543);	
			}//end if($arrd[0] > 2500){
		}else{
			return "";		
