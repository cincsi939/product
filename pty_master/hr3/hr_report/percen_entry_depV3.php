...
<tr bgcolor="#A3B2CC"  style="font-weight:bold;"     align="center"       >
	<td width="4%" height="51" rowspan="2" class="boder_rb">ลำดับ</td>
	<td rowspan="2" class="boder_rb">สำนักงานเขตพื้นที่</td>
	<td colspan="5" class="boder_rb">จำนวนบุคคลากรที่ต้องบันทึกข้อมูล (คน)<br />
    ตามหนังสือเลขที่ ศธ.0206.6/1158 ลว.24 ก.ย. 2550 </td>
  </tr>

<tr bgcolor="#A3B2CC"  style="font-weight:bold;"     align="center"            >
  <td width="7%" height="51" class="boder_rb">ทั้งหมด  </td>
  <td width="8%" class="boder_rb">ไม่มีการ นำข้อมูลเข้าสู่ระบบ </td>
  <td width="8%" class="boder_rb">อยู่ระหว่าง บันทึกข้อมูล(ยังไม่รับรอง) </td>
  <td width="15%" class="boder_rb">ผ่านการ รับรองข้อมูล </td>
...
<u><?=$depname?></u><a> </td>
	...
	$arr_notkey[$depid]  = $arr_arearef[$depid]  -  $arr_keyin[$depid]    ; 

 

?>
      <?=number_format($arr_notkey[$depid] )?></td>
	<td align="right" class="boder_rb"><?
 
	$arr_keying[$depid]  = $arr_keyin[$depid]  - $arr_approve[$depid]  ; 
	echo $arr_keying[$depid]  ; 
	?></td>
	<td align="right" class="boder_rb"><?
...
?>
	    <?=number_format($arr_approve[$depid])?>    </td>
    <td align="right" class="boder_rb"> 
<?
$xx1 = @($arr_keyin[$depid]  * 100 )/  $arr_arearef[$depid]  ; 
...
<tr bgcolor="#A3B2CC" align="right" style="font-weight:bold;">
	...
	$xnotkey = @array_sum($arr_notkey) ;
	$keyin_ing = $xkeyin -   $xapprove ; 	
#	$keyin_ing = $xgain -   $xapprove ;  ?>
      <?=number_format($xnotkey)?></td>
    <td align="center" class="boder_rb">
<?  $xnotkey = @array_sum($arr_keying)   ?>	
	<?=number_format($xnotkey)?></td>
...
<tr bgcolor="#A3B2CC" align="right" style="font-weight:bold;">
  <td colspan="7" align="left" class="boder_rb">&nbsp;</td>
  </tr>
</table>
