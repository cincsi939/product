...
$competency_system_db = $dbsystem ;
$edubkk_master_db = $dbnamemaster ;
$xxusername=  $hr_username  ; 
$xxpassword =  $hr_password ;
...
function checkFields() {
	missinginfo1 = "";
	missinginfo = "";
//	#if (document.form1.search_name.value == "")  {	missinginfo1 += "\n        - ชื่อผู้ใช้ ไม่สามารถเป็นค่าว่าง"; }		
//	if(document.form1.search_sername.value=="") { missinginfo1 += "\n        - รหัสผ่าน ไม่สามารถเป็นช่องว่าง";  }
//	if(document.form1.search_id.value=="") { missinginfo1 += "\n        - รหัสผ่าน ไม่สามารถเป็นช่องว่าง";  }
  	xform = "document.form1"; 
	if ((document.form1.search_name.value == "")||(document.form1.search_sername.value=="")||(document.form1.search_id.value=="")){
		missinginfo1  = "notok"; 
	}

	if (missinginfo1 != "") { 
		missinginfo  = "ขออภัย ต้องเลือก ชื่อ ,นามสกุล หรือ รหัสประจำตัว  อย่างน้อยหนึ่งรายการ \n";
		alert(missinginfo);
		return false;
		}
	}
  ...
  <tr>
    <td><table width="100%" border="0" cellspacing="0" cellpadding="3">
      <tr>
        ...
        <td width="84%"><select name="txt_siteid">
		  ...
		  if($rs1[secid] == $txt_siteid){ $selected = "selected";}else{ $selected=""; }
		   ?>
          <option value="<?=$rs1[secid]?>"<?=$selected?>>
            <?=$rs1[secname]?>
            </option>
          <? } ?>
        </select></td>
...
#	echo " ขออภัย ไม่ได้รับสิทธิในการเข้าถึง ";
} ##### END  if ($allow_kp7 != "no"){    ?>		  
          </label>
        </span></td>
      </tr>
...
<? if ($allow_kp7 == "no"){           
	echo " <br> ขออภัย ไม่ได้รับสิทธิในการเข้าถึง ";
} ##### END  if ($allow_kp7 == "no"){    ?>		 
