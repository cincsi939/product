...
## Created By :                 xxxxx
 ## E-mail :                           xxxx@sapphire.co.th
 ## Tel. :                                xxxxxx
## Company :                    Sappire Research and Development Co.,Ltd. (C)All Right Reserved
 ###################################################################
## Version :                        			20131121.001
## Modified Detail :                        แก้ไขความผิดพลาดของการแสดงผล listbox ของการเพิ่ม และ แก้ไข วุฒิการศึกษา 
## Modified Date :                          2013-11-21 04:50
 ## Modified By :                             วิเศษณ์ วิเศษวัชระเจริญ (วิ)
###################################################################
session_start();
...
//	echo "<pre>";
//	print_r($_POST[);
	$id = $_POST["id"] ; // แก้ไข กรณีบันทึกแล้วส่ง id เป็น 1 มา
	add_log("ข้อมูลการศึกษา",$id,$action,$menu_id);
	
	if(!get_magic_quotes_gpc()){
		$place	= addslashes($place);
		$grade = addslashes($grade);
		$year_label = addslashes($year_label);
		$level1 = addslashes($level1);
		$major1 = addslashes($major1);
		$minor_1 =addslashes($minor_1);
		$university1 = addslashes($university1);
 	}
...
//		print_r($_POST);die;
		...
		if(count($sent_type) > 0){
			foreach($sent_type as $k => $v){
				if($v == "Y"){// เรื่องของการศึกษา
					$sql		= " update graduate set runno='".$runno[$k]."' where id='".$id."' and runid='".$runid[$k]."' ";		
					$result 	= mysql_query($sql)or die("Query line ". __LINE__ ." error<hr>".mysql_error());
				}else{// เรื่องการฝึกอบรม
					$sql		= " update seminar set runno='".$runno[$k]."' where id='$id' and runid='".$runid[$k]."' ";		
					$result 	= mysql_query($sql)or die("Query line ". __LINE__ ." error<hr>".mysql_error());
				}//end foreach($sent_type as $k => $v){
			}//end foreach(){
		}//end if(count($sent_type) > 0){
...
function check_doc(){
	var chk_choice = "";
	if(document.post_doc.show[0].checked){
		chk_choice = document.post_doc.show[0].value; // วุฒิในประเทศ
	}else if(document.post_doc.show[1].checked){
		chk_choice = document.post_doc.show[1].value; // วุฒิต่างประเทศ
	}
//alert(chk_choice);
...
/*if(document.post_doc.finishyear.value.length == 0){
		alert("ระบุ ปีพ.ศ. เพื่อเก็บข้อมูลทางสถิติ");
		document.post_doc.finishyear.focus();
		return false;
}
*/
if(chk_choice == "d1"){ // กรณีเป็นการศึกษาในประเทศ
	/* if(document.post_doc.university.value.length == 0 && document.post_doc.place.value.length == 0){
			alert("กรุณาระบุสถานศึกษา");
			document.post_doc.university.focus();
			return false;
	 }else*/ if(document.post_doc.level1_a1.value == "" && document.post_doc.grade.value == ""){
		alert("กรุณาระบุวุฒิการศึกษา");
		document.post_doc.level1_a1.focus();
		return false;
}

}else if(chk_choice == "d2"){
		if(document.post_doc.level_2.value == ""){
			alert("กรุณาระบุวุฒิการศึกษา");
			document.post_doc.level_2.focus();
			return false;
		}else if(document.post_doc.university_2.value == ""){
				alert("กรุณาระบุสถาบันการศึกษา");
			document.post_doc.university_2.focus();
			return false;
		}
}//end if(chk_choice == "d1"){
...
return (false);
		...
		return false;
}*//*else if(document.post_m.university.value.length == 0 & document.post_m.place.value.length == 0 & (post_m.show[1].checked == false)){
		alert("กรุณาระบุสถานศึกษา");
		document.post_m.university.focus();
		return false;
}else */if(document.post_m.level1_1.value.length == 0 & document.post_m.grade.value.length == 0 & (post_m.show[1].checked == false)){
		alert("กรุณาระบุวุฒิการศึกษา");
...
return (false);
		...
		return false;
}*//*else if(document.post_mm.university.value.length == 0 & document.post_mm.place.value.length == 0 & (post_mm.show[1].checked == false)){
		alert("กรุณาระบุสถานศึกษา");
		document.post_mm.university.focus();
		return false;
}else*/ if(document.post_mm.level1_1.value.length == 0 & document.post_mm.grade.value.length == 0 & (post_mm.show[1].checked == false)){
		alert("กรุณาระบุวุฒิการศึกษา");
...
function check(){
	if(document.post1.place.value.length == 0){
		...
		return false;
	}else if(document.post1.year_label.value.length > 0 && document.post1.finishyear.value==""){
		alert("ระบุวันที่");
		document.post1.year_label.focus();
		return false;
...
function check2(){
	if(document.post2.place.value.length == 0){
		...
		return false;
	}else if(document.post2.year_label.value.length > 0 && document.post2.finishyear.value==""){
		alert("ระบุวันที่");
		document.post2.year_label.focus();
		return false;
...
function check3(){
	if(document.post3.place.value.length == 0){
		...
		return false;
	}else if(document.post3.year_label.value.length > 0 && document.post3.finishyear.value==""){
		alert("ระบุวันที่");
		document.post3.year_label.focus();
		return false;
...
function check4(){
	if(document.post4.place.value.length == 0){
		...
		return false;
	}else if(document.post4.year_label.value.length > 0 && document.post4.finishyear.value==""){
		alert("ระบุวันที่");
		document.post4.year_label.focus();
		return false;
...
function check5(){
	if(document.post5.place.value.length == 0){
		...
		return false;
	}else if(document.post5.year_label.value.length > 0 && document.post5.finishyear.value==""){
		alert("ระบุวันที่");
		document.post5.year_label.focus();
		return false;
...
if($action ==""){
	$sql_show = "SELECT 
runid as run_id,if(runno > 0,runno,99)as run_no,place as place ,startyear as s_year,finishyear as e_year,year_label as la_year,grade as grade, kp7_active as at_kp7,type_graduate as g_type,major_level,major_label
  FROM graduate  WHERE id = '$id'
UNION
SELECT
 runid as run_id,if(runno >0,runno,99) as run_no,place as place,startdate as s_year,enddate as e_year,'',subject as grade ,kp7_active as at_kp7,'','',''
  FROM seminar WHERE id = '$id' order by run_no ASC";
  $result_show = mysql_db_query($dbname,$sql_show);
 // echo "$dbname: $sql_show <hr>";
  $i=0;
  while($rs_s = mysql_fetch_assoc($result_show)){
	  ...
	  $key_major[$i] = $rs_s[major_level];
	  $key_major_label[$i] = $rs_s[major_label];
	  $i++;
...
<tr bgcolor="#A3B2CC" align="center" style=" font-weight:bold;"> 
	<td width="4%">เรียงลำดับ</td>
	<td width="23%">สถานศึกษา</td>
	<td width="12%">ตั้งแต่ - ถึง (เดือน ปี)</td>
	<td width="25%">วุฒิที่ได้รับ</td>
	<td width="17%">สาขาวิชาเอก</td>
	<td width="7%">หมวด<br>
	  รายการ</td>
	<td width="6%"><input type="button" name="add" value=" เพิ่มข้อมูล " onClick="location.href='graduate.php?id=<?=$id?>&action=add'"></td>
    <td width="6%">สถานะการ<br>
      แสดงผลใน ก.พ.7</td>
</tr>
<?
//$i			= 0;
...
foreach($key_runid as $k => $v){
	...
	if($key_gtype[$k] != ""){
		...
		$xtype_data = "<font color=\"#009900\">การศึกษา</font>";
	}else{
...
?>	
	...
	?>
	<INPUT TYPE="hidden" NAME="runid[]" VALUE="<?=$key_runid[$k]?>">
    <input type="hidden" name="sent_type[]" value="<?=$sent_type?>">
    	</td>
	...
	?>	</td>
	<td><?
    	if($key_major_label[$k] != ""){
			$major_xlabel = " $key_major_label[$k]";
			
	}else{
		
			$sql_major = "SELECT major  FROM $dbnamemaster.hr_addmajor WHERE major_id LIKE '".$key_major[$k]."' ";	
			$result_major = mysql_query($sql_major);
			$rsmj = mysql_fetch_assoc($result_major);
			if($rsmj[major] != ""){
				$major_xlabel = "$rsmj[major]";
			}else{
				$major_xlabel = "";	
			}// end if($rsmj[major] != ""){
	}
	...
	?></td>
	<td align="center"><?=$xtype_data?></td>
	<td align="center">
	<a href="<?=$url_file?>?id=<?=$id?>&runid=<?=$key_runid[$k]?>&action=edit2&type_graduate=<?=$key_gtype[$k]?>&sent_type=<?=$sent_type?>&graduate_edit=1"><img src="bimg/b_edit.png" width="16" height="16" border="0" alt="Edit"></a>&nbsp; 
	<a href="#" onClick="if (confirm('คุณจะทำการลบข้อมูลในแถวนี้ใช่หรือไม่!!')) location.href='?action=delete&id=<?=$id?>&runid=<?=$key_runid[$k]?>&sent_type=<?=$sent_type?>';" >	<img src="bimg/b_drop.png" width="16" height="16" border="0" alt="Delete"></a>	</td>
    <td align="center"><? if($key_kp7[$k]=="1"){ echo "<img src=\"../../../images_sys/approve20.png\" alt='แสดงข้อมูลใน กพ7'>";}else{ echo "<img src=\"../../../images_sys/unapprove.png\" alt='ไม่แสดงข้อมูลใน กพ7'>";}?></td>
</tr>
<?
	}//end foreach(){
...
<table width="450" border="0" align="center" cellpadding="0" cellspacing="0">
  <tr>
    <td bgcolor="#000000"><table width="100%" border="0" cellspacing="1" cellpadding="3">
        <tr bgcolor="">
        <td colspan="2" align="center" bgcolor="#A3B2CC"><strong>เลือกระดับการศึกษาก่อนทำการบันทึกข้อมูล</strong></td>
        </tr>
	...
	?>
      ...
      <tr>
        <td colspan="2" align="center" bgcolor="#dddddd"><input type="button" name="Button" value="ย้อนกลับ" onClick="location.href='?'"></td>
        </tr>
    </table></td>
  </tr>
</table>
		...
		<tr>
          ...
          <td align="left">
            <select name="university">
			  <option value="">ไม่ระบุ</option>
              <? 
		$sql4 = " SELECT  *  FROM  hr_adduniversity   ORDER BY  u_name  ASC ";
	...
	<span class="style11">ส่วนแสดงผลใน ก.พ.7 </span>
		  ...
		  <td height="25" colspan="2" align="right"><table width="98%" border="0" align="center" cellpadding="1" cellspacing="2">
            <tr>
              <td colspan="2" bgcolor="#666666"><table width="100%" border="0" cellpadding="0" cellspacing="0" bgcolor="#A5B2CE">
                  <tr>
                    <td height="25" colspan="2">วุฒิที่ได้รับ ระบุสาขาวิชาเอก (ถ้ามี)</td>
                  </tr>
		<tr>
        ...
        <tr>
		  ...
		  <option value="">ไม่ระบุ</option>
              <? 
		$sql3 = " SELECT  *  FROM hr_addmajor  WHERE  major_id  LIKE '%0000%00'    ORDER BY  major  ASC ";
...
<!--	<div id="divA" style="display: none;">--><br>
	...
	?>
                    ...
                    <tr>
                      <td align="right" height="25"></td>
                      <td align="left"><input type="text" name="year_label" size="50" value="<?=htmlspecialchars($rs[year_label])?>">
                        <span class="style11"><br>
                        หากท่านบันทึกในช่องแสดงผล กพ.7 ระบบจะนำข้อมูลนี้แสดงผลใน กพ.7 แทน</span></td>
                    </tr>
                    <tr>
	  ...
	  ?>
       	...
       	<tr> 
	<td width="26%" height="25" align="right">สถานศึกษา&nbsp;</td>
	<td width="74%" align="left"><input type="text" name="place" size="50" value="<?=htmlspecialchars($rs[place])?>">
	  <span class="style11"><br>
	  หากท่านบันทึกในช่องแสดงผล กพ.7 ระบบจะนำข้อมูลนี้แสดงผลใน กพ.7 แทน</span></td>
</tr>
                  <tr>
		...
		<tr>
		  ...
		  <option value="">ไม่ระบุ</option>
			<?
	$xsql1 = "select * from hr_adddegree where degree_id like '".$rs[graduate_level]."%' ORDER BY degree_fullname  ASC";
	...
	echo "<option value='$xrs1[degree_id]' SELECTED>$xrs1[degree_fullname]</option>";
      ...
      <tr>
          ...
          <td align="left"><select name="major" id="major" onChange="refreshproductList1();">
		 		<option value="">ไม่ระบุ</option>
              <? 
		if(strlen($rs[major_level])<=8)
		{
		$tem_major=substr($rs[major_level],0,1);
		}
		else
		{
		$tem_major=substr($rs[major_level],0,2);
		}
		$sql3 = " SELECT  *  FROM hr_addmajor  WHERE  major_id  LIKE '%0000%00' ORDER BY major  ASC ";
		$result3 	= mysql_db_query($dbnamemaster,$sql3);	
		while($rs3 = mysql_fetch_assoc($result3)){
			if(strlen($rs3[major_id])<=8)
			{
			$str_id=substr($rs3[major_id],0,1);
			}
			else
			{
			$str_id=substr($rs3[major_id],0,2);
			}
			if($tem_major==$str_id){
	  ...
	  ?>
		...
		{
			$xtem_major=substr($rs[major_level],0,2);
		}
		$str_sql3 = " SELECT  *  FROM hr_addmajor  WHERE  major_id  LIKE '$xtem_major%' ORDER BY major  ASC ";
		...
		?>
		 ...
		 <tr>
		  ...
		  <option value="">ไม่ระบุ</option>
		  <?
		  if(strlen($rs[minor_level])<=8)
		{
		 $tem_major=substr($rs[minor_level],0,1);
		}
		else
		{
		$tem_major=substr($rs[minor_level],0,2);
		}
		$sql3 = " SELECT  *  FROM hr_addmajor  WHERE  major_id  LIKE '%0000%00' ORDER BY  major  ASC ";
		$result3 	= mysql_db_query($dbnamemaster,$sql3);	
		while($rs3 = mysql_fetch_assoc($result3)){
		...
		{
			$t_major=substr($rs[minor_level],0,2);
		}
		$xsql3 = " SELECT  *  FROM hr_addmajor  WHERE  major_id  LIKE '$t_major%' ORDER BY  major  ASC ";
        ...
        <tr>
		  ...
		  <td align="left">&nbsp;</td>
			    ...
			    <td width="41%" bgcolor="#666666"><table width="100%" height="38%" border="0" cellpadding="1" cellspacing="0" bgcolor="#A5B2CE">
                  ...
                  <tr>
                    <td><label>
                      <input type="text" name="grade" size="40" value="<?=htmlspecialchars($rs[grade])?>">
                    </label></td>
                  </tr>
                  <tr>
                    <td>&nbsp;</td>
...
<table width="100%" border="0" align="center" cellpadding="0" cellspacing="2">
          ...
          <tr>
            <td width="26%" align="right">วุฒิการศึกษา</td>
            <td width="74%"><input name="level1" type="text" size="60" value="<?=htmlspecialchars($rs[grade])?>"></td>
          </tr>
          <tr>
            <td align="right">สาขาวิชาเอก</td>
            <td><input name="major1" type="text" size="60" value="<?=htmlspecialchars($rs[major_label])?>"></td>
          </tr>
          <tr>
            <td align="right">สาขาวิชาโท</td>
            <td><input name="minor_1" type="text" size="60" value="<?=htmlspecialchars($rs[minor_label])?>"></td>
          </tr>
          <tr>
            <td align="right">สถาบันการศึกษา</td>
            <td>
			
              <input name="university1" type="text" size="60" value="<?=htmlspecialchars($rs[place])?>">           </td>
          </tr>
          <tr>
            <td align="right">&nbsp;</td>
...
<tr> 
	<td width="26%" height="25" align="right">สถานศึกษา&nbsp;</td>
	<td width="74%" align="left"><input type="text" name="place" size="60" value="<?=htmlspecialchars($rs[place])?>">
...
<tr>
  <td align="right" height="25">&nbsp;</td>
  <td align="left"><label>
    <input type="text" name="year_label" value="<?=htmlspecialchars($rs[year_label])?>">
    <span class="style11"><br>
    หากท่านบันทึกในช่องแสดงผล กพ.7 ระบบจะนำข้อมูลนี้แสดงผลใน กพ.7 แทน</span></label></td>
</tr>
<tr> 
	<td align="right" height="25">วุฒิที่ได้รับ ระบุสาขาวิชาเอก (ถ้ามี)&nbsp;</td>
	<td align="left"><input type="text" name="grade" size="60" value="<?=htmlspecialchars($rs[grade])?>"></td>
</tr>
<tr>
  <td align="right" height="25">&nbsp;</td>
