...
?>
 ...
 ?>
  <tr>
    ...
    <thead>  
      <tr>
        <td colspan="12" align="left" bgcolor="#CCCCCC"><a href="?action=">กลับหน้าหลัก</a> || รายงานรายชื่อพนักงานคีย์ข้อมูลที่บันทึกข้อมูลผิดปกติ ตั้งแต่วันที่ 2 กุมภาพันธ์ 2555 ถึง วันที่ 4 กุมภาพันธ์ 2555</td>
        </tr>
      <tr>
...
t2.card_id
FROM
log_case_per_day AS t1
Inner Join keystaff AS t2 ON t1.staffid = t2.staffid
WHERE t2.sapphireoffice <>  '1' and t1.num_pointsalary > 0  $constatus
GROUP BY t2.staffid
...
if($hd_put) {
	...
	#print "Cannot Open $url ";
        ...
        <td align="center"><? if($rs[nump] > 0){ echo "<a href='index_report_detail_perday.php?staffid=$rs[staffid]&id_code=$rs[id_code]&fullname=$fullname' target=\"_blank\">".number_format($rs[nump])."</a>";}else{ echo "0";}?></td>
