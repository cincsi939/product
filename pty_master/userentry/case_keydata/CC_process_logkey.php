...
order by ordersite asc $xlimit ";
		...
		while($rs = mysql_fetch_assoc($reuslt)){
			...
			mysql_db_query($dbnameuse,"DELETE FROM log_case WHERE siteid='$rs[siteid]' and datekey='$before_date'");
...
t1.subject =  'ข้อมูลเงินเดือน' and date(t1.updatetime)='$before_date' $xlimit1
GROUP BY
t1.username,
t1.staff_login,
date(t1.updatetime)
...
num_delete >  $num_del";
			...
			if($numdiff > 0 and $num_pointall > 0){
				...
				$sql_insert = "INSERT INTO log_case SET staffid='$rs1[staffid]',idcard='$rs1[idcard]',siteid='$rs[siteid]',num_delete='$rs1[num_delete]',key_start='$rs1[key_start]',key_end='$rs1[key_end]',num_point='$num_pointall',num_pointsalary='$num_pointsalary',timeupdate=NOW(),datekey='$before_date'";
				#echo $sql_insert."<hr>";
				mysql_db_query($dbnameuse,$sql_insert) or die(mysql_error()."<hr>$sql_insert</hr>".__LINE__);
				
				#echo $sql_insert3."<hr>";
				}//end  if($numdiff > 0 and $num_pointall > 0){
