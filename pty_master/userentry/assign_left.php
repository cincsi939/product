...
<html>
<head>
<title>บริหารจัดการผู้ใช้</title>
...
<!--
	d = new dTree('d');
	d.add(0,-1,'<B style="color: black;"><? echo "$rs[prename]$rs[staffname]  $rs[staffsurname]";?></B>','assign_list.php?action=view_main&staffid=<?=$staffid?>&parent=0&profile_id=<?=$profile_id?>','assign person','mainFrame');
...
if($profile_id == ""){
	...
	while ($xrs2=mysql_fetch_array($xresult2,MYSQL_ASSOC)){	
		...
		$temp_date = thai_date($xrs2[assign_date]);
			echo "d.add($menuitem,$parent,'วันที่ $temp_date ($xrs2[ticketid])','assign_list.php?action=assign_detail&staffid=$xrs2[staffid]&TicketId=$xrs2[ticketid]&profile_id=$profile_id','$temp_date','mainFrame');\n";
			$xparent = $menuitem;
