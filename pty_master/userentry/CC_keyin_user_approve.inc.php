...
while($rss  = mysql_fetch_assoc($results)){
	...
	### ทำการบันทึกข้อมูลในระบบ
			...
			if($rs_check[idcard] != ""){
				$sql_key_person = "UPDATE stat_user_keyperson SET numpoint='".CutNumberPoint($TPOINT_PERSON[$key_person][$k],2)."', numkeyin='$v',status_approve='$status_approve' WHERE datekeyin='$datereq1' AND staffid='$key_person' AND idcard='$k'";
			}else{
			$sql_key_person = "INSERT INTO stat_user_keyperson(datekeyin,staffid,idcard,numkeyin,numpoint,status_approve)VALUES('$datereq1','$key_person','$k','$v','".CutNumberPoint($TPOINT_PERSON[$key_person][$k],2)."','$status_approve')";	
			}
