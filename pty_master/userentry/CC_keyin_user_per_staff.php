...
//die;
//$constaff = " AND keystaff.staffid='10493' ";
$dayrecord = array("2010-07-14");
//echo "<pre>"; print_r($dayrecord);echo"</pre>";
//die;
...
WHERE   ( keystaff.sapphireoffice = 0 )  AND (keystaff.status_extra = 'NOR' ) AND timestamp_key LIKE  '$val%' $constaff  GROUP BY idcard   ORDER BY keystaff.staffid ASC    " ;
...
foreach($sumkeyuser AS $key2=>$val2){
	...
	$sql_ins = " REPLACE INTO  stat_user_keyin(datekeyin,staffid,val_avg,val_max,val_min,numkeyin,kval_avg,kval_max,kval_min,numkpoint,numperson)VALUES('$val','$key2','$avg','$val2','$minc','".$sumkeyuser[$key2]."','','".number_format($TPOINT[$key2],2)."','".number_format($TPOINT[$key2],2)."','".number_format($TPOINT[$key2],2)."','".$xnumrows[$key2]."')";
	echo "$sql_ins <hr>";
//	echo "$numkey[$key2] <hr>";
	mysql_db_query($dbnameuse,$sql_ins);
	
	$sql_ins_hour = " INSERT INTO  stat_user_keyin_hour(datekeyin,staffid,val_avg,val_max,val_min,numkeyin,kval_avg,kval_max,kval_min,numkpoint,numperson)VALUES('$val','$key2','$avg','$val2','$minc','".$sumkeyuser[$key2]."','','".number_format($TPOINT[$key2],2)."','".number_format($TPOINT[$key2],2)."','".number_format($TPOINT[$key2],2)."','".$xnumrows[$key2]."')";
	echo $sql_ins_hour."<br>";
...
/*	$sql_key_person = "REPLACE INTO stat_user_keyperson(datekeyin,staffid,idcard,numkeyin,numpoint)VALUES('$val','$key_person','$k','$v','".$TPOINT_PERSON[$key_person][$k]."')";	
	...
	mysql_db_query($dbnameuse,$sql_key_person);*/
			...
			if($rs_check[idcard] != ""){
				$sql_key_person = "UPDATE stat_user_keyperson SET numpoint='".number_format($TPOINT_PERSON[$key_person][$k],2)."', numkeyin='$v', status_approve='$approve_id' WHERE datekeyin='$datereq1' AND staffid='$key_person' AND idcard='$k'";
			}else{
			$sql_key_person = "INSERT INTO stat_user_keyperson(datekeyin,staffid,idcard,numkeyin,numpoint,status_approve)VALUES('$datereq1','$key_person','$k','$v','".number_format($TPOINT_PERSON[$key_person][$k],2)."','$approve_id')";	
			}
