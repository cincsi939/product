...
class PostGraph
{
    ...
    */
    function PostGraph($width = 400, $height = 300)
    {
         ...
         $this->colorInsideBarArray = array(255, 255, 255);
		  $this->colorCentralPointArray=array(250,231,18);
    }
    ...
    */
    function initColors()
    {
		...
		$this->colorCentralPoint = ImageColorAllocate($this->img, $this->colorCentralPointArray[0], $this->colorCentralPointArray[1], $this->colorCentralPointArray[2]);
    }
    ...
    */
    ...
    {
        // draw lines
        $top = $this->posYStart;//((($this->posYEnd-$this->posYStart)*$this->centralPoint)/$this->maxData)
		...
		for($i=0;$i<=$this->graphHeight;$i++){
					...
					$barHeight = (($this->posYEnd-$this->posYStart)*$i)/$this->maxData;
					$startY = $this->posYEnd - $barHeight;
					if($i==$this->centralPoint){
					ImageFilledRectangle($this->img, $this->posXStart, $startY, $this->posXEnd , $this->posYEnd, $this->colorCentralPoint);//Area Central Point
					ImageRectangle($this->img, $this->posXStart, $this->posYStart, $this->posXEnd , $this->posYEnd, $this->colorLines);//Area Central Point
					//ImageString($this->img, 1, $this->posXStart, $barHeight, "------------------------------", $this->colorAboveBar);
					}
		}
        ...
        for($i=0; $i<=$this->yTicks; $i++)
        {
			...
			ImageLine($this->img, $this->posXStart-5, $top, $this->posXStart+$this->graphWidth, $top, $this->colorLines);
        ...
        foreach($this->data as $key => $value){
			if(is_array($value)){
					...
					foreach($value as $ky=>$val){
						$int_ky++;
						$barWidth = (0.75*($this->posXEnd-$this->posXStart)/$this->countData)/2;
						$barHeight = (($this->posYEnd-$this->posYStart)*$val)/$this->maxData;
						...
						if(($barHeight>13 && $this->yValueMode == 1) || $this->yValueMode == 2){
							$startY = $this->posYEnd - $barHeight;
							ImageString($this->img, 1, $startX, $startY, $val, $this->colorInsideBar);
						}else{
							$startY = $this->posYEnd - $barHeight - 13;
							if($val!=100){
								ImageString($this->img, 1, $startX+7, $startY+7, $key." (".$val."-".($val+10).")", $this->colorAboveBar);
							}else{
								ImageString($this->img, 1, $startX+7, $startY+7, $key." (0-".($val).")", $this->colorAboveBar);
							}
							Imagefilledellipse($this->img,  $startX, $startY+10, 7, 7, $this->colorBars);//XY
						}
						if($int_ky==1){
				...
				if($int_data==1){
					$areaHeight = (($this->posYEnd-$this->posYStart)*$this->centralPoint)/$this->maxData;
					ImageFilledRectangle($this->img, $this->posXStart, $areaHeight, $this->posXEnd , $this->posYEnd, $this->colorCentralPoint);//Area Central Point
					ImageRectangle($this->img, $this->posXStart, $this->posYStart, $this->posXEnd , $this->posYEnd, $this->colorLines);//Area Central Point
				}				
				$startX = $startPos + $step - (strlen($value)*3);
				if(($barHeight>13 && $this->yValueMode == 1) || $this->yValueMode == 2){
					...
					$startY = $this->posYEnd - $barHeight - 13;
					ImageString($this->img, 1, $startX+7, $startY+7, $key, $this->colorAboveBar);
					Imagefilledellipse($this->img,  $startX, $startY+10, 7, 7, $this->colorBars);//XY
				}
				$startPos = round((($this->posXEnd-$this->posXStart)/$this->countData),2) + $startPos;    
			}      
    ...
    */
    function drawTitles()
    {
		...
		//ImageTTFText ($this->img, 5, 0,5,  $this->graphWidth/2-strlen($this->graphTitle)*4, $this->colorText, $this->FontName, $this->graphTitle);
    ...
    */
	...
	function  setCentralPointColor($userRGB){
		 if($userRGB != "" && count($userRGB) == 3)
        $this->colorCentralPointArray = array(0 => $userRGB[0], 1 => $userRGB[1], 2 => $userRGB[2]);
	}
    //------------------------------------------------------------------------
