...
if($_SERVER['REQUEST_METHOD'] == "POST"){
		if($Aaction == "SAVE"){
				if(count($status_file) > 0){
					foreach($status_file as $k => $v){
						$sql_replace = "replace into log_pdf_check(idcard,date_c,secid,status_file,comment_file,check_file,page_pdf)values('$k','".date("Y-m-d")."','$xsiteid[$k]','$v','$comment_file[$k]','1','$page_pdf[$k]')";
...
//	 WHERE $db_pobec.IDCODE <> ''  $conx1  $conx   order by postcode.POST_CODE  ASC ";
	 ...
	 $sql = "SELECT  $db_pobec.IDCODE AS idcard, $db_pobec.NAME1 AS name_th, $db_pobec.NAME2 AS surname_th, postcode.POST_NAME AS position_now, $db_pobec.I_CODE AS schoolid FROM $db_pobec Inner Join postcode ON $db_pobec.POST_CODE = postcode.POST_CODE 
  ...
  <tr>
    <td align="center">
      <table width="100%" border="0" cellspacing="1" cellpadding="2">
        <tr>
          <td colspan="7" align="center" bgcolor="#A3B2CC"><strong>รายการที่จะทำการบันทึก</strong></td>
        </tr>
        <tr>
          <td width="11%" align="center" bgcolor="#A3B2CC"><strong>บัตรประชาชน</strong></td>
          <td width="15%" align="center" bgcolor="#A3B2CC"><strong>ชื่อ - นามสกุล </strong></td>
          <td width="27%" align="center" bgcolor="#A3B2CC"><strong>หน่วยงาน</strong></td>
          <td width="7%" align="center" bgcolor="#A3B2CC"><strong>PFD</strong></td>
          <td width="9%" align="center" bgcolor="#A3B2CC"><strong>จำนวนหน้า</strong></td>
          <td width="10%" align="center" bgcolor="#A3B2CC"><strong>ความสมบูรณ์</strong></td>
          <td width="21%" align="center" bgcolor="#A3B2CC"><strong>หมายเหตุ</strong></td>
        </tr>
		 <?
		 if($ch_num < 1){
        ...
        <tr bgcolor="<?=$bg?>" <?=$title_a?>>
          ...
          <td align="center" valign="top"><? if(check_kp7file($rs[idcard],$sent_siteid) == 1){?><a href="../../../edubkk_kp7file/<? echo "$rs[siteid]/$rs[idcard]".".pdf";?>" target="_blank"><img src="../../images_sys/gnome-mime-application-pdf.png" alt="แสดงเอกสาร ก.พ.7 ต้นฉบับ" width="20" height="20" border="0"></a><? } ?></td>
          <td align="center" valign="top"><label>
		  <? 
		  $page_result = count_page($rs[idcard],$rs[siteid]); 
          ...
          <td align="left" valign="middle"><label>
            <textarea name="comment_file[<?=$rs[idcard]?>]" cols="30" rows="3"><?=$rs_chf[comment_file]?></textarea>
		...
		?>
        <tr>
          <td colspan="7" align="center" bgcolor="#CCCCCC"><label>
