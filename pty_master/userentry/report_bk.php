...
global $dbnamemaster;
	$sql = "SELECT COUNT(*) AS num FROM log_pdf_check WHERE secid='$secid'";
...
function count_file_pdf($secid){ // นับจำนวนไฟล์ pdf ที่ up ขึ้นในระบบ
	global $dbnamemaster;
	$sql1 = "SELECT COUNT(*) AS num1 FROM  log_pdf  WHERE  secid='$secid'";
...
global $db_name;
$sql = "SELECT COUNT(*) AS num1 FROM tbl_assign_sub Inner Join keystaff ON tbl_assign_sub.staffid = keystaff.staffid Inner Join tbl_assign_key ON tbl_assign_sub.ticketid = tbl_assign_key.ticketid WHERE keystaff.sapphireoffice =  '$xtype'  AND tbl_assign_key.approve =  '2' AND tbl_assign_key.siteid =  '$siteid'";
//echo $sql;
$result = mysql_db_query($db_name,$sql);
$rs = mysql_fetch_assoc($result);
return $rs[num1];
...
global $db_name;
$sql = "SELECT COUNT(*) AS num1 FROM tbl_assign_sub Inner Join keystaff ON tbl_assign_sub.staffid = keystaff.staffid Inner Join tbl_assign_key ON tbl_assign_sub.ticketid = tbl_assign_key.ticketid WHERE keystaff.sapphireoffice =  '$xtype'  AND tbl_assign_key.approve =  '3' AND tbl_assign_key.siteid =  '$siteid'";
$result = mysql_db_query($db_name,$sql);
$rs = mysql_fetch_assoc($result);
return $rs[num1];
}// end function count_qc_pass1(){
...
global $db_name;
$sql = "SELECT COUNT(*) AS num1 FROM tbl_assign_sub Inner Join keystaff ON tbl_assign_sub.staffid = keystaff.staffid Inner Join tbl_assign_key ON tbl_assign_sub.ticketid = tbl_assign_key.ticketid WHERE keystaff.sapphireoffice =  '$xtype' AND (tbl_assign_key.approve =  '0' or tbl_assign_key.approve = '1') AND tbl_assign_key.siteid =  '$siteid'";
$result = mysql_db_query($db_name,$sql);
$rs = mysql_fetch_assoc($result);
return $rs[num1];
}// end function count_qc_pass1(){
...
global $db_name;
		...
		$rs1 = mysql_fetch_assoc($result1);
		//echo "$rs1[est_num] ===  $rs[num1]";
		if($rs1[est_num] != $rs[num1]){
			$temp_n = "<font color='red'>$rs[num1]</font>";
		}else{
			$temp_n = $rs[num1];
		}
...
function count_key_pass($secid){
	global $db_name;
	$sql ="SELECT count(
 ".DB_CHECKLIST.".temp_check_pdf.idcard) as num1
FROM
 ".DB_CHECKLIST.".temp_check_pdf
Inner Join ".DB_USERENTRY.".tbl_assign_key ON  ".DB_CHECKLIST.".temp_check_pdf.idcard = ".DB_USERENTRY.".tbl_assign_key.idcard
WHERE ".DB_USERENTRY.".tbl_assign_key.approve =  '2' AND
 ".DB_CHECKLIST.".temp_check_pdf.secid =  '$secid' group by  ".DB_CHECKLIST.".temp_check_pdf.idcard";
	$result  = mysql_db_query($db_name,$sql);
...
function count_pdf_fail($secid){// นับจำนวนไฟล์เอกสารที่ไม่สมบูรณ์
	global $dbnamemaster;
	$sql = "SELECT COUNT(*) num1 FROM log_pdf  WHERE  secid='$secid' and status_file ='0'";
	$result = mysql_db_query($dbnamemaster,$sql);
	$rs = mysql_fetch_assoc($result);
	return $rs[num1];
...
global $dbnamemaster;
	$sql_check_file = "SELECT COUNT(*) AS num1 FROM log_pdf   WHERE idcard='$idcard' AND status_file='0' AND secid='$secid'";
        ...
        <td width="70%" valign="top"><table width="100%" border="0" cellspacing="0" cellpadding="0">
          <tr>
            <td bgcolor="#000000"><table width="100%" border="0" cellspacing="1" cellpadding="1">
              <tr>
                <td colspan="3" bgcolor="#A3B2CC"><span class="style1">รายงานสรุปการบันทึกข้อมูล </span></td>
              </tr>
              <tr>
                <td width="32%" align="center" bgcolor="#A3B2CC">วันเริ่มดำเนินงาน</td>
                <td width="31%" align="center" bgcolor="#A3B2CC">วันสิ้นสุดดำเนินงาน</td>
                <td width="37%" align="center" bgcolor="#A3B2CC">จำนวนวันดำเนินการ(วัน)</td>
              </tr>
              <tr>
      ...
      <tr>
        <td bgcolor="#000000"><table width="100%" border="0" cellspacing="1" cellpadding="3">
          <tr>
            ...
            <td colspan="3" rowspan="2" align="center" bgcolor="#A3B2CC"><strong>PDF</strong></td>
  ...
  <table width="100%" border="0" cellspacing="0" cellpadding="0">
    <tr>
      <td><table width="100%" border="0" cellspacing="1" cellpadding="3">
		...
		if($type_view == "all"){
$sql = "SELECT
 ".DB_CHECKLIST.".temp_check_pdf.idcard,
 ".DB_CHECKLIST.".temp_check_pdf.prename_th,
 ".DB_CHECKLIST.".temp_check_pdf.name_th,
 ".DB_CHECKLIST.".temp_check_pdf.surname_th,
 ".DB_CHECKLIST.".temp_check_pdf.schoolname,
$db_site.general.position_now,
 ".DB_CHECKLIST.".temp_check_pdf.secid
FROM
 ".DB_CHECKLIST.".temp_check_pdf
Inner Join $db_site.general ON  ".DB_CHECKLIST.".temp_check_pdf.idcard = $db_site.general.idcard
WHERE
 ".DB_CHECKLIST.".temp_check_pdf.secid =  '$secid'
";
...
WHERE
 ".DB_CHECKLIST.".temp_check_pdf.secid =  '$secid' AND ".DB_USERENTRY.".tbl_assign_key.approve =  '2'";
...
if($xidcard != ""){ $xidcard = $xidcard;}else{ $xidcard = "''";}
	...
	Inner Join $db_site.general ON  ".DB_CHECKLIST.".temp_check_pdf.idcard = $db_site.general.idcard
	WHERE
	 ".DB_CHECKLIST.".temp_check_pdf.secid =  '$secid' AND  ".DB_CHECKLIST.".temp_check_pdf.idcard NOT IN($xidcard)";
...
//echo "$sql<br>";
		...
		while($rs = mysql_fetch_assoc($result)){
			...
			$path_img = "../../../edubkk_kp7file/$rs[secid]/$rs[idcard]".".pdf";
				if(file_exists($path_img)){
					...
					$link_del = "  <img src=\"images/Cancel-2-32x32.png\" alt=\"กำหนดสถานะเอกสารไม่สมบูรณ์\" width=\"18\" height=\"18\" border=\"0\" onClick=\"return confirmDelete('pupup_kp7_fail.php?idcard=$rs[idcard]')\" style=\"cursor:hand\">";
				}else{
					$link_del = "";
		...
		if($type_view == "discount"){		
			if(check_file_fail($rs[idcard],$secid) == "0"){
		?>

                <tr bgcolor="<?=$bg?>">
                  <td align="center"><?=$m?></td>
                  <td align="left"><?=$rs[idcard]?>&nbsp;&nbsp;<?=$link_img?>&nbsp;<?=$link_del?> </td>
                  <td align="left"><? echo "$rs[prename_th]$rs[name_th]  $rs[surname_th]";?></td>
                  <td align="left"><?=$rs[position_now]?></td>
                  <td align="left"><?=$rs[schoolname]?></td>
                  <td align="center">
				  <?
				  	$sql_c= "SELECT COUNT(*) AS num FROM tbl_assign_key WHERE  idcard='$rs[idcard]' and  approve = '2'";
				  ...
				  ?>				  </td>
                  <td align="center">				  <?
				  	$sql_c1= "SELECT COUNT(*) AS num FROM tbl_assign_key WHERE   idcard='$rs[idcard]' and approve <> '2'";
				...
				<?
							} //end 	if(check_file_fail($rs[idcard],$secid) == "0"){
						}else{//end if($type_view == "discount"){		
				?>
				                <tr bgcolor="<?=$bg?>">
                  <td align="center"><?=$m?></td>
                  <td align="left"><?=$rs[idcard]?>&nbsp;&nbsp;<?=$link_img?>&nbsp;<?=$link_del?> </td>
