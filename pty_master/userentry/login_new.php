...
if($_SERVER['SERVER_NAME'] == APPHOST_TEST){
	$url = APPURL."/edubkk_master/application/userentry/login_new.php";
	header("Location: $url");
	 ...
	 $str_INTRA=$arr_INTRA[0].".".$arr_INTRA[1];
		if($str_userip==$str_APPHOST || $str_userip==$str_INTRA){
		     //   header("Location: login_new.php");
		        ...
		        header("Location: ".APPURL."/edubkk_master/application/userentry/login_new.php");
				// die();
		}
	}
...
if(substr($ipn,0,8) == "192.168."){
		header("Location: http://".APPHOST."/edubkk_master/application/userentry/login_new.php");
}
*/
...
if($_POST["submit"]){
	...
	#echo   "$sql_day1<br>$rsdate[day1] =>   $rs_staff[numstaff] => $h1";
	  if($rsdate[day1] >1 and $rsdate[day1]  < 7 and $rs_staff[numstaff] > 0 ){
			#if($h1 >=8 and $h1 <= 17){
		  	echo "<script>alert('การบันทึกข้อมูล update สำหรับรหัสการใช้งานของท่านสามารถทำงานได้เฉพาะวันเสาร์เท่านั้น หากมีข้อสงสัยกรุณาติดต่อทีมศูนย์ภาค'); location.href='login_new.php';</script>";
			exit;
		  #}
		 # echo "<hr> YES";
		}
	 ### end block การคีย์
		...
		echo "<script>alert('ไม่สามารถเข้าสู่ระบบได้ รหัสผู้ใช้นี้ได้ถูกระงับสิทธิการใช้โปรแกรมเนื่องจากสิ้นสุดการจ้าง'); location.href='login_new.php';</script>";
		exit;
	}
	...
	/*if($rs[period_time] == "am" and  $rs[sapphireoffice] == "0" and CheckLogin($rs[staffid]) > 0){
			...
			echo "<script>alert('ไม่สามารถเข้าสู่ระบบได้ เนื่องจากเวลาในการเข้าระบบของท่านคือ 09.00 น. - 17.30 น.'); location.href='login_new.php';</script>";
			exit;
			...
			echo "<script>alert('ไม่สามารถเข้าสู่ระบบได้ เนื่องจากเวลาในการเข้าระบบของท่านคือ 18.00 น. - 22.00 น.'); location.href='login_new.php';</script>";
			exit;	
			}
	}*/
        ...
        <td style="border:#818181 solid 1px; "><table width="100%" border="0" cellspacing="0" cellpadding="0">
          ...
          <tr>
            <td align="center" style=" padding:10px;"><table width="100%" border="0" cellpadding="0" cellspacing="0" bgcolor="#ECECEC">
              <tr>
                <td width="463" background="images/emp_main_new_20.gif" style="background-repeat:repeat-x; background-position:top"><form name="form1" method="post" action="login_new.php"> 
                  <br>
