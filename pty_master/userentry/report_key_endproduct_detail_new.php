...
<?		
	...
	if(count($arrc) > 1){
			...
			if($xtype == "1"){
			$conid_staff = " AND ((t1.start_date <= '".$arrc[1]."' AND t1.status_permit='YES' and t1.start_date IS NOT NULL ) or ( t1.status_permit='NO' AND t1.retire_date > '".$arrc[0]."' and  t1.start_date between '".$arrc[0]."' and '".$arrc[1]."'))";
			}else{
				$conid_staff = " AND ((t1.start_date <= '".$arrc[1]."' AND t1.status_permit='YES' and t1.start_date IS NOT NULL ))";	
			}
			...
			if($xtype == "1"){
				$conid_staff = " AND ((t1.start_date <= '".$arrc[0]."-31' AND t1.status_permit='YES' and t1.start_date IS NOT NULL ) or ( t1.status_permit='NO' AND t1.retire_date > '".$arrc[0]."-01' and  t1.start_date between '".$arrc[0]."-01' and '".$end_date."'))";
			}else{
				$conid_staff = " AND ((t1.start_date <= '".$arrc[0]."-31' AND t1.status_permit='YES' and t1.start_date IS NOT NULL ))";	
			}//end 	if($xtype == "1"){
	...
	//echo $sql_staff; WHERE   t1.keyin_group='2' and t1.period_time='am' and t1.status_extra='NOR'   $conid_staff $con1
