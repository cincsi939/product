								...
								foreach($result1_diff as $key2 => $val2){
									...
									if(($spos === false)){
											...
											$arrtbl[$tblname]['after'][] = $rs_af[$key2];			
											 SaveLogKey($log_id,"1",$rs_be[$key2],$rs_af[$key2],$timestart,$timeend);							
									}// end if(($spos === false)){
			...
			if($tblname == ""){
				...
				if($tblname == "general"){
					...
					if(count($Pkey) > 0){
						...
						$table_in_db = "Tables_in_".$dbsite;
						$sql_check_table = "show tables where $table_in_db='$table_after'";
						$result_tbl = mysql_db_query($dbsite,$sql_check_table);
						$rstbl = mysql_fetch_assoc($result_tbl);
						if($rstbl["$table_in_db"] != ""){
						...
						if(count($result1_diff) > 0){
								foreach($result1_diff as $key2 => $val2){
									...
									$spos = strpos($key2,"time");
									if(($spos === false)){
													 SaveLogKey($rs[log_id],$rsl[menu_id],$rs_be[$key2],$rs_af[$key2],$time_before,$rsl[updatetime]);
...
function SaveLogKey($log_id,$menu_id,$val_before,$val_after,$timestart,$timeend){
	global $dbnameuse;
	$sql = "INSERT INTO tbl_assign_edit_log_detail_key_temp SET log_detail_id='$log_id',menu_id='$menu_id',val_before='$val_before',val_after='$val_after',timestart='$timestart',timeend='$timeend' ";	
...
FROM ".DB_USERENTRY.".tbl_assign_edit_key as t1 Inner Join ".DB_USERENTRY.".tbl_assign_edit_log as t2 ON t1.idcard = t2.idcard AND t1.ticketid = t2.ticketid Inner Join ".DB_USERENTRY.".tbl_assign_edit_log_detail as t3 ON t2.runid = t3.log_id
where t1.userkey_wait_approve='1'
GROUP BY t1.idcard
";
