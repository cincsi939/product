...
if($xsearch == "search2"){
	...
	#if(in_area() != ""){ $in_site = in_area(); }else{ $in_site = "''";} // รหัสเขต 20 เขต
	$sql = "	SELECT t2.siteid,t1.CZ_ID as idcard,t1.prename_th,t1.name_th,t1.surname_th,t1.schoolid,t1.schoolname,t1.birthday,t1.begindate,t1.position_now, (TIMESTAMPDIFF(MONTH,t1.begindate,'$year1')/12) as age_gov,t1.siteid as site FROM $dbnamemaster.view_general as t1 Inner join $dbname_temp.tbl_checklist_kp7 as t2 On t1.CZ_ID=t2.idcard   WHERE t2.siteid  IN($in_site)  $conx ";
      ...
      <table width="100%" border="0" cellspacing="1" cellpadding="2">
        <tr>
          <td colspan="8" align="center" bgcolor="#A3B2CC">
		   <?php
		   #echo $sql;
		 #  echo "HOST : ".HOST ."user : ".USERNAME_HOST." pwd : ".PASSWORD_HOST."<hr>";
		   #ConHost(HOST,USERNAME_HOST,PASSWORD_HOST); 
		   if($show_by_school != ""){
		     $db_new_site = STR_PREFIX_DB.$_GET['new_site'];
		     $sql = "SELECT * , (TIMESTAMPDIFF(MONTH,begindate,'$year1')/12) as age_gov,siteid as site FROM general WHERE schoolid = '".$show_by_school."' ";
		   ...
		   if($show_by_person != ""){
			 ...
			 if(count($_POST['arr_idcard']) > 0){
		      ...
		      $sql = "SELECT * , (TIMESTAMPDIFF(MONTH,begindate,'$year1')/12) as age_gov,siteid as site FROM view_general WHERE CZ_ID IN($str_idcard) ";
			 }else{
			  $_POST['arr_idcard'][] = $idcard_show;
			  $sql = "SELECT * , (TIMESTAMPDIFF(MONTH,begindate,'$year1')/12) as age_gov,siteid as site FROM view_general WHERE CZ_ID = '$idcard_show' ";
			 } 
		...
		#echo $sql;
		 while($rs = mysql_fetch_assoc($result)){
			...
			#echo "site :".$site;
			 $arrx = CheckProtectionAssign($rs[$idcard_field],$site,$profile_id);
...
## หาข้อมูลรหัสเขต
$sql_site = "SELECT siteid FROM view_general WHERE  CZ_ID='$rs_d[idcard]'";
$result_site = mysql_db_query($dbnamemaster,$sql_site) or die(mysql_error()."".__LINE__);
$rssite = mysql_fetch_assoc($result_site);
$rs_d[siteid] = $rssite[siteid];
...
<?php
	#echo "$rows[CZ_ID] => $rows[siteid] => $profile_id<hr>";
	$sql_profile = "SELECT COUNT(idcard) as num FROM tbl_checklist_kp7 WHERE idcard='$rows[CZ_ID]' AND siteid='$rows[siteid]' AND profile_id='$profile_id'";
	...
	if($rspro[num] < 1){
					$dis = "disabled='disabled'";
					$msg = "<font color='red'> ไม่พบข้อมูลโปรไฟล์การทำงานใน checklist</font>";
					$file_status = 0;  // สถานะของการตรวจสอบเอกสาร
	}else{
