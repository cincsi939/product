...
//session_start();
			...
			$curent_date_project = $date_run; // วันที่ปัจจุบันของโครงการ
	...
	function GetKeyKp7GroupE($type=""){
		global $dbnameuse,$date_run;
			...
			$contime = " AND (date(t1.datetime_approve) LIKE '$yymm%' or date(t1.datetime_approve) LIKE '$lastmonth%') and  date(t1.datetime_approve) < '".$date_run."' $condate1 ";
...
/*		$arrkeynew = GetKeyKp7GroupN("NEW");
	...
	function GetKeykp7(){
		...
		if($insite != ""){
			...
			if($in_date != ""){
					$condate1 = " AND (date(t2.update_time) NOT IN($in_date))";
			}else{
					$condate1 = "";	
			}
			ConHost(HOST,USERNAME_HOST,PASSWORD_HOST); 
			...
			$yymm = date("Y-m");
			$lastmonth = LastMonth($yymm);			
			$conid = " AND date(t2.update_time) LIKE '$yymm%' and (t1.datekeyin LIKE '$yymm%' or t1.datekeyin LIKE '$lastmonth%' )and t1.datekeyin < '".$date_run."' $condate1";
...
limit 1";
   ...
   if($xFTime['wday'] != 0){
		...
		$sql_replace = "REPLACE INTO view_report_keykp7 SET date_report='".$date_run."',numdoc_new='$num_kp7all',numdoc_continue='$num_kp7all_old',keydoc_new='$num_approve',keydoc_continue='$num_approve_old',day_work='$numdayproject' ";
	...
	$sql_keydetail = "REPLACE INTO view_report_keykp7_detail SET date_report='".$date_run."',groupreport_id='$rs1[groupreport_id]',dockey_new='$dockey_new',dockey_continue='$dockey_continue',timeupdate=NOW()";
	...
	$sql_sum = "SELECT SUM(dockey_new) AS sumkey  FROM view_report_keykp7_detail WHERE date_report='".$date_run."' ";
	...
	$sql_update4 = "UPDATE view_report_keykp7 SET dockey_perday='$rss[sumkey]'  WHERE date_report='".$date_run."' ";
