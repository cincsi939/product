...
WHERE   ( keystaff.sapphireoffice = 0 )  AND (keystaff.status_extra = 'NOR' )   AND   keystaff.staffid =  '10378' AND monitor_keyin.timestamp_key LIKE  '$datereq1%'  GROUP BY idcard   ORDER BY keystaff.staffid ASC        " ;
$result1 = mysql_db_query($dbnameuse,$sql1);
//echo " $sql1 <br> $name : $rs1[idcard] <hr> "; 
$numrows = mysql_num_rows($result1);
while($rs1 = mysql_fetch_assoc($result1)){
	///echo "รหัสพนักงาน :: ".$rs1[staffid]."<br>";
	 ## debug จำนวนการคีย์ข้อมูลของ พนักงานที่มีรายการคีย์ผิดปกติ
...
$dbsite = STR_PREFIX_DB.$rs3[siteid] ;
	...
	if($list_field != ""){ 
  // echo "$c =>> id,runid";     
//$sql_c = " SELECT COUNT(*) AS num  FROM  $t[0]".$subfix." WHERE id = '$rs1[idcard]' AND $rs_ff[Field] LIKE  '$datereq1%'  GROUP BY  $c " ;
		$sql_c1 = "SELECT ".$list_field." ,min(auto_id) as auto_id, id,runid    FROM $t[0]".$subfix." WHERE id='$rs1[idcard]'  $contimestamp  GROUP BY $c";
	   $sql_c1_v2 = "SELECT ".$list_field." ,min(auto_id) as auto_id,$c  FROM $t[0]".$subfix." WHERE id='$rs1[idcard]'  $contimestamp  GROUP BY $c";
     $sql_c1 = $sql_c1_v2;
    }else{
		...
		if($result_c1){
			echo $sql_c1."<br>";	
           echo $sql_c1_v2."<br><hr>";     
		} else{
          // echo "ERROR  ". $sql_c1."<br>";
        }
	...
	//	echo $sql_c1."<br>";
				...
				$result_c2 = mysql_db_query($dbsite,$sql_c2) or die(mysql_error());
...
$resultx= mysql_db_query($dbnameuse," SELECT  *    FROM  keystaff  WHERE  staffid  = '10378' ");
