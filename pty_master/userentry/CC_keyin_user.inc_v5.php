	...
	while($rs_c1 = mysql_fetch_assoc($result_c1)){
				...
				if($rs_c2[id] > 0){
						...
						$calcuatepoint = true;
							...
							//echo "จำนวนรายการที่แก้ไข :: ".$numpoint."<br>";
								if($numpoint > 0){ // กรณีมีจุดการบันทึกที่ต่างกันให้คำณวณโดยใช้ค่า price_point ในการคูณ
									...
									$TPOINT[$rss[staffid]][$rss[idcard]] = $TPOINT[$rss[staffid]][$rss[idcard]] + ( $numpoint*$price_point[$tb1]) ;
					...
					//echo $sql_c1."<br>";
						$tb1 = $t[0]."$subfix";
						$TNUM += 1;
						$sumkeyuser[$rss[staffid]][$rss[idcard]] +=  1 ;
						$numkey[$rss[staffid]][$rss[idcard]] =  $numkey[$rss[staffid]][$rss[idcard]] + 1 ;
						$TPOINT[$rss[staffid]][$rss[idcard]] = $TPOINT[$rss[staffid]][$rss[idcard]] + ( 1*$k_point[$tb1]) ;
