...
echo "<script>alert('ติดต่อกับ server นานเกินไป กรุณา login อีกครัั้ง'); location.href='login.php';</script>";
exit();
}
...
<tr><td colspan="2" class="fillcolor"><table width="100%" border="0" cellspacing="0" cellpadding="5">
            <tr>
              <td colspan="4" align="left"><font color="#FFFFFF"><h3>ระบบมอบหมายงานสำหรับ Subcontract</h3></font></td>
        </tr>
            <tr>
              <td width="13%"  bgcolor="<? if($xmode == ""){ $bgcolor = "BLACK";echo "#FFFFFF";}else{ $bgcolor = "white"; echo "#000066"; }?>" align=center style="border-right: solid 1 white;"><A HREF="?xmode="><strong><U style="color:<?=$bgcolor?>;">จัดการข้อมูลผู้ใช้</U></strong></A></td>
              <td width="26%"  bgcolor="<? if($xmode == "1"){ $bgcolor = "BLACK"; echo "#FFFFFF";}else{ $bgcolor = "white"; echo "#000066";}?>" align=center style="border-right: solid 1 white;"><A HREF="?xmode=1"><strong><U style="color:<?=$bgcolor?>;"> ส่วนจ่ายงานให้กับผู้รับงาน</U></strong></A></td>
			   <td width="26%"  bgcolor="<? if($xmode == "2"){ $bgcolor = "BLACK"; echo "#FFFFFF";}else{ $bgcolor = "white"; echo "#000066";}?>" align=center style="border-right: solid 1 white;"><A HREF="?xmode=2"><strong><U style="color:<?=$bgcolor?>;"> ส่วนของการรอตรวจงาน</U></strong></A></td>
              <td width="61%">&nbsp;</td>
            </tr>
  </table></td></tr>
...
?>
          <tr>
            <td width="19%" align="center" bgcolor="<?=$bg1?>"><strong><a href="?xmode=2&xtype=1"><?=$fcolor1?>อยู่ระหว่างดำเนินการ<?=$fcolor_n1?></a></strong></td>
		<td width="20%" align="center" bgcolor="<?=$bg3?>"><strong><a href="?xmode=2&xtype=3"><?=$fcolor3?>รอตรวจสอบงาน<?=$fcolor_n3?></a></strong></td>
            <td width="24%" align="center" bgcolor="<?=$bg2?>"><strong><a href="?xmode=2&xtype=2"><?=$fcolor2?>ตรวจงานเรียบร้อยแล้ว<?=$fcolor_n2?></a></strong></td>
  ...
  <tr>
    <td bgcolor="#000000"><table width="100%" border="0" cellspacing="1" cellpadding="3">
      <tr>
        <td colspan="8" align="left" bgcolor="#A5B2CE"><strong>รายการรอตรวจงาน &nbsp;&nbsp;<a href="?xmode=<?=$xmode?>&xtype=<?=$xtype?>&xsapphireoffice=<?=$xsapphireoffice?>&xstatus_pay=WAIT">รอจ่ายเงิน</a> || <a href="?xmode=<?=$xmode?>&xtype=<?=$xtype?>&xsapphireoffice=<?=$xsapphireoffice?>&xstatus_pay=YES">จ่ายเงินเรียบร้อยแล้ว</a></strong></td>
        </tr>
      <tr>
	...
	$i			= (!isset($page) || ($page == 1)) ?  0 : ($page - 1) * $e ; 
	  ...
	  ## แยกประเภทพนักงานบันทึกข้อมูล
		$constaff = " AND keystaff.sapphireoffice='$xsapphireoffice'";
...
<table width="98%" border="0" align="center" cellpadding="0" cellspacing="0">
  <tr>
    <td bgcolor="#000000"><table width="100%" border="0" cellspacing="1" cellpadding="3">
      <tr>
        <td colspan="6" align="left" bgcolor="#A5B2CE"><strong>รายชื่อพนักงานที่จะมอบหมายงาน <? 
/*		if($utype == "sub"){
		 $arrday = GetDay($group_id_sub);
		 if(count($arrday) > 0){
			echo "<font color='#FF0000'>วันที่เปิดทำการมอบหมายงานคือ "	; 
			foreach($arrday as $key => $val){
				if($xstrday > "") $xstrday .= ", ";
				$xstrday .= $arr_gday[$val];	
			}
			echo "$xstrday</font>";
		}
			$xstrday = "";
			...
			if(array_key_exists($curent_day, $arrday)){
				$open_assign = 1;
			}// end if(array_key_exists($curent_day, $arrday)){
			}//end end 		if($utype == "sub"){
*/		 ?></strong></td>
        </tr>
      <tr>
        <td width="4%" align="center" bgcolor="#A5B2CE"><strong>ลำดับ</strong></td>
	 ...
	 // $sql_staff = "SELECT * FROM keystaff  WHERE status_permit = 'YES'  AND $con $xcon order by sapphireoffice ASC ";
	  ...
	  $sql_staff = "SELECT *,keystaff.staffid as staff_id  FROM keystaff LEFT JOIN tbl_assign_sub ON keystaff.staffid=tbl_assign_sub.staffid  WHERE keystaff.status_permit='YES'  AND $con $xcon  
	...
	$i=0;
	 ...
	 // $result_staff = mysql_db_query($db_name,$sql_staff);
	  ...
	  $i++;
		...
		$arrstaff = GetStatusAssign($rs1[staff_id]); // ตรวจสอบสถานะการมอบหมายงาน
		
/*		if($utype == "sub"){
				if($open_assign == "1" and  $arrstaff[0] == "1"){
					$link_assign = "<a href=\"assign_main_sub.php?staffid=$rs1[staff_id]\" target=\"_blank\"><img src=\"images/budget.gif\" alt=\"คลิ๊กเพื่อมอบหมายงาน\" width=\"16\" height=\"16\" border=\"0\"></a>";	
				}else if($arrstaff[0] == "0" and $open_assign == "1"){
					if($arrstaff[1] != ""){
							$link_assign = "<font color='#FF0000'>ส่งงานช้าก่วากำหนด มอบหมายงานได้อีกทีวันที่ ".DBThaiLongDate($arrstaff[1])."</font>";	
					}else{
							$link_assign = "<font color='#FF0000'>ใบงานเก่ายังไม่ได้ส่งไม่สามารถมอบหมายงานใหม่ได้</font>";	
					}
					
				}else{
					$link_assign = "<font color='#FF0000'>ปิด</font>";	
				}
		}else{
			$link_assign = "<a href=\"assign_main_sub.php?staffid=$rs1staff_id\" target=\"_blank\"><img src=\"images/budget.gif\" alt=\"คลิ๊กเพื่อมอบหมายงาน\" width=\"16\" height=\"16\" border=\"0\"></a>";		
		}
*/		
$link_assign = "<a href=\"assign_main_sub.php?staffid=$rs1[staff_id]\" target=\"_blank\"><img src=\"images/budget.gif\" alt=\"คลิ๊กเพื่อมอบหมายงาน\" width=\"16\" height=\"16\" border=\"0\"></a>";		
      ...
      <tr bgcolor="<?=$bg?>">
        ...
        <td align="center"><? echo $link_assign;?></td>
      </tr>
	  	  <?
	  	} // end while(){
	  ?>
...
?>
    <tr>
      <td width="16%" align="center" bgcolor="<?=$bg1?>"><strong><a href="?xmode=1&xtype=1"><?=$fcolor1?>รอจ่ายงาน<?=$fcolor_n1?></a></strong></td>
      <td width="17%" align="center" bgcolor="<?=$bg2?>"><strong><a href="?xmode=1&xtype=2"><?=$fcolor2?>จ่ายงานเรียบร้อยแล้ว<?=$fcolor_n2?></a></strong></td>
...
if($surname_th != ""){
	$xcon2 .= " AND keystaff.staffsurname LIKE '%$surname_th%'";
		...
		$consf = " AND keystaff.sapphireoffice='$xsapphireoffice'";
...
<table width="98%" border="0" align="center" cellpadding="0" cellspacing="0">
  <tr>
    <td bgcolor="#000000"><table width="100%" border="0" cellspacing="1" cellpadding="2">
      <tr>
        <td colspan="8" bgcolor="#A3B2CC"><input type="button" name="btnBB" value="ย้อนกลับ" onClick="location.href='?xmode=<?=$xmode?>&xtype=<?=$xtype?>'">
