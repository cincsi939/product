...
//session_start();
	...
	############  กำหนดกลุ่มสุดท้ายก่อนออกจากงาน
	$sql_staff = "SELECT staffid,status_extra,status_permit,period_time,keyin_group,retire_date FROM `keystaff`
where status_permit='NO' and keyin_group='0' and status_extra='NOR' and sapphireoffice='0' and period_time='am' and retire_date >  '2011-09-30'";
   $result_staff = mysql_db_query($dbnameuse,$sql_staff) or die(mysql_error()."$sql_staff<br>LINE__".__LINE__);
   while($rss = mysql_fetch_assoc($result_staff)){
	   $sql_staff1 = "SELECT keyin_group FROM stat_user_keyin WHERE staffid='$rss[staffid]' AND keyin_group > 0 ORDER BY datekeyin DESC LIMIT 1";
	   $result_staff1 = mysql_db_query($dbnameuse,$sql_staff1);
	   $rss1 = mysql_fetch_assoc($result_staff1);
	   if($rss1[keyin_group] > 0){
		 	 	 $kgroup = $rss1[keyin_group];
		}else{
				$kgroup = 3;	 
		}
	  	 
		 $sql_update = "UPDATE keystaff SET  keyin_group='$kgroup' WHERE staffid='$rss[staffid]'  ";
...
}// end 	function CleanStaffHistory(){
			
			
			
	####################  ทำการล้างค่าประวัติการย้ายกลุ่มที่ข้อมูลอยู่ 2 กลุ่มในเดือนเดียวกัน
	CleanStaffHistory();		
	...
	function GetDataGroupE($site_id,$compare_id,$date1,$date2=""){ // $ctype = 1 คือเดือนปัจจุบัน 0 คือเดือนที่ผ่านมา
		global $dbnameuse,$idcard_ex,$point_per_doc,$config_num_min,$config_num_max;
		...
		if($compare_id == "2"){
			$conid = " AND t2.datekeyin LIKE '$date1%'";
				...
				if($cyy_cmm == $date1){
					...
					$constaff_tranfer1 = "t2.datekeyin like '$date1%'";
			...
			$constaff_tranfer1 = "t2.datekeyin between '$date1' and '$date2' ";
			if($idcard_ex != ""){
				$conid .= "  AND t1.card_id IN($idcard_ex)";
				$conid_staff .= " AND t1.card_id IN($idcard_ex) ";
	...
	###########  นับจำนวนบุคลากรที่ย้ายเข้ากลุ่มใหม่
	$sql_transfer_in = "SELECT 
t1.staffid,
t1.prename,
t1.staffname,t1.staffsurname,
sum(if($constaff_tranfer1 and t2.keyin_group='3',1,0)) as numgroupN,
...
order by numgroupN desc";
//echo $sql_transfer_in."<hr>";
	$result_tranfer_in = mysql_db_query($dbnameuse,$sql_transfer_in) or die(mysql_error()."$sql_transfer_in<br>LINE__".__LINE__);
	$num_tranferin = mysql_num_rows($result_tranfer_in);
	...
	#########  นับจำนวนพนักงานคีย์ที่ยังทำงานอยู่ ########################
	
	$sql_staffkey = "SELECT Count(distinct t1.card_id) AS numstaff FROM keystaff as t1 WHERE t1.keyin_group='2'  AND  t1.period_time='am' and t1.status_extra='NOR' and t1.sapphireoffice='0' AND t1.status_permit='YES'  AND t1.card_id IN($idcard_ex)";
	$result_staffkey = mysql_db_query($dbnameuse,$sql_staffkey) or die(mysql_error()."$sql_staffkey<br>LINE__".__LINE__);
	$rssk = mysql_fetch_assoc($result_staffkey);
...
if($rs2[numday] > 25){ $numday = 25;}else{ $numday = $rs2[numday];}
	...
	$arr['staffin'] = $rsin[numstaff]+$num_tranferin;
	$arr['staffout'] = $rsout[numstaff];
	$arr['staffkey'] = $rssk[numstaff];
...
//echo $sql_transfer_in."<hr>";
	...
	$sql_staffkey = "SELECT Count(distinct t1.card_id) AS numstaff FROM keystaff as t1 WHERE t1.keyin_group='2'  AND  t1.period_time='am' and t1.status_extra='NOR' and t1.sapphireoffice='0' AND t1.status_permit='YES'  AND t1.card_id NOT IN($idcard_ex)";
	$result_staffkey = mysql_db_query($dbnameuse,$sql_staffkey) or die(mysql_error()."$sql_staffkey<br>LINE__".__LINE__);
	$rssk = mysql_fetch_assoc($result_staffkey);
...
function GetDataGroupN($site_id,$compare_id,$date1,$date2=""){
	global $dbnameuse,$point_per_doc,$config_num_min,$config_num_max;
	$temp_date = $date1."-01";
	$cyy_cmm = date("Y-m");/// เดือนปัจจุบัน
	$end_date = GetdateEnd($temp_date);
	if($compare_id == "2"){
...
FROM
keystaff AS t1
where  t1.keyin_group='3'  AND  t1.period_time='am' and t1.status_extra='NOR' and t1.sapphireoffice='0' AND t1.status_permit='YES'  
group by t1.ratio_id";
	$result_staffkey = mysql_db_query($dbnameuse,$sql_staffkey) or die(mysql_error()."$sql_staffin<br>LINE__".__LINE__);
	while($rskey = mysql_fetch_assoc($result_staffkey)){
			$arr[$rskey[ratio_id]]['staffkey'] = $rskey[numstaff];
	}//end while($rsin = mysql_fetch_assoc($result_staffin)){
		...
		###########  การแสดงข้อมูล ################
							...
							while($rs = mysql_fetch_assoc($result)){
								...
								if($rs[groupreport_id] == "1"){
										...
										$staffin = $arr_e1['staffin']+$arr_e['staffin'];// จำนวนพนักงานที่เข้างาน
										$staffout = $arr_e1['staffout']+$arr_e['staffout']; // จำนวนพนักงานที่ออกงาน
										...
										$numstaffkey = $arr_e1['staffkey']; // นับจำนวนพนักงานที่ทำงานอยู่ปัจจุบัน
										if($staff_val1 > 0){
										...
										$staffin = $arr_l1['staffin']+$arr_l['staffin'];// จำนวนพนักงานที่เข้างาน
										$staffout = $arr_l1['staffout']+$arr_l['staffout']; // จำนวนพนักงานที่ออกงาน
										...
										$numstaffkey = $arr_l1['staffkey']; // นับจำนวนพนักงานที่ทำงานอยู่ปัจจุบัน
										if($staff_val1 > 0){
										...
										$staffin =$arr_n1[$rs[r_point]]['staffin']+$arr_n[$rs[r_point]]['staffin'];// จำนวนพนักงานที่เข้างาน
										$staffout = $arr_n1[$rs[r_point]]['staffout']+$arr_n[$rs[r_point]]['staffout']; // จำนวนพนักงานที่ออกงาน
										$numdaykey1 = $arr_n[$rs[r_point]]['numday']; // จำนวนวันคีย์ข้อมูล
										$numdaykey2 = $arr_n1[$rs[r_point]]['numday']; // จำนวนวันคีย์ข้อมูล
										$numstaffkey = $arr_n1[$rs[r_point]]['staffkey']; // นับจำนวนพนักงานที่ทำงานอยู่ปัจจุบัน
										if($staff_val1 > 0){
						...
						$arr_show[$rs[groupreport_id]]['staffin'] = $staffin; // พนักงานเข้า
						$arr_show[$rs[groupreport_id]]['staffout'] = $staffout; // พนักงานออก
						...
						$arr_show[$rs[groupreport_id]]['numstaffkey'] = $numstaffkey; // นับจำนวนพนักงานที่ยังทำงานอยู่ในปัจจุบัน
...
(คน)</strong></td>
				...
				$numrx = @mysql_num_rows($result_exsum);
				if($numrx < 1){
						$sql_exsum = "SELECT * FROM view_report_keykp7  ORDER BY date_report DESC LIMIT 1 ";
						$result_exsum = mysql_db_query($dbnameuse,$sql_exsum) or die(mysql_error()."".__LINE__);
				}
				$rsex = mysql_fetch_assoc($result_exsum);
				

					$exsum_workkey = $rsex[dockey_perday]; // จำนวนชุดที่คีย์ได้ในแต่ละวัน
					$numkey_all = $rsex[keydoc_new]+$rsex[keydoc_continue]; // จำนวนเอกสารที่ทำได้ทั้งหมด
					$numkey1 = (($rsex[numdoc_new]-$rsex[keydoc_new])/$rsex[day_work]); // ข้อมูลเป้าหมาย
                ...
                <tr>
                  ...
                  <td align="center" bgcolor="#FFFFFF"><? if($exsum_workkey > 0 ){ echo "<a href='report_key_endproduct_perday.php?yymm=".date("Y-m")."' target=\"_blank\">".number_format(($exsum_workkey),2)."</a>";}else{ echo "0";}?></td>
		             ...
		             <tr>
		               <td bgcolor="#000000"><table width="100%" border="0" cellspacing="1" cellpadding="3">
		                 <tr>
		                   ...
		                   <td width="13%" rowspan="2" align="center" bgcolor="#A5B2CE"><strong>กลุ่มพนักงาน</strong></td>
		                   <td colspan="3" align="center" bgcolor="#A5B2CE"><strong>
		                     <?=$sub_h1?>
	                        </strong></td>
		                   <td colspan="9" align="center" bgcolor="#A5B2CE"><strong>
		                     <?=$sub_h2?>
	                        </strong></td>
		                   </tr>
		                 <tr>
		                   ...
		                   <td width="10%" align="center" bgcolor="#A5B2CE"><strong>ผลผลิตต่อ<br>
	                        คนต่อวัน(ชุด)</strong></td>
		                   <td width="6%" align="center" bgcolor="#A5B2CE"><strong>จำนวน (คน)</strong></td>
		                   <td width="4%" align="center" bgcolor="#A5B2CE"><strong>(+/-)</strong></td>
		                   <td width="7%" align="center" bgcolor="#A5B2CE"><strong>เข้าใหม่/<br>
	                        ปรับกลุ่ม(คน)</strong></td>
		                   <td width="7%" align="center" bgcolor="#A5B2CE"><strong>ลาออก(คน)</strong></td>
		                   <td width="6%" align="center" bgcolor="#A5B2CE"><strong>จำนวนพนักงาน<br>
		                     ปัจจุบัน(คน)</strong></td>
		                   <td width="8%" align="center" bgcolor="#A5B2CE"><strong>ผลผลิตรวม(ชุด)</strong></td>
		                   <td width="6%" align="center" bgcolor="#A5B2CE"><strong>(+/-)</strong></td>
		                   <td width="8%" align="center" bgcolor="#A5B2CE"><strong>ผลผลิตต่อคน<br>
					...
					$rpoint = 	$val['r_point'];						
		                 ...
		                 <tr bgcolor="<?=$bg?>">
						  ...
						  ?>
		                   ...
		                   <td align="center"><? if($val['numstaffkey'] > 0){ echo "<a href='report_key_endproduct_inout.php?xtype=numstaffkey&rpoint=$rpoint&groupreport_id=$key' target='_blank'>".number_format($val['numstaffkey'])."</a>";}else{ echo "0";}?></td>
                           ...
                           <?
							...
							$num9 += $val['numavg_pd'];
							$numstaffall_key += $val['numstaffkey']; // รวมจำนวนพนักงานที่ทำงานทั้งหมด
		                 ...
		                 <tr>
		                   ...
		                   <td align="center" bgcolor="#CCCCCC"><? if($numstaffall_key > 0){ echo "<a href='report_key_endproduct_inout.php?xtype=numstaffkeyall' target='_blank'>".number_format($numstaffall_key)."</a>";}else{ echo "0";}?></td>
		                   <td align="center" bgcolor="#CCCCCC"><strong>
