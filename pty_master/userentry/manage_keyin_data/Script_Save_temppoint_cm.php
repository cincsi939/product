...
where t1.datekeyin between '$date1' and '$date2' and t.site_id NOT IN('6','69','71')
...
while($rs = mysql_fetch_assoc($result)){
	...
	if($k_date > 0){
		$v_k_date = (($rs[numkpoint]*$k_date)/100);
		$netkpoint = $rs[numkpoint]+$v_k_date;
	}else{
		$k_date = 0;
		$v_k_date = 0;
		$netkpoint = $rs[numkpoint];
			
	}
	
	$kpoint_end = $netkpoint-$rs[subpoint];
	
	$sqlr = "REPLACE INTO stat_addkpoint_report SET staffid='$rs[staffid]',datekeyin='$rs[datekeyin]',numkpoint='$rs[numkpoint]',k_date='$k_date',v_k_date='$v_k_date',netkpoint='$netkpoint',subtarct_val='$rs[subpoint]',kpoint_end='$kpoint_end'";
