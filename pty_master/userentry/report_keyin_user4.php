...
//session_start();
	...
	if (!isset($datereq)){
		...
		$datereq1 = ($datetemp[0]-543)."-$datetemp[1]-$datetemp[2]";
	}
...
$sql1 = " SELECT *  FROM  stat_user_keyin   WHERE  staffid = '$staffid'  AND datekeyin  LIKE  '$datereq' ORDER BY  datekeyin  DESC  " ;
$result1 = mysql_db_query($dbnameuse,$sql1);
while($rs1 = mysql_fetch_assoc($result1)){

$sql2 = " SELECT MAX(val_max) AS maxnum  FROM  stat_user_keyin    WHERE  datekeyin = '$rs1[datekeyin]'  ";
$result2 = mysql_db_query($dbnameuse,$sql2);
$rs2 = mysql_fetch_assoc($result2);

$arrdata[$rs1[datekeyin]] = $rs1[val_max] ; 
	if($rs1[val_max] == $rs2[maxnum]){$usrstar="<img src=\"../../images_sys/iopcstar.gif\" width=\"15\" height=\"14\">";}else{$usrstar="";}
	  ?>
            <tr>
              ...
              <td align="center" bgcolor="#FFFFFF"><?=$rs1[numkeyin]?><?=$usrstar?></td>
              <td align="center" bgcolor="#FFFFFF"><?=$rs2[maxnum]?><?=$usrstar?></td>
              <td align="center" bgcolor="#FFFFFF"><?=$rs1[val_avg]?></td>
            </tr>
            <? 
	 	$j++ ;   
		}
      ...
      <tr>
        <td>
<?
$sql2 = " SELECT  *  FROM stat_user_time WHERE staffid = '$staffid'  AND  datekeyin LIKE '$datereq%'   ORDER BY  datekeyin ASC ";
//echo "$dbsite : $sql2 <br>";
$result2 = mysql_db_query($dbnameuse,$sql2) ;
while($rs2 = mysql_fetch_assoc($result2)){
	$exptime = explode(" ",$rs2[datekeyin]);
	$rs2[datekeyin] = $exptime[1];
	
$arrdata2[$rs2[datekeyin]] =  $rs2[numkeyin] ;
//echo $arrdata2[$rs2[datekeyin]];
...
$graph_path="http://".$hostgraph."/graphservice/graphservice.php";
$w1 = 850;
$h1 = 400 ;
...
foreach($arrdata2 AS $key1 => $values ){
	$d1 .= $arrdata2[$key1] . ";";
	$daylist .= $key1 . ";";
}
...
$graphurl = $graph_path . "?category=$daylist&data1=$d1&outputstyle=&numseries=1&seriesname=จำนวนรายการ&graphtype=area&title=$title&xname=วัน เดือน ปี&yname=จำนวนรายการ(ครั้ง)&subtitle=&graphstyle=srd_for_dataentry";
if($_GET['debug'] == "ON"){
	echo $graphurl."<hr>";
	echo $hostgraph."<hr>";
}
