      ...
      <td><table width="100%" border="0" cellspacing="1" cellpadding="3" style="border-color:#990000">
        <tr>
          <td><p>  <font color="#990000">
              <h3>ประกาศการเพิ่มประสิทธิภาพของเครื่องคอมพิวเตอร์แม่ข่าย</h3>
              </font>
            </p></td>
        </tr>
        <tr>
          <td><p>&nbsp;&nbsp;&nbsp;<strong>เ</strong><strong>พื่อเพิ่มประสิทธิภาพของเครื่องคอมพิวเตอร์แม่ข่าย  ในระยะเบื้องต้น บริษัท จะเปลี่ยนการคำนวณคะแนน Incentive</strong><strong><br>
จาก ทุก ๆ 2 ชั่วโมง  เป็น <u>วันละสองครั้ง คือเวลา </u><u>12.00 น. และ 24.00  น.</u> ส่งผลให้การรับรองค่าคะแนน <br>
ต้องทำการรับรองย้อนหลัง วันก่อนหน้า  ไม่สามารถรับรองในวันนั้น ๆ ได้</strong><br>
<strong>และในปัจจุบันได้มีการเพิ่มเติมหน่วยความจำ(</strong><strong>RAM) เสร็จสิ้นแล้ว&nbsp;</strong><br>
<strong>แต่เนื่องจากหน่วยประมวลผลกลาง(</strong><strong>CPU) ต้องนำเข้าจากต่างประเทศซึ่งจะทำการUpgrade  ทันทีที่ได้รับสินค้า</strong></p></td>
        </tr>
        <tr>
          <td><p><strong>จึงแจ้งมาให้ทราบโดยทั่วกัน </strong></p></td>
        </tr>
      </table></td>
    </tr>
  </table></td>
...
<td class="Label_big_black">รายงานการบันทึกข้อมูลของ   <?=$_SESSION[session_fullname]?>&nbsp;&nbsp;<a href="report_incentive_per_month.php?sent_staffid=<?=$_SESSION[session_staffid]?>&staff=keyin">รายงาน Incentive ประจำเดือน</a></td>
</tr>
<tr valign=top height=1 bgcolor="#808080"><td colspan=2></td></tr>
<tr valign=top>
  <td colspan=2 valign="middle"><img src="../../images_sys/new.gif" width="31" height="23">&nbsp;ขณะนี้ระบบได้ปรับปรุงการประมวลผลคะแนน จากทุกๆ 3 ชั่วโมงมาเป็นทุกๆ 2 ชั่วโมงแล้ว ข้อมูลของท่านจะถูกปรับปรุงทุกๆชั่วโมงที่ 2 หารลงตัว เพื่อแก้ไขปัญหาข้อมูลอัพเดตล้าช้าเบื้องต้น หากท่านใช้งานระบบช่วงต้นชั่วโมงอาจเกิดการประมวลช้าสักนิด..ระบบจะใช้เวลาการประมวลประมาณ 1 นาทีครึ่งเพื่อรวมคะแนนให้กับทุกท่าน ขออภัยในความไม่สะดวก </td>
</tr>
<tr valign=top>
  <td colspan=2><table width="750" border="0" align="center" cellpadding="1" cellspacing="0">
                ...
                <td background="images/box_bg.gif" bgcolor="#0879c7"><table width="100%" border="0" cellspacing="0" cellpadding="0">
                    <tr>
                      <td width="49%"><table width="100%" border="0" cellspacing="1" cellpadding="3">
                          <tr>
                            <td width="47%" class="style6"><span class="style7">ชื่อ - นามสกุล : </span></td>
                            <td width="53%" class="style4"><?=$name?></td>
                          </tr>
                  <!--        <tr>
                            <td class="style6"><span class="style7">เวลาเริ่มปฏิบัติงาน : </span></td>
                    ...
                    <tr>
                      <td bgcolor="#273f6f"><span class="style4">&nbsp;จำนวนบุคลากรที่บันทึกข้อมูล <strong><?=number_format($objKeyResult->CountResult)?></strong> คน</span></td>
	...
	//echo $rs1[timeupdate];
	echo "วันที่ ".ShowThaiDateTime($rs1[timeupdate]);
?></span></td>
                    </tr>
                    <tr>
...
$arrbese = GetBasePointAndPercenAdd($xstaff_id); // ค่าคะแนนมาตรฐาน
...
//$graphurl = $graph_path . "?category=$daylist&data1=$d1&data2=$d2&data3=$d3&data4=$d4&outputstyle=&numseries=3&seriesname=จำนวนรายการ;ค่าสงสุด;ค่าเฉลี่ย;เกณฑ์ค่าคะแนน&graphtype=area&title=$title&xname=วัน เดือน ปี&yname=จำนวนรายการ(ครั้ง)&subtitle=&graphstyle=srd_for_dataentry";
$graphurl = $graph_path . "?category=$daylist&data1=$d1&data2=$d2&data3=$d3&data4=$d4&outputstyle=&numseries=4&seriesname=จำนวนรายการ;ค่าสงสุด;ค่าเฉลี่ย;ค่ามาตรฐาน&graphtype=area&title=$title&xname=วัน เดือน ปี&yname=จำนวนรายการ(ครั้ง)&subtitle=&graphstyle=srd_for_dataentry_new";
                                        ...
                                        <tr>
                                          <td height="19" align="center" valign="middle" bgcolor="003499"><form name="form1" method="post" action="">
                                            <table width="100%" border="0" cellspacing="0" cellpadding="3">
                                              <tr>
                                                <td width="28%" class="style7"><strong>เลือกช่วงเวลาในการแสดงกราฟ:</strong></td>
                                                <td width="72%" class="style7"><INPUT name="start_date" onFocus="blur();" value="<?=$start_date?>" size="15" readOnly>
            <INPUT name="button" type="button"  onClick="popUpCalendar(this, form1.start_date, 'dd/mm/yyyy')"value="วันเดือนปี"> ถึง <INPUT name="end_date" onFocus="blur();" value="<?=$end_date?>" size="15" readOnly>
            <INPUT name="button" type="button"  onClick="popUpCalendar(this, form1.end_date, 'dd/mm/yyyy')"value="วันเดือนปี">
            <input type="submit" name="button2" id="button" value="แสดงกราฟ"></td>
                                              </tr>
                                            </table>
                                          </form></td>
                                        </tr>
