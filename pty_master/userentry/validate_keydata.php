...
$module_code 		= "validate_keydata"; 
$process_id			= "validate_keydata";
$VERSION 				= "9.91";
$BypassAPP 			= true;
ob_start(); 
	...
	###################################################################
	## Version :		20110709.00
	## Modified Detail :		ระบบลงลิสรายการ
...
if($_GET['action'] == "del"){
	...
	$ip_server = GET_IPADDRESS();
	$sqlfile = "SELECT * FROM validate_checkdata_upload WHERE  runid='$file_id' AND idcard='$idcard' ";
	$resultfile = mysql_db_query($dbnameuse,$sqlfile) or die(mysql_error()."$sqlfile<br>LINE__".__LINE__);
	$rsf1 = mysql_fetch_assoc($resultfile);

		
	$sqldel = "DELETE FROM validate_checkdata_upload WHERE runid='$file_id' AND idcard='$idcard'";
	mysql_db_query($dbnameuse,$sqldel) or die(mysql_error()."$sqldel<br>LINE__".__LINE__);
	######เก็บข้อมูลไฟล์ที่ลบรายการ
	
	$file_del = "../../../../../kp7file_qc/$rsf1[ticketid]/$rsf1[file_upload]";
	if(is_file($file_del)){
			unlink($file_del);
	}//end 	if(is_file($file_del)){
	
//	echo "<pre>";
//	print_r($rsf1);
	$sql_log = "INSERT INTO validate_checkdata_upload_logdelete SET idcard='$rsf1[idcard]',siteid='$rsf1[siteid]',staffid='$rsf1[staffid]',ticketid='$rsf1[ticketid]',file_upload='$rsf1[file_upload]',comment_upload='$rsf1[comment_upload]',status_edit='$rsf1[status_edit]',comment_edit='$rsf1[comment_edit]',status_approve='$rsf1[status_approve]',staff_del='".$_SESSION['session_staffid']."',ip_server='$ip_server',time_upload=NOW(),time_update=NOW() ";
	//echo $sql_log."::";die;
	mysql_db_query($dbnameuse,$sql_log) or die(mysql_error()."$sql_log<br>LINE__".__LINE__);
	
	echo "<script>alert('ลบรายการเรียบร้อยแล้ว'); location.href='?action=&idcard=$idcard&TicketID=$TicketID&staffid=$staffid&fullname=$fullname&staffname=$staffname&xsiteid=$xsiteid&flag_qc=$flag_qc';</script>";
...
//				
		...
		while($rsd = mysql_fetch_assoc($result_data)){
				...
				if($status_process_point == "1"){
					$conwp = ",status_process_point='NO'";	
				}

			
			$ArrWhere[] = " UPDATE validate_checkdata SET status_cal='$rsd[status_cal]', datecal='$rsd[datecal]',date_check='$rsd[date_check]',qc_date='".$xqc_date."' $conwp WHERE idcard='$rsd[idcard]' AND staffid='$rsd[staffid]' AND ticketid='$rsd[ticketid]' AND checkdata_id='$rsd[checkdata_id]' ";
		...
		if(count($chData) > 0){
			...
			$sql_rep = "REPLACE INTO validate_checkdata SET idcard='$idcard',staffid='$staffid', num_point='$xnum_point', checkdata_id='$v1',ticketid='$TicketID', staffid_check='".$_SESSION['session_staffid']."', date_check='".date("Y-m-d")."',timeupdate=NOW(),result_check='0',qc_staffid='$qc_staffid', qc_date='$xqc_date',datetime_check='".date("Y-m-d H:i:s")."' $conwp";
		...
		############  คำนวณคะแนน QC
		if($status_process_point !=  "1"){
			CalSubtractQc($idcard,$staffid); // คำนวนคะแนนผลการ QC
		}
...
<?
	...
	##############  วันที่คีย์ข้อมูลและเก็บข้อมูลในระบบ
/*	$datekey_stat = GetDateKeyPerson($idcard,$staffid);
	$arr_ds = explode("-",$datekey_stat);
	if($arr_ds[0] > 2000){
		$dateqc1 = $arr_ds[2]."/".$arr_ds[1]."/".($arr_ds[0]+543);
	}//end if($arr_ds[0] > 2000){
*/?>
              ...
              <td colspan="4" align="center" bgcolor="#F4F4F4"><table width="100%" border="0" cellspacing="0" cellpadding="0">
                <tr>
                  <td align="center"><table width="100%" border="0" cellspacing="1" cellpadding="3">
                    <tr>
                    <? if($dateqc1 != ""){ $qc_date= $dateqc1; } else if($date_qc != "0000-00-00" and $date_qc != ""){ $qc_date = $date_qc;}?>
                  ...
                  <td width="49%" align="left"><table width="100%" border="0" cellspacing="0" cellpadding="0">
                    <tr>
                      <td bgcolor="#000000"><table width="100%" border="0" cellspacing="1" cellpadding="3">
                       ...
                       <?
                       	$sql_file = "SELECT t1.runid,t1.idcard, t1.siteid, t1.staffid, t1.file_upload, t1.comment_upload, t1.ticketid, t1.status_edit, t1.status_approve
FROM validate_checkdata_upload as t1
WHERE t1.idcard='$idcard' and t1.siteid='$xsiteid' AND  t1.staffid='$staffid'  and t1.ticketid='$TicketID' ";
						$result_file = mysql_db_query($dbnameuse,$sql_file) or die(mysql_error()."$sql_file<br>LINE__".__LINE__);
						$numR_file = mysql_num_rows($result_file);
						if($numR_file < 1){
							echo "<tr bgcolor=\"#FFFFFF\"><td colspan='4' align='center'><b> - ไม่พบข้อมูลไฟล์แนบ -  </b></td></tr>";
						}else{
							$i=0;
							while($rsf = mysql_fetch_assoc($result_file)){
								if ($i++ %  2){ $bg = "#F0F0F0";}else{$bg = "#FFFFFF";}
								
								$file_upload = "../../../../../kp7file_qc/$rsf[ticketid]/$rsf[file_upload]";
								if(is_file($file_upload)){
									if($rsf[status_edit] == "NO"){
										$linkdel = "<a href='#' onClick=\"confirmDelete('?action=del&file_id=$rsf[runid]&idcard=$idcard&xsiteid=$xsiteid&staffid=$staffid&fullname=$fullname&staffname=$staffname&TicketID=$TicketID&flag_qc=$flag_qc')\"><img src=\"../../../../images_sys/b_drop.png\" width=\"16\" height=\"16\" title='คลิ๊กเพื่อลบรายการ' border='0'></a>";	
									}else{
										$linkdel = "<em>ไม่สามารถลบได้เนื่องจากมีการยืนยันการแก้ข้อมูลเรียบร้อยแล้ว</em>";	
									}
					   ?>
                        <tr bgcolor="<?=$bg?>">
                          <td align="center"><?=$i?></td>
                          <td align="center"><a href="<?=$file_upload?>" target="_blank"><?=$rsf[file_upload]?></a></td>
                          <td align="center"><?=$rsf[comment_upload]?></td>
                          <td align="center"><?=$linkdel?></td>
                        </tr>
                        <?
								}//end 	if(is_file($file_upload)){
							}//end while($rsf = mysql_fetch_assoc($result_file)){
						}// end if($numR_file < 1){
						?>
                      </table></td>
                    </tr>
              ...
              <? 
			  	$sql_process = "SELECT COUNT(idcard) AS numc1 FROM validate_checkdata WHERE  idcard='$idcard' AND staffid='$staffid' AND ticketid='$TicketID' AND status_process_point='NO'";
				$result_p = mysql_db_query($db_name,$sql_process);
				$rs_p = mysql_fetch_assoc($result_p);
				if($rs_p[numc1] > 0){ $checkP = "checked='checked'";}else{ $checkP = "";}
			  ?>

            <tr>
              <td colspan="4" align="center" bgcolor="#F4F4F4"><input type="checkbox" name="status_process_point" id="status_process_point" value="1" <?=$checkP?>>
