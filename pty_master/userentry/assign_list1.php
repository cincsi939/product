...
if($Aaction == "SAVE_W"){
	...
	//print_r($_POST);die;
			...
			echo "<script>alert('บันทึกข้อมูลใบงานเรียบร้อยแล้ว'); if (parent.leftFrame) {parent.leftFrame.location=parent.leftFrame.location;} location.href='assign_list1.php?action=assign_detail&TicketId=$TicketId&staffid=$staffid&profile_id=$profile_id';</script>";
...
if($action == "del"){
		...
		if($rs_c[num1] > 0){
	echo "<script>alert('ไม่สามารถลบรายการได้เนื่องจากมีรายการย่อยอยู่ในใบงานนี้'); if (parent.leftFrame) {parent.leftFrame.location=parent.leftFrame.location;} location.href='assign_list1.php?action=view_main&staffid=$staffid&profile_id=$profile_id';</script>";
		}else{
	...
	if(!(mysql_error())){
		echo "<script>alert('ลบรายการเรียบร้อยแล้ว'); if (parent.leftFrame) {parent.leftFrame.location=parent.leftFrame.location;} location.href='assign_list1.php?action=view_main&staffid=$staffid&profile_id=$profile_id';</script>";
	}
...
@mysql_db_query($db_name,$sql_del);
	...
	if(!(mysql_error())){
		echo "<script>alert('ลบรายการเรียบร้อยแล้ว'); if (parent.leftFrame) {parent.leftFrame.location=parent.leftFrame.location;} location.href='assign_list1.php?action=assign_detail&staffid=$staffid&TicketId=$ticketid&profile_id=$profile_id';</script>";
	}
    ...
    <td bgcolor="#000000"><table width="100%" cellpadding="2" cellspacing="1">
      ...
      <tr>
        ...
        <td width="9%" align="center" bgcolor="#A5B2CE"><label>
          <input type="button" name="btnA" value="เพิ่มรายการ" onClick="location.href='assign_list1.php?action=assign_key&staffid=<?=$staffid?>&smode=1&profile_id=<?=$profile_id?>'">
        </label></td>
      </tr>
	  <? 
      ...
      <tr bgcolor="<?=$bg?>">
        ...
        <td align="center">
		<?			
			echo "<a href='assign_list1.php?action=view_person&staffid=$staffid&ticketid=$rs_m[ticketid]&profile_id=$profile_id&mode=all'>".$arrkeydata[$rs_m[ticketid]]['numall']."</a>";
			//echo $rs_c1[num1];
		?>
		</td>
        <td align="center"><? if($arrkeydata[$rs_m[ticketid]]['notapprove'] > 0){ echo "<a href='assign_list1.php?action=view_person&staffid=$staffid&ticketid=$rs_m[ticketid]&profile_id=$profile_id&mode=diffkey'>".$arrkeydata[$rs_m[ticketid]]['notapprove']."</a>";}else{ echo "0";} ?></td>
        <td align="center"><? if($rs_m[assign_status] == "Y"){ echo "มอบงานแล้ว";}else{ echo "รอมอบงาน";} ?></td>
        <td align="center"><a href="assign_list1.php?action=assign_key&staffid=<?=$rs_m[staffid]?>&TicketId=<?=$rs_m[ticketid]?>&profile_id=<?=$profile_id?>"><img src="images/b_edit.png" width="16" height="16" border="0" alt="แก้ไขข้อมูล"></a> &nbsp;<img src="images/b_drop.png" width="16" height="16" border="0" alt="ลบรายการข้อมูล" onClick="return confirmDelete1('assign_list1.php?action=del&staffid=<?=$rs_m[staffid]?>&TicketId=<?=$rs_m[ticketid]?>&profile_id=<?=$profile_id?>')" style="cursor:hand"></td>
      </tr>
   	    <?
...
<table width="98%" border="0" align="center" cellpadding="0" cellspacing="0">
  <tr>
	...
	<table width="100%" border="0" cellspacing="0" cellpadding="5">
            <tr>
              <td width="17%"  bgcolor="<? if($smode == ""){ $bgcolor = "BLACK";echo "#FFFFFF";}else{ $bgcolor = "white"; echo "#000066"; }?>" align=center style="border-right: solid 1 white;"><A HREF="assign_list1.php?action=assign_key&smode=&staffid=<?=$staffid?>&TicketId=<?=$TicketId?>&profile_id=<?=$profile_id?>"><strong><U style="color:<?=$bgcolor?>;">สร้างรายการโดยระบบ</U></strong></A></td>
              <td width="24%"  bgcolor="<? if($smode == "1"){ $bgcolor = "BLACK"; echo "#FFFFFF";}else{ $bgcolor = "white"; echo "#000066";}?>" align=center style="border-right: solid 1 white;"><A HREF="assign_list1.php?action=assign_key&smode=1&staffid=<?=$staffid?>&TicketId=<?=$TicketId?>&profile_id=<?=$profile_id?>"><strong><U style="color:<?=$bgcolor?>;"> สร้างใบจ่ายงานย้อนหลัง</U></strong></A></td>
              <td width="28%" bgcolor="<? if($smode == "extraW"){ $bgcolor = "BLACK"; echo "#FFFFFF";}else{ $bgcolor = "white"; echo "#000066";}?>" align="center" style="border-right:solid 1 white;"><a href="assign_list1.php?action=assign_key&smode=extraW&staffid=<?=$staffid?>&TicketId=<?=$TicketId?>&profile_id=<?=$profile_id?>"><strong><U style="color:<?=$bgcolor?>;"> สร้างใบงานรายสัปดาห์</U></strong></a></td>
			<!-- <td width="34%"  bgcolor="<? if($smode == "2"){ $bgcolor = "BLACK"; echo "#FFFFFF";}else{ $bgcolor = "white"; echo "#000066";}?>" align=center style="border-right: solid 1 white;"><A HREF="assign_list1.php?action=assign_key&smode=2&staffid=<?=$staffid?>&TicketId=<?=$TicketId?>"><strong><U style="color:<?=$bgcolor?>;"> ระบเงื่อนไขขั้นสูง</U></strong></A></td>-->
			 <td width="31%">&nbsp;</td>
            </tr>
      </table>	</td>
  </tr>
...
<? 		  
	if($xnumwait > $maxWait){
						echo "<br><img src=\"../../images_sys/alert.png\" width=\"20\" height=\"20\" border=\"0\" title=\"แจ้งเตือนการมอบหมายงาน\"><strong><font color='red'>ขณะนี้มีจำนวนชุดที่ค้างคีย์มีจำนวน $xnumwait ชุด </font></strong>"; //die;
				}
        ...
        <tr>
          ...
          <input type="hidden" name="profile_id" value="<?=$profile_id?>">
		   ...
		   <input type="hidden" name="TicketId" value="<?=$TicketId?>">
			...
			<input type="button" name="btnB" value="ย้อนกลับ" onClick="location.href='assign_list1.php?action=assign_key&xsearch=&staffid=<?=$staffid?>'">
          </label></td>
          </tr>
      </table>    </td>
  </tr>
...
$rs2 = mysql_fetch_assoc($result2);
  ...
  <table width="100%" border="0" cellspacing="1" cellpadding="2">
        <tr>
		  ...
		  ?>
            <tr>
              <td colspan="2" align="left"><? if($xnumwait > 0){?>
              <strong><font color="#FF0000">จำนวนชุดข้อมูลที่ค้างและยังไม่ได้บันทึกข้อมูล <?=$xnumwait;?> ชุด </font></strong> <br><? }?>
        ...
        <tr>
		  ...
		  <input type="hidden" name="Aaction" value="SAVE_W">
           ...
           <input type="hidden" name="mode_assign" value="WEEK">
		    <? if($j < 1){ $dis_s = " disabled=disabled' ";}?>
            <input type="submit" name="Submit2" value="บันทึก" <?=$dis_s?>>
			<input type="button" name="btnB" value="ย้อนกลับ" onClick="location.href='assign_list1.php?action=assign_key&xsearch=&staffid=<?=$staffid?>&smode=<?=$smode?>'">
          </label></td>
          </tr>
      </table>
...
<table width="98%" border="0" align="center" cellpadding="0" cellspacing="0">
  <tr>
    <td bgcolor="#000000"><table width="100%" border="0" cellspacing="1" cellpadding="3">
      <tr>
        <td colspan="8" bgcolor="#A5B2CE"><input type="button" name="btnBB" value="ย้อนกลับ" onClick="location.href='assign_list1.php?action=view_main&staffid=<?=$staffid?>&profile_id=<?=$profile_id?>'">&nbsp;<strong>รหัสจ่ายงาน 
            <?=$ticketid?>
  ...
  <tr>
    <td bgcolor="#000000"><table width="100%" border="0" cellspacing="1" cellpadding="3">
      ...
      <tr>
        ...
        <td width="7%" align="center" bgcolor="#A5B2CE"><input type="button" name="btnA" value="เพิ่มรายการ" onClick="location.href='assign_list1.php?action=assign_key&staffid=<?=$staffid?>&TicketId=<?=$TicketId?>&profile_id=<?=$profile_id?>'"></td>
      </tr>
	  <?
//	  	$sql_detail = "SELECT
//$db_name.tbl_assign_key.ticketid,
