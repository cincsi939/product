...
//die;
			...
			$Hm = date("i");
			#echo $Hcurent."<hr>";
			#if((($Hcurent == '22' or  $Hcurent  == "23" or $Hcurent  == "12") or ($Hcurent  == "11" and $Hm > 50) or ($Hcurent  == "13" and $Hm < 10)) or $_GET['process'] == 'run'){
...
WHERE   ( keystaff.sapphireoffice = 0 )  AND (keystaff.status_extra = 'NOR' ) AND (monitor_keyin.timestamp_key LIKE  '$val%' or monitor_keyin.timeupdate LIKE '$val%') GROUP BY idcard   ORDER BY keystaff.staffid ASC    " ;
...
//$key_rpoint_arr[$rss[staffid]] = ShowQvalue($rss[staffid]);
#echo "<pre>";
#print_r($key_group_arr);
#print_r($key_rpoint_arr);
...
$yymm = substr($datereq1,0,7); // หาเปีเดือน
foreach($sumkeyuser_person as $key_person => $val_person){
	foreach($val_person as $k => $v){
		$point_key = CutNumberPoint($TPOINT_PERSON[$key_person][$k],2);
	//	echo "รหัสพนักงาน :: $key_person  เลขบัตร :: $k  ค่าคะแนน ::".$TPOINT_PERSON[$key_person][$k]."<br>";

			$check_replace = CheckPointReplacePerPerson($yymm,$k,$key_person,$point_key); 
			# ตรวจสอบว่ามีรายการคะแนนที่ทำได้ของแต่ละเอกสารในชุดเหมือนกันหรือไม่ถ้าเหมือนแสดงว่าเป็นข้อมูลที่เคยจัดเก็บในวันก่อนหน้านั้นอยู่แล้ว
			...
			//echo "idcard :: $rs_check[idcard]<br>ตรวจสอบซ้ำ : $check_replace<br>";
			...
			if($rs_check[idcard] != ""){
				...
				mysql_db_query($dbnameuse,$sql_key_person) or die(mysql_error()." LINE : ".__LINE__);
			}else{
				if($check_replace < 1){
