...
#echo "<hr>dbnameuse1 : ".$dbnameuse."<hr>";
include ("../../common/common_competency.inc.php")  ;
#echo "<hr>dbnameuse2 : ".$dbnameuse."<hr>";
include("function_block_key.php");
#echo "<hr>dbnameuse3 : ".$dbnameuse."<hr>";
include("function_face2cmss.php");
#echo "<hr>dbnameuse4 : ".$dbnameuse."<hr>";
$time_start = getmicrotime();
## JOE COMMENT
$yymmkey = date("Y-m");
$curent_datekey = date("Y-m-d"); // วันที่ปัจจุบันในการคีย์ข้อมูล
$arr_date = GetHolidayStaff($yymmkey); // วันหยุดของพนักงานคีย์ข้อมูล
ConHost(HOST,USERNAME_HOST,PASSWORD_HOST); 
##$arr_date = array("$curent_datekey"=>"$curent_datekey");

$status_key_sub = CheckBlockKeySub();// ส่วนของงาน sub 1 คือคีย์ได้ 0 คีย์ไม่ได้
if (array_key_exists($curent_datekey, $arr_date)) {
	$status_key_sub = 1;
}

##END JOE COMMENT เลี้ยงข้าวกูมื้อนึง
function GetLinkEx($area_site,$idcard,$siteid,$fullname,$profile_id){
if($area_site != ""){
		$img_link = "<a href='../hr3/tool_competency/diagnosticv1/process_check_qcdata.php?idcard=$idcard&xsiteid=$siteid&fullname=$fullname&profile_id=$profile_id' target='_blank'><img src=\"../../images_sys/gear_replace.gif\" width=\"16\" height=\"16\" border=\"0\" title=\"คลิ๊กเพื่อตรวจสอบรับรองข้อมูล\"></a>";
}else{
		$img_link = "";
}
return $img_link;
...
#echo "<hr>dbnameuse : ".$dbnameuse."<hr>";
...
<font color="#CC0000"><H1 align=center> ประกาศ เวลา 13.30-15.30  ของวันที่ 30 สิงหาคม 2554 <br>
...
FROM  `area_info`    Inner Join `eduarea` ON `area_info`.`area_id` = `eduarea`.`area_id`     WHERE  1
ORDER BY  IP , secname
" ;
...
WHERE eduarea.secid =  '$rs[siteid]' ";
		...
		$sql_confvitaya = "SELECT COUNT(idcard) as numvitaya FROM log_check_vitaya WHERE idcard='$rs[CZ_ID]' GROUP BY idcard";
		$result_confvitaya = mysql_db_query($dbnamemaster,$sql_confvitaya) or die(mysql_error()."$sql_confvitaya<br>LINE__".__LINE__);
		$rsvitaya = mysql_fetch_assoc($result_confvitaya);
		if($rsvitaya[numvitaya] > 0){
				$imgvitay  = "<img src=\"../../images_sys/iopcstar.gif\" width=\"15\" height=\"14\" title=\"สถานะยืนยันการมีวิทยฐานะ\" border=\"0\">";
		}else{
				$imgvitay = "";	
		}
?>
...
<td align=center>
<!--<A HREF="?action=edit&id=<?=$rs[id]?>&getareaid=<?=$serverid?>"></A>--><!--<img src="../../images_sys/b_edit.png" alt="กำลังปรับโปรแกรมให้แก้ไขได้ครับ" border=0>-->
<?=$imgvitay?>&nbsp; <? if($area_site != ""){ echo GetLinkEx($area_site,$rs[CZ_ID],$rs[siteid],$fullname,"99");}else{?><a href="../hr3/tool_competency/diagnosticv1/userkey_checkdata_admin.php?open_check_vitaya=1&idcard=<?=$rs[CZ_ID]?>&xsiteid=<?=$rs[siteid]?>&fullname=<?=$fullname?>' target='_blank'" target="_blank"><img src="images/package_utilities.png" title="ตรวจสอบรับรองการบันทึกข้อมูลโดย Admin" width="20" height="20" border="0"></a><? } ?>
</td>
</tr>
<?
		}//while
...
<table width="98%" border=0 align=center cellpadding=5 cellspacing=1 bgcolor="#404040">
  ...
  <?
	...
	if($status_key_sub == 0){
		$alert_msg = "สถานะการทำงานของคุณคือ Subcrontact คุณสามารถคีย์ข้อมูลได้ในวันทำงานคือ ตั้งแต่ 17:31 น. ถึง 08:59 น. และคีย์ได้ในวันอาทิตย์และวันหยุดที่บริษัทกำหนดให้เท่านั้น<br>ขณะนี้เวลา ".date("H:i:s")." น.";	
	}
...
Inner Join ".DB_USERENTRY.".tbl_assign_key as t2 ON t1.CZ_ID = t2.idcard
#AND t1.siteid = t2.siteid

Inner Join ".DB_USERENTRY.".tbl_assign_sub as t3 ON t2.ticketid = t3.ticketid
WHERE
t3.staffid =  '".$_SESSION[session_staffid]."' AND t2.approve <>  '2' AND  t2.profile_id >= 5  $consite $xconv
GROUP BY t1.CZ_ID
ORDER BY t3.ticketid DESC, t2.userkey_wait_approve ASC ,t1.CZ_ID ASC ";
...
?>
  <tr height=30 bgcolor="#efefef">
    ...
    <td align=left><?
	//echo" $flag_block ||  $status_key_sub  <br>";
	 if($flag_block == "1"){  
...
/*	$sql_check_vitaya = "SELECT idcard FROM log_check_vitaya WHERE idcard = '".$rs[CZ_ID]."' ";
    ...
    $sql_key = "SELECT COUNT(idcard) AS num_key  FROM tbl_assign_key WHERE idcard='$rs[CZ_ID]' AND nonactive='0' AND userkey_wait_approve='1' and profile_id='$rs[profile_id]' GROUP BY idcard";
	$arr_sql[] = $sql_key;
	...
	if($area_site != ""){  echo GetLinkEx($area_site,$rs[CZ_ID],$rs[siteid],$fullname,"99");}else{
	?>
    
    <a href="../hr3/tool_competency/diagnosticv1/userkey_checkdata.php?open_check_vitaya=1<?=$txt_link?>&idcard=<?=$rs[CZ_ID]?>&xsiteid=<?=$rs[siteid]?>&fullname=<?=$fullname?>first_time=1' target='_blank'" target="_blank"><img src="images/package_utilities.png" alt="ตรวจสอบผลการคีย์ข้อมูลเบื้องต้นจากพนักงานคีย์ข้อมูล" width="20" height="20" border="0"></a></td>
  </tr>
 <?
	}//end 
		}//while
