...
$point_excellent = 350;// คะแนนมาตรฐานของกลุ่ม excellent


$host_face = HOST;#"192.168.2.101";
$user_face = "edubkk_keydata";
$pass_face = "sprd!@#$%";
$dbface = "faceaccess";



##############  function นับจำนวนวันหยุด ################# โดยนับเฉพาะพนักงานชั่วคราว
function GetNumHoliday($date_s,$date_e){
	global $dbface,$host_face,$user_face,$pass_face;
	ConHost($host_face,$user_face,$pass_face); // connect faceaccess	
	$sql = "SELECT count(distinct holiday_date) as numday FROM faceacc_holiday_group  WHERE  holiday_date between '$date_s' and '$date_e'  AND period_group_id='1'";
	$result = mysql_db_query($dbface,$sql) or die(mysql_error()."$sql<br>LINE__".__LINE__);
	$rs = mysql_fetch_assoc($result);
	return $rs[numday];
}// end function GetNumHoliday($date_s,$date_e){


function GetipFaceIn($idcard){
	global $dbface,$host_face,$user_face,$pass_face;
	ConHost($host_face,$user_face,$pass_face); // connect faceaccess	
	$sql = "SELECT ipaddress FROM `faceacc_error_login` where pin='$idcard' and date(timelogin)='".date("Y-m-d")."' order by timelogin DESC limit 1;";
	$result = mysql_db_query($dbface,$sql) or die(mysql_error()."$sql<br>LINE__".__LINE__);
	$rs = mysql_fetch_assoc($result);
	ConHost(HOST,USERNAME_HOST,PASSWORD_HOST); 
	return $rs[ipaddress];
}// end function GetipFaceIn($date_s,$date_e){



##############  function แสดงการขาดลาของพนักงานในแต่ละเดือน #############
function GetNumAbsentStaff($yymm){
	global $dbface,$host_face,$user_face,$pass_face;
	ConHost($host_face,$user_face,$pass_face); // connect faceaccess	
		
/*	$sql = "SELECT
t1.pin,
Sum(if(t2.access_type_id='1',1,0)) AS sumabsent1,
Sum(if(t2.access_type_id<>'1',1,0)) AS sumabsent2
FROM
faceacc_officer as t1
Inner Join faceacc_approve as t2  ON t1.officer_id = t2.officer_id
Inner Join faceacc_site as t3 ON t1.site_id = t3.site_id
where t3.site_parent='62' and t2.approve_date LIKE '$yymm%' and t2.access_type_id <> '3' and t1.status_id='1'
group by t1.pin ";*/
$sql = "SELECT pin,sum(if(access_type_id='1',1,0))  AS sumabsent1, sum(if(access_type_id<>'1',1,0))  AS sumabsent2   FROM faceacc_log_sumperday WHERE  logDate LIKE '$yymm%'  and access_type_id NOT IN('3','6','9','8')  and approve_id > 0 GROUP BY pin";


	$result = mysql_db_query($dbface,$sql) or die(mysql_error()."$sql<br>LINE__".__LINE__);
	while($rs = mysql_fetch_assoc($result)){
		$arr[$rs[pin]]['absent_a']  = $rs[sumabsent1];	
		$arr[$rs[pin]]['absent_l'] = $rs[sumabsent2];
	}//end while($rs = mysql_fetch_assoc($result)){
	return $arr;
}// end function GetNumAbsentStaff($yymm){

###############  function แสดงวันหยุดของพนักงานคีย์ข้อมูล #######################
function GetHolidayStaff($yymm){
	global $dbface,$host_face,$user_face,$pass_face;
	#echo "$host_face :: $user_face :: $pass_face<hr>";
	ConHost($host_face,$user_face,$pass_face); // connect faceaccess	
	$sql = "SELECT holiday_date as date1 FROM faceacc_holiday_group where holiday_date LIKE '$yymm%' and (period_group_id='1' or period_group_id='3' or period_group_id='8')
GROUP BY holiday_date"; // เฉพาะกลุ่มพนักงานคีย์และลูกจ้างรายปีเท่านั้น
	$result = mysql_db_query($dbface,$sql) or die(mysql_error()."$sql<br>LINE__".__LINE__);
	while($rs = mysql_fetch_assoc($result)){
		$arr[$rs[date1]] = $rs[date1];		
	}//end while($rs = mysql_fetch_assoc($result)){
		return $arr;
}//end function GetHolidayStaff(){
	
	
function GetHolidayBetween($date1,$date2){
	global $dbface,$host_face,$user_face,$pass_face;
	ConHost($host_face,$user_face,$pass_face); // connect faceaccess	
	$sql = "SELECT holiday_date as date1 FROM faceacc_holiday_group where date(holiday_date) Between '$date1' and '$date2' and  (period_group_id='1')
GROUP BY holiday_date"; // เฉพาะกลุ่มพนักงานคีย์และลูกจ้างรายปีเท่านั้น
	$result = mysql_db_query($dbface,$sql) or die(mysql_error()."$sql<br>LINE__".__LINE__);
	while($rs = mysql_fetch_assoc($result)){
		$arr[$rs[date1]] = $rs[date1];		
		...
		return $arr;
}//end function GetHolidayBetween($date1,$date2){
	
##############  function ในการตรวจสอบการออกงานของพนักงาน ###############
...
function GetDateHoliday(){
	...
	ConHost($host_face,$user_face,$pass_face); // connect faceaccess		
			...
			$Lastmonth = date("Y-m",$xdate);// วันถัดไป	
			$sql = "SELECT t1.holiday_date FROM  faceacc_holiday_group as t1 WHERE t1.period_group_id='1' AND (t1.holiday_date like '$Lastmonth%' OR t1.holiday_date LIKE '".date("Y-m")."%')";
			$result = mysql_db_query($dbface,$sql);
			while($rs = mysql_fetch_assoc($result)){
					$arr_date[$rs[holiday_date]] = $rs[holiday_date];
			}// end 	while($rs = mysql_fetch_assoc($result)){
		return $arr_date;
}//end function GetDateHoliday(){

function GetStaffOut(){
		global $dbface,$host_face,$user_face,$pass_face;
		ConHost($host_face,$user_face,$pass_face); // connect faceaccess		
		$sql = "SELECT
t1.pin,
max(t3.time_start) as dateout,
t1.start_work
FROM
faceacc_officer AS t1
Inner Join faceacc_status AS t2 ON t1.status_id = t2.status_id
Inner Join faceacc_officer_to_status as t3 ON t1.officer_id = t3.officer_id AND t2.status_id = t3.status_id
where t2.status_id='2' or t2.status_id='3'
group by t1.pin";
		$result = mysql_db_query($dbface,$sql) or die(mysql_error()."$sql<br>LINE__".__LINE__);
		while($rs = mysql_fetch_assoc($result)){
			$arr[$rs[pin]] = $rs[dateout];
		}//end while($rs = mysql_fetch_assoc($result)){
		return $arr;
}//end function GetStaffOut(){
...
group by t1.pin";
		$result = mysql_db_query($dbface,$sql) or die(mysql_error()."$sql<br>LINE__".__LINE__);
		while($rs = mysql_fetch_assoc($result)){
			$arr[$rs[pin]] = $rs[start_work];
		}//end while($rs = mysql_fetch_assoc($result)){
		return $arr;
}//end function GetStaffOut(){
###############  function แสดงวันหยุดของพนักงานคีย์ข้อมูล #######################
function GetHolidayStaffAll($yy){
	global $dbface,$host_face,$user_face,$pass_face;
	ConHost($host_face,$user_face,$pass_face); // connect faceaccess	
	$sql = "SELECT holiday_date as date1 FROM faceacc_holiday_group where holiday_date LIKE '$yy%' and (period_group_id='1' or period_group_id='3' or period_group_id='8')
GROUP BY holiday_date"; // เฉพาะกลุ่มพนักงานคีย์และลูกจ้างรายปีเท่านั้น
	$result = mysql_db_query($dbface,$sql) or die(mysql_error()."$sql<br>LINE__".__LINE__);
	while($rs = mysql_fetch_assoc($result)){
		$arr[$rs[date1]] = $rs[date1];		
		...
		return $arr;
}//end function GetHolidayStaffAll(){

############## end function แสดงวันหยุดของพนักงานคีย์ข้อมูล #########################

 
function GetSiteKeyData(){
	global $dbface,$host_face,$user_face,$pass_face;
	ConHost($host_face,$user_face,$pass_face); // connect faceaccess
	$sql = "SELECT site_id, site_name FROM faceacc_site where site_parent='62' and site_hidden='0' order by site_name asc ";
	$result = mysql_db_query($dbface,$sql) or die(mysql_error()."$sql<br>LINE :: ".__LINE__);
	while($rs = mysql_fetch_assoc($result)){
			$arr[$rs[site_id]] = $rs[site_name];
	}//end while($rs = mysql_fetch_assoc($result)){
	return $arr;
}// end function GetSiteKeyData(){
	
	
#### function หารายชื่อของพนักงานในกลุ่มนั้น
function GetPinStaff($site_id){
	global $dbface,$host_face,$user_face,$pass_face;
	ConHost($host_face,$user_face,$pass_face); // connect faceaccess
	$sql = "SELECT t1.officer_id, t1.pin FROM faceacc_officer as t1 Inner Join faceacc_site as t2  ON t1.site_id = t2.site_id where  t2.site_id='$site_id' group by  t1.pin";	
	$result=  mysql_db_query($dbface,$sql) or die(mysql_error()."$sql<br>LINE::".__LINE__);
	$in_id = "";
	while($rs = mysql_fetch_assoc($result)){
			if($in_id > "") $in_id .= ",";
...
}// end function GetPinStaff($site_id){
