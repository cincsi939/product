...
class TableGear
{
	...
	function connect()
	{
		...
		$this->connection = mysql_connect(HOST,USERNAME_HOST, PASSWORD_HOST);
		mysql_select_db($db["database"], $this->connection);
		$iresult = mysql_query("SET character_set_results=tis-620");
		//$iresult = mysql_query("SET CHARACTER SET tis620;");
	...
	function query($query)
	{
		...
		if(!$result){
			...
			while($row = mysql_fetch_assoc($result)){
				//convert result to tis-620
				//foreach ($row as $fld => $fld_value){
				//	$row[$fld] = this->utf2tis($fld_value);
				//}
				array_push($data, $row);
			}
			return $data;
	...
	function _getPrimaryKey()
	{
		...
		$key = $columns[0]["Field"];
		//echo " SHOW COLUMNS FROM $table WHERE `Key`='PRI'  ";$key;die;
	...
	function _getInputFormat($value, $field)
	{
		...
		if($type == "date" || $type == "eDate" || $type == "timestamp" || $type == "eTimestamp"){
			...
			$value = mb_convert_kana($value, "as", "TIS-620");
			$value = preg_replace("/^(\d+)?(\d+)?(\d+)?$/", "\\2/\\3/\\1", $value);
	...
	function _updateTable($key, $insert = null)
	{
		...
		if($this->connection){
			...
			foreach($fields as $field){
				...
				if(!$this->_validateData($data, $field, $this->_httpArray["column"])) continue;
	...
	function _validateData($data, $field, $column)
	{
		...
		return preg_match($validation, $data);
	}
	...
	function _jsonEncode($array)
	{
		...
		foreach($array as $key => $value){
			...
			elseif(is_string($value)) $json .= "'".addslashes($this->utf2tis($value))."'";
