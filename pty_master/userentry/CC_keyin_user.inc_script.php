...
$strTime = (int) $_GET['time'];

$specTime = date('Y-m-d',$strTime);

$mname	= array('0'=>'','1'=>'มกราคม','2'=>'กุมภาพันธ์','3'=>'มีนาคม','4'=>'เมษายน','5'=>'พฤษภาคม','6'=>'มิถุนายน','7'=>'กรกฎาคม','8'=>'สิงหาคม',
'9'=>'กันยายน','10'=>'ตุลาคม','11'=>'พฤศจิกายน','12'=>'ธันวาคม');
			
$mname = array_map('utf2Tis620',$mname);
//@end
//echo date('Y-m-d',);
...
//die;
#$conw  = " and keystaff.staffid='10631'";
//$dayrecord = array("2011-11-25");
...
WHERE   ( keystaff.sapphireoffice = 0 )  AND (keystaff.status_extra = 'NOR' ) AND (monitor_keyin.timestamp_key LIKE  '$val%' or monitor_keyin.timeupdate LIKE '$val%')
...
//echo "<hr>$ndate $rss[staffname] :  $rss[idcard] <hr>";
		...
		foreach($arr_f_tbl1 AS $key1 => $val1){
			...
			#### ËÒ¿ÔÅ´ì·Õè¨Ð¹Óà»ç¹à»ç¹à§×Í¹ä¢¡ÒÃ¤Ô´¤Ó¹Ç³¨Ø´
	...
	while($rs_c1 = mysql_fetch_assoc($result_c1)){
				...
				//echo $sql_c2."<br> ÃËÑÊºÑµ :: $rs_c2[id]<hr>";
				$result_c2 = mysql_db_query($dbsite,$sql_c2);
				$rs_c2 = mysql_fetch_assoc($result_c2);
				$calcuatepoint=false ;
				if($rs_c2[id] > 0){
					//echo "ÃËÑÊ¤¹¤ÕÂì»Ñ¨¨ØºÑ¹  :: ".$rs1[staffid]."<br>";
					/// selectr  staff ·ÕèÁÕ auto_id ¹éÍÂ¡ÇèÒ¢Í§ºÃÃ·Ñ´à´ÕÂÇ¡Ñ¹   ¡èÍ¹Ë¹éÒÇèÒà»ç¹µÑÇàÍ§ËÃ×ÍäÁè ËÒ¡äÁèãªèãËé¤Ô´¤Ðá¹¹ÃÒÂ¨Ø´ â´Â	 $calcuatepoint =true	
					$conList = "";
					...
					if($rs_check[staffid] != $rss[staffid]){
						$calcuatepoint = true;
							...
							//echo "¨Ó¹Ç¹ÃÒÂ¡ÒÃ·Õèá¡éä¢ :: ".$numpoint."<br>";
								if($numpoint > 0){ // ¡Ã³ÕÁÕ¨Ø´¡ÒÃºÑ¹·Ö¡·ÕèµèÒ§¡Ñ¹ãËé¤Ó³Ç³â´Âãªé¤èÒ price_point ã¹¡ÒÃ¤Ù³
									$tb1 = $t[0]."$subfix";
...
foreach($sumkeyuser AS $key2=>$val2){
	###  ºÑ¹·Ö¡¢éÍÁÙÅÊ¶ÔµÔ¡ÒÃ¤ÕÂì¢éÍÁÙÅÃÒÂÇÑ¹
	$sql_ins = " REPLACE INTO  stat_user_keyin(datekeyin,staffid,val_avg,val_max,val_min,numkeyin,kval_avg,kval_max,kval_min,numkpoint,numperson,keyin_group,rpoint)VALUES('$val','$key2','$avg','$val2','$minc','".$sumkeyuser[$key2]."','','".CutNumberPoint($TPOINT[$key2],2)."','".CutNumberPoint($TPOINT[$key2],2)."','".CutNumberPoint($TPOINT[$key2],2)."','".$xnumrows[$key2]."','".$key_group_arr[$key2]."','".$key_rpoint_arr[$key2]."')";
	//echo "$sql_ins <hr>";
//	echo "$numkey[$key2] <hr>";
...
######  à¡çº¢éÍÁÙÅ¡ÒÃ·Ö¡¢éÍÁÙÅ¤Ðá¹¹ÃÒÂºØ¤Å
foreach($sumkeyuser_person as $key_person => $val_person){
	foreach($val_person as $k => $v){
	//	echo "ÃËÑÊ¾¹Ñ¡§Ò¹ :: $key_person  àÅ¢ºÑµÃ :: $k  ¤èÒ¤Ðá¹¹ ::".$TPOINT_PERSON[$key_person][$k]."<br>";
...
//			if(CheckOnlineKey($k) < 1){  /// ¡Ã³ÕäÁèä´éÍÂÙèÃÐËÇèÒ§¡ÒÃ¤ÕÂìãËé approve ä´é
//					$approve_id = 1;
//			}else{
//					$approve_id = 0;	
//			} // 
		...
		$sql_check = "SELECT * FROM stat_user_keyperson WHERE datekeyin='$datereq1' AND staffid='$key_person' AND idcard='$k'";
			echo $sql_check."<br><hr>";
			$result_check = mysql_db_query($dbnameuse,$sql_check) or die(mysql_error()."$sql_check<br>LINE__".__LINE__);
			$rs_check = mysql_fetch_assoc($result_check);
	
			if($rs_check[idcard] != ""){
				$sql_key_person = "UPDATE stat_user_keyperson SET numpoint='".CutNumberPoint($TPOINT_PERSON[$key_person][$k],2)."', numkeyin='$v',keyin_group='".$key_group_arr[$k]."',rpoint='".$key_rpoint_arr[$k]."'  WHERE datekeyin='$datereq1' AND staffid='$key_person' AND idcard='$k'";
			}else{
			$sql_key_person = "INSERT INTO stat_user_keyperson(datekeyin,staffid,idcard,numkeyin,numpoint,keyin_group,rpoint)VALUES('$datereq1','$key_person','$k','$v','".CutNumberPoint($TPOINT_PERSON[$key_person][$k],2)."','".$key_group_arr[$key_person]."','".$key_rpoint_arr[$key_person]."')";	
			}
			echo $sql_key_person."<br><hr>";
			mysql_db_query($dbnameuse,$sql_key_person) or die(mysql_error()."$sql_key_person<br>LINE__".__LINE__);
	
	}// end foreach($val_person as $k => $v){
}//end foreach($sumkeyuser_person as $key_person => $val_person){
### end à¡çº¢éÍÁÙÅ¡ÒÃ·Ö¡¢éÍÁÙÅ¤Ðá¹¹ÃÒÂºØ¤Å
###### à¡çº¤èÒ¤Ðá¹¹ªØ´¢éÍÁÙÅáÂ¡µÒÁÃÒÂ¡ÒÃ¢éÍÁÙÅ
...
#######  end à¡çº¤èÒ¤Ðá¹¹ªØ´¢éÍÁÙÅáÂ¡µÒÁÃÒÂ¡ÒÃ¢éÍÁÙÅ
...
##  end à¢ÕÂ¹¢éÍÁÙÅãÊèã¹ ranking 
$time_end = getmicrotime();
echo utf2Tis620("เวลาในการประมวลผล :").($time_end - $time_start);echo " Sec.";
