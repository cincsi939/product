...
include("function_assign_sub.php");
...
if($Aaction == "SAVE_W"){
	...
	//print_r($_POST);die;
			...
			echo "<script>alert('บันทึกข้อมูลใบงานเรียบร้อยแล้ว'); if (parent.leftFrame) {parent.leftFrame.location=parent.leftFrame.location;} location.href='assign_list_sub.php?action=assign_detail&TicketId=$TicketId&staffid=$staffid&profile_id=$profile_id';</script>";
...
if($action == "del"){
		...
		if($rs_c[num1] > 0){
	echo "<script>alert('ไม่สามารถลบรายการได้เนื่องจากมีรายการย่อยอยู่ในใบงานนี้'); if (parent.leftFrame) {parent.leftFrame.location=parent.leftFrame.location;} location.href='assign_list_sub.php?action=view_main&staffid=$staffid&profile_id=$profile_id';</script>";
		}else{
	...
	if(!(mysql_error())){
		echo "<script>alert('ลบรายการเรียบร้อยแล้ว'); if (parent.leftFrame) {parent.leftFrame.location=parent.leftFrame.location;} location.href='assign_list_sub.php?action=view_main&staffid=$staffid&profile_id=$profile_id';</script>";
	}
...
@mysql_db_query($db_name,$sql_del);
	...
	if(!(mysql_error())){
		echo "<script>alert('ลบรายการเรียบร้อยแล้ว'); if (parent.leftFrame) {parent.leftFrame.location=parent.leftFrame.location;} location.href='assign_list_sub.php?action=assign_detail&staffid=$staffid&TicketId=$ticketid&profile_id=$profile_id';</script>";
	}
...
return true;		
	...
	// function ในการ CheckFromSearch
	function CheckF(){
		if(document.form1.sent_siteid.value == ""){
				alert("กรุณาเลือกเขตพื้นที่การศึกษา");
				document.form1.sent_siteid.focus();
				return false;
		}//end 	if(document.form1.sent_siteid.value == ""){
			return true;
	}// end 	function CheckF(){
</script>
<style type="text/css">
    ...
    <td bgcolor="#000000"><table width="100%" cellpadding="2" cellspacing="1">
      ...
      <tr>
        ...
        <td width="9%" align="center" bgcolor="#A5B2CE"><label>
          <input type="button" name="btnA" value="เพิ่มรายการ" onClick="location.href='assign_list_sub.php?action=assign_key&staffid=<?=$staffid?>&smode=1&profile_id=<?=$profile_id?>'">
        </label></td>
      </tr>
	  <? 
      ...
      <tr bgcolor="<?=$bg?>">
        ...
        <td align="center">
		<?			
			echo "<a href='assign_list_sub.php?action=view_person&staffid=$staffid&ticketid=$rs_m[ticketid]&profile_id=$profile_id&mode=all'>".$arrkeydata[$rs_m[ticketid]]['numall']."</a>";
			//echo $rs_c1[num1];
		?>
		</td>
        <td align="center"><? if($arrkeydata[$rs_m[ticketid]]['notapprove'] > 0){ echo "<a href='assign_list_sub.php?action=view_person&staffid=$staffid&ticketid=$rs_m[ticketid]&profile_id=$profile_id&mode=diffkey'>".$arrkeydata[$rs_m[ticketid]]['notapprove']."</a>";}else{ echo "0";} ?></td>
        <td align="center"><? if($rs_m[assign_status] == "Y"){ echo "มอบงานแล้ว";}else{ echo "รอมอบงาน";} ?></td>
        <td align="center"><a href="assign_list_sub.php?action=assign_key&staffid=<?=$rs_m[staffid]?>&TicketId=<?=$rs_m[ticketid]?>&profile_id=<?=$profile_id?>"><img src="images/b_edit.png" width="16" height="16" border="0" alt="แก้ไขข้อมูล"></a> &nbsp;<img src="images/b_drop.png" width="16" height="16" border="0" alt="ลบรายการข้อมูล" onClick="return confirmDelete1('assign_list_sub.php?action=del&staffid=<?=$rs_m[staffid]?>&TicketId=<?=$rs_m[ticketid]?>&profile_id=<?=$profile_id?>')" style="cursor:hand"></td>
      </tr>
   	    <?
...
<table width="98%" border="0" align="center" cellpadding="0" cellspacing="0">
  <tr>
	...
	<table width="100%" border="0" cellspacing="0" cellpadding="5">
            <tr>
              <td width="56%"  bgcolor=""><strong><font color="#FFFFFF">สร้างใบงานสำหรับงาน Subcontract</font></strong></td>
              <td width="7%"  bgcolor="" align=center style="border-right: solid 1 white;"><!--<A HREF="assign_list_sub.php?action=assign_key&smode=1&staffid=<?=$staffid?>&TicketId=<?=$TicketId?>&profile_id=<?=$profile_id?>"><strong><U style="color:<?=$bgcolor?>;"> สร้างใบจ่ายงานย้อนหลัง</U></strong></A>--></td>
              <td width="6%" bgcolor="" align="center" style="border-right:solid 1 white;"><!--<a href="assign_list_sub.php?action=assign_key&smode=extraW&staffid=<?=$staffid?>&TicketId=<?=$TicketId?>&profile_id=<?=$profile_id?>"><strong><U style="color:<?=$bgcolor?>;"> สร้างใบงานรายสัปดาห์</U></strong></a>--></td>
			<!-- <td width="34%"  bgcolor="<? if($smode == "2"){ $bgcolor = "BLACK"; echo "#FFFFFF";}else{ $bgcolor = "white"; echo "#000066";}?>" align=center style="border-right: solid 1 white;"><A HREF="assign_list_sub.php?action=assign_key&smode=2&staffid=<?=$staffid?>&TicketId=<?=$TicketId?>"><strong><U style="color:<?=$bgcolor?>;"> ระบเงื่อนไขขั้นสูง</U></strong></A></td>-->
			 <td width="31%">&nbsp;</td>
            </tr>
      </table>	</td>
  </tr>
 ...
 ?>
  ...
  <tr>
    <td align="center" bgcolor="#000000"><table width="100%" border="0" cellspacing="1" cellpadding="2">
      ...
      <tr>
        <td width="17%" align="left" bgcolor="#FFFFFF"><strong>เขตพื้นที่การศึกษา</strong></td>
        <td width="28%" align="left" bgcolor="#FFFFFF"><label>
          <select name="sent_siteid" id="sent_siteid" onChange="refreshproductList();">
...
WHERE eduarea_config.group_type =  'keydata'  order by eduarea.secname ASC";*/
      ...
      <tr>
        <td align="left" bgcolor="#FFFFFF"><strong>จำนวนชุดที่มอบหมายงาน</strong></td>
        <td align="left" bgcolor="#FFFFFF">
          <select name="num_assign" id="num_assign">
          <option value="">ชุดมอบหมายงาน</option>
          <?
		  $temp_numassign = GetNumAssign($staffid);
		  if($num_assign > 0){
			 	$temp_numassign = $num_assign;
			}else if($temp_numassign < 1){
			 	$temp_numassign = $doc_assign1; 
			}
		  
          	for($intA=1;$intA <= $maxassign;$intA++){
				if($intA == $temp_numassign){ $sel = " selected='selected'";}else{ $sel="";}
				echo "<option value='$intA' $sel>$intA</option>";
			}
		  ?>
          </select></td>
        <td align="left" bgcolor="#FFFFFF"><strong>รหัสบัตร</strong></td>
      ...
      <tr>
        <td align="left" bgcolor="#FFFFFF"><strong>เงือนไขอายุราชการมากว่าหรือเท่ากับ</strong></td>
        <td align="left" bgcolor="#FFFFFF"><?=$config_age?></td>
...
<? } //end if($smode == "1"){?>	  
 ...
 #########   ###########################  ค้นหาแบบละเอียด ################
	...
	if($sent_siteid == ""){ // กรณีไม่มี sent_siteid
		echo "กรุณาเลือกเขตพื้นที่การในการมอบหมายงาน ";die;
	}
	...
	if($key_name != ""){ 
		...
		if(!($xpos === false)){ // กรณีที่มีการค้นหาชื่อหลายคน
			...
			$conW = " AND $db_site.general.name_th IN($name_search) ";
		}else{
			$conW = " AND $db_site.general.name_th LIKE '%$key_name%' ";
		} // end if(!($xpos === false)){
	}// end if($name != ""){ 
## end กรณีค้นหาชื่อ
## กรณีค้นหานามสกุล
if($key_surname != ""){
	$xpos1 = strpos($key_surname,",");
		if(!($xpos1 === false)){
			...
			$conW1 = " AND $db_site.general.surname_th IN($surname_search) ";
		}else{
			$conW1 = " AND $db_site.general.surname_th LIKE '%$key_surname%' ";
		}
}// end if($surname != ""){
...
if($key_idcard != ""){
	...
	if(!($xpos2 === false)){
		$arr_t2 = explode(",",$key_idcard);
			...
			$conW2 = " AND $db_site.general.idcard IN($idcard_search)";
	}else{
			$conW2 = " AND $db_site.general.idcard LIKE '%$key_idcard%'";
	}
}
##  end กรณีค้นหารหัสบัตร
if($schoolid > 0){
	$w_school = " AND $db_site.general.schoolid='$schoolid' ";
}
$year1 = (date("Y")+543)."-09-30";
$tbl1 = "$db_site.general";

	$sql = "SELECT $tbl1.idcard,$tbl1.siteid,$tbl1.prename_th,$tbl1.name_th ,$tbl1.surname_th,$tbl1.birthday,$tbl1.begindate,$tbl1.position_now,$tbl1.schoolid,(TIMESTAMPDIFF(MONTH,begindate,'$year1')/12) as age_gov FROM  $db_site.general
Left Join ".DB_USERENTRY.".tbl_assign_key ON $db_site.general.idcard = ".DB_USERENTRY.".tbl_assign_key.idcard AND ".DB_USERENTRY.".tbl_assign_key.profile_id = '$profile_id'  WHERE $db_site.general.siteid =  '$sent_siteid' AND ".DB_USERENTRY.".tbl_assign_key.idcard IS NULL  $conW  $conW1 $conW2 $w_school HAVING age_gov >= $config_age ";
...
//echo "$db_site :: $sql ";
	//echo $db_site;
	$result = mysql_db_query($db_site,$sql) or die(mysql_error()."$sql<br>LINE::".__LINE__);
	$ch_num = mysql_num_rows($result);
	if($ch_num > 0){
		$xnum=0;
			while($rs = mysql_fetch_assoc($result)){
							$arrx = CheckProtectionAssign($rs[idcard],$rs[siteid],$profile_id);
							if($arrx[0] > 0){
							...
							$arrpage = CheckNumKp7File($rs[idcard],$profile_id);
							if($arrpage['nump'] == "0"){
								$msg .= $arrpage['msg_err'];
								$dis = "disabled='disabled'";
							}
							...
							if(CheckTypeUser($staffid) > 0 and CheckOldData($rs[idcard]) > 0){
								if($dis == ""){
										$dis = "disabled='disabled'";
										$msg = "<font color='red'>ไม่อนุญาต assign ข้อมูลเก่าจากโครงการปีที่แล้วให้กับ sub ได้</font>";
								}	
							}else{
								$dis = $dis;
								$msg = $msg;
							}
							//echo "$rs[idcard] :: $msg<br>";
						#########  เริ่มเก็บข้อมูล
						if($msg == ""){	 // ข้อมูลผ่านเงื่อนไขการตรวจสอบ		
						$xnum++;				
							if(CheckFileKp7($rs[idcard],$rs[siteid]) == 1){
									 $link_kp7 = "<a href='../../../edubkk_kp7file/$rs[siteid]/$rs[idcard].pdf'  target='_blank'><img src='../../images_sys/gnome-mime-application-pdf.png' title='แสดงเอกสาร ก.พ.7 ต้นฉบับ' width='20' height='20' border='0'></a>";
							}else{
									$link_kp7 = "";
							}
							//	echo "$rs[prename_th]$rs[name_th]  $rs[surname_th] :: ".$rs[idcard]."<br>";
										$arrpicpage = GetNumPicPage($rs[idcard],$profile_id);
										$picpage = $arrpicpage['pic']."/".$arrpicpage['page'];
										$arr_idcard[$rs[idcard]] = $rs[idcard];
										$arr_siteid[$rs[idcard]] = $rs[siteid];
										$arr_schoolid[$rs[idcard]] = $rs[schoolid];
										$arr_fullname[$rs[idcard]] = "$rs[prename_th]$rs[name_th]  $rs[surname_th]";
										$arr_prename_th[$rs[idcard]] = "$rs[prename_th]";
										$arr_name_th[$rs[idcard]] = "$rs[name_th]";
										$arr_surname_th[$rs[idcard]] = "$rs[surname_th]";
										$arr_position[$rs[idcard]] = $rs[position_now];	
										$arr_pagepic[$rs[idcard]] = $picpage;
										$arr_kp7[$rs[idcard]] = $link_kp7;
										$arr_age[$rs[idcard]] = $rs[age_gov];
										if($xnum == $num_assign){ break;}
						}// end if($msg = "";){
  ...
  <tr>
    <td align="center" bgcolor="#000000">
      <table width="100%" border="0" cellspacing="1" cellpadding="2">
        <tr>
          <td colspan="8" align="LEFT" bgcolor="#A3B2CC">
		   <?php
		   #echo $sql;
		   if($show_by_school != ""){
		     $db_new_site = STR_PREFIX_DB.$_GET['new_site'];
		     $sql = "SELECT * , (TIMESTAMPDIFF(MONTH,begindate,'$year1')/12) as age_gov FROM general WHERE schoolid = '".$show_by_school."'  HAVING age_gov >= $config_age  ";
		   ...
		   if($show_by_person != ""){
			 ...
			 if(count($_POST['arr_idcard']) > 0){
		      ...
		      $sql = "SELECT * , (TIMESTAMPDIFF(MONTH,begindate,'$year1')/12) as age_gov FROM view_general WHERE CZ_ID IN($str_idcard)  HAVING age_gov >= $config_age ";
			 }else{
			  $_POST['arr_idcard'][] = $idcard_show;
			  $sql = "SELECT * , (TIMESTAMPDIFF(MONTH,begindate,'$year1')/12) as age_gov FROM view_general WHERE CZ_ID = '$idcard_show' HAVING age_gov >= $config_age ";
			 } 
		  ...
		  ?>
            ...
            <tr>
              <td width="23%" align="left"><strong>วันที่มอบหมายงาน ก.พ.7</strong></td>
              <td width="77%" align="left"><INPUT name="c_date" onFocus="blur();" value="<?=sw_date_intxtbox($cdate)?>" size="10" readOnly>
        ...
        <tr>
		 ...
		 if(count($arr_idcard) < 1){ // กรณีไมพบรายการ
		 		echo "<tr bgcolor='#FFFFFF'><td colspan='8' align='center'>  -ไม่พบรายการที่ค้นหา กรุณาตรวจสอบเงื่อนไขการค้นอีกครั้ง - </td></tr>";
		 }else{
        ...
        <tr>
          ...
          <input type="hidden" name="profile_id" value="<?=$profile_id?>">
		   ...
		   <input type="hidden" name="TicketId" value="<?=$TicketId?>">
			...
			<input type="button" name="btnB" value="ย้อนกลับ" onClick="location.href='assign_list_sub.php?action=assign_key&xsearch=&staffid=<?=$staffid?>'">
          </label></td>
          </tr>
      </table>    </td>
  </tr>
...
<table width="98%" border="0" align="center" cellpadding="0" cellspacing="0">
  <tr>
    <td bgcolor="#000000"><table width="100%" border="0" cellspacing="1" cellpadding="3">
      <tr>
        <td colspan="8" bgcolor="#A5B2CE"><input type="button" name="btnBB" value="ย้อนกลับ" onClick="location.href='assign_list_sub.php?action=view_main&staffid=<?=$staffid?>&profile_id=<?=$profile_id?>'">&nbsp;<strong>รหัสจ่ายงาน 
            <?=$ticketid?>
  ...
  <tr>
    <td bgcolor="#000000"><table width="100%" border="0" cellspacing="1" cellpadding="3">
      ...
      <tr>
        ...
        <td width="7%" align="center" bgcolor="#A5B2CE"><input type="button" name="btnA" value="เพิ่มรายการ" onClick="location.href='assign_list_sub.php?action=assign_key&staffid=<?=$staffid?>&TicketId=<?=$TicketId?>&profile_id=<?=$profile_id?>'"></td>
      </tr>
	  <?
//	  	$sql_detail = "SELECT
//$db_name.tbl_assign_key.ticketid,
