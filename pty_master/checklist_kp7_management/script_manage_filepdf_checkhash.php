...
?>
  <tr bgcolor="#<?=$bgcolor1?>">
    ...
    <td width="8%" align="center"><a href="report_log_upload_test.php?action=&amp;xsiteid=<?=$rs[secid]?>&amp;profile_id=<?=$profile_id?>"><img src="../../images_sys/button_select.png" width="14" height="13" / border="0" title="แสดงข้อมูล log กาีร upload ไฟล์ pdf"></a></td>
    </tr>
   <? 
...
/*if($action == "copyfile"){
	...
	$path_dest = "../../../kp7file_original/$xsecid/";
...
### log pdf main ก่อนทำการเก็บ รายละเอียด
	...
	$path_filekp7 = "../../../kp7file_original/$xsecid/";
			...
			foreach($file_pdf as $k => $v){
				...
				if($hashfile_new == $hashfile_old){
					...
					if($num_check > 0){// กรณีมีข้อมูลในเขตนี้เท่านั้น
						...
						if(file_exists($file_dest)){
								if(checkMod($rs_c[idcard],$rs_c[siteid],"checklist_kp7file") == "0"){
									unlink($file_dest);	
									SaveLogUnlinkFile($rs_c[idcard],$rs_c[siteid],"script_manage_filepdf.php",$file_dest,$profile_id);
								}//end if(checkMod($rs_c[idcard],$rs_c[siteid],"checklist_kp7file") == "0"){
						}//end if(file_exists($file_dest)){
      ...
      <tr>
        ...
        if($numrow < 0){
			...
			while($rs = mysql_fetch_assoc($result)){
			 ...
			 $file_pdf = "../../../kp7file_original/$rs[siteid]/$rs[idcard]".".pdf";
			 	if(is_file($file_pdf)){
  ...
  <tr>
    <td bgcolor="#000000"><table width="100%" border="0" cellspacing="1" cellpadding="3">
      ...
      <?
		...
		while($rs = mysql_fetch_assoc($result)){
			 if ($i++ %  2){ $bg = "#F0F0F0";}else{$bg = "#FFFFFF";}
			 $file_pdf = "../../../kp7file_original/fileall/$rs[idcard]".".pdf";
			 	if(is_file($file_pdf)){
