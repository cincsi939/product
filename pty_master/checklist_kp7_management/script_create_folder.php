...
?>
  <tr bgcolor="#<?=$bgcolor1?>">
    ...
    <td width="10%" align="center"><? if($arr1[NumPageFail] > 0){ echo "<a href='?action=view_page_fail&sentsecid=$rs[secid]&amount=$arr1[NumPageFail]&profile_id=$profile_id' target='_blank'>".number_format($arr1[NumPageFail])."</a>";}else{ echo "0";}?></td>
    <td width="16%" align="center"><? if($arr1[NumPerson] > 0){ echo "<a href='?action=view_all&sentsecid=$rs[secid]&amount=$arr1[NumPerson]&profile_id=$profile_id' target='_blank'>".number_format($arr1[NumPerson])."</a>";}else{ echo "0";}?></td>
    <td width="11%" align="center"><? if($arr1[NumPdf] > 0){ echo "<a href='?action=view_in&sentsecid=$rs[secid]&amount=$arr1[NumPdf]&profile_id=$profile_id' target='_blank'>".number_format($arr1[NumPdf])."</a>";}else{ echo "0";}?></td>
    <td width="11%" align="center"><? if($DisCount > 0){ echo "<a href='?action=view_discount&sentsecid=$rs[secid]&amount=$DisCount&profile_id=$profile_id' target='_blank'>".number_format($DisCount)."</a>";}else{ echo "0";}?></td>
    ...
    <td width="9%" align="center"><a href="?action=copyfile&xsecid=<?=$rs[secid]?>&profile_id=<?=$profile_id?>">คัดลอกไฟล์</a></td>
    <td width="6%" align="center"><a href="report_log_upload.php?action=&xsiteid=<?=$rs[secid]?>&profile_id=<?=$profile_id?>"><img src="../../images_sys/button_select.png" width="14" height="13" / border="0" title="แสดงข้อมูล log กาีร upload ไฟล์ pdf"></a></td>
    </tr>
   <? 
		...
		//}
					...
					if(copy($file_source,$file_dest)){
						...
						if(file_exists($file_dest)){
							@unlink($file_source);	
							SaveLogUnlinkFile($rs[idcard],$rs[siteid],"script_create_folder.php",$file_source,$profile_id);
					...
					if($num_check > 0){// กรณีมีข้อมูลในเขตนี้เท่านั้น
						...
						if((CheckXrefPdf($rs_c[idcard],$rs_c[siteid]) == "ok") and $xEncrytp == "ok" and $xPdfError == "ok"){ // ตรวจสอบหา xref
							...
							if(is_file($file_dest)){
								...
								@unlink($file_source);
								SaveLogUnlinkFile($rs_c[idcard],$rs_c[siteid],"script_create_folder.php",$file_source,$profile_id);
							}else{
