...
@mysql_db_query($dbname_temp,$sql_status_field);
	...
	if(count($arr_id) > 0){ // นำเข้าเฉพาะเลขบัตรที่เลือกรายบุคลที่จะทำการคีย์ข้อมูล
		...
		echo "<script>alert('ไม่พบข้อมูลที่ท่านเลือก');location.href='?action=$action&xsiteid=$xsiteid&secname=$secname&flag_crontab=1&lv=$lv&schoolid=$schoolid&profile_id=$profile_id';</script>";
		exit;	
...
###  end ทำการนำข้อมูลจาก edubkk_checklist เข้าสู่ฐาน Cmss
		...
		$result_update_general = mysql_db_query($dbsite,$sql_update_general);
			...
			if($num_error > 0){
				$msg_error = "แต่มีรายงานที่รอการยืนยันข้อมูล";	
			}
		
		echo "<script>alert(\"นำเข้าข้อมูลเรียบร้อยแล้ว \\n $msg_error\");location.href='?action=$action&xsiteid=$xsiteid&secname=$secname&flag_crontab=1&lv=$lv&schoolid=$schoolid&profile_id=$profile_id&import_id=$last_id';</script>";
		exit;
               ...
               <tr>
                 <td bgcolor="#000000"><table width="100%" border="0" cellspacing="1" cellpadding="3">
                   ...
                   <tr>
                     ...
                     <td width="41%" align="center" bgcolor="#CAD5FF"><strong>จำนวน(คน)</strong></td>
                   </tr>
                    <? if(CheckNumJ18($xsiteid) > 0){ ?>
                    <tr>
...
order by edubkk_master.hr_addposition_now.orderby  ASC";
	  ...
	  if ($i++ %  2){ $bg = "#F0F0F0";}else{$bg = "#FFFFFF";}
	  	$file_pdf = "../../../".PATH_KP7_FILE."/$xsiteid/$rsv[idcard]".".pdf";
			 	if(is_file($file_pdf)){
