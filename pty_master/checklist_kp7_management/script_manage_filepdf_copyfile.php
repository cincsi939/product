...
if($action == "getdata"){
	####  sript log pdf
	if($type == "pdf"){
	$file_pdf = ReadFileFolder($xsecid);
	$count = 1;
...
### log pdf main ก่อนทำการเก็บ รายละเอียด
	...
	foreach($file_pdf as $key => $val){
		...
		if($strlen1 > 13){
			...
			$x=0;
		for($j=0;$j<$loop1;$j++){
			$subid = substr($val,$x,13);
			$file_s = $path_n.$subid.$pdf;
			$file_d = $path_filekp7.$subid.$pdf;
			if(!file_exists($file_d)){
			@copy($file_s,$file_d);
			chmod("$file_d",0777);
			$i++;
			}//end if(!file_exists($file_d)){
			$x = $x+13;
			}//end for($j=0;$j<$loop1;$j++){
		}else{
			
			
		$file_s = $path_n.$val.$pdf;
