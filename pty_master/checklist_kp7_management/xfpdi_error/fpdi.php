...
class FPDI extends FPDF_TPL {
    ...
    function setSourceFile($filename) {
        ...
        if (!isset($this->parsers[$fn]))
          $xxx = new fpdi_pdf_parser($fn, $this);
		  $this->xvalue1= $xxx->xvalue; 
    }
    
	
	 function setSourceEncrypt($filename) {
        $this->current_filename = $filename;
        $fn =& $this->current_filename;
		
        if (!isset($this->parsers[$fn]))
          $xxx = new xfpdi_pdf_parser($fn, $this);
		  $this->xvalue2= $xxx->xvalue_x; 
    }
	
	
	function setSourceError($filename) {
        $this->current_filename = $filename;
        $fn =& $this->current_filename;
		
        if (!isset($this->parsers[$fn]))
          $xxx = new fpdi_pdf_parser_error($fn, $this);
		  $this->xvalue3= $xxx->xvaluex3; 
    }
