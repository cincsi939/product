...
class pdf_parser {
	...
	function pdf_parser($filename) {
        ...
        // Read xref-Data
       	$xreturn = $this->pdf_read_xref($this->xref, $this->pdf_find_xref());
		//echo "xv : ".$xreturn."<br>";
		$this->closeFile();
		return $xreturn;
    }
	...
	function xpdf_parser($filename) {
        ...
        $this->c =& new pdf_context($this->f);
        // Read xref-Data
       	$xreturn = $this->xpdf_read_xref($this->xref, $this->pdf_find_xref());
		//echo "xv : ".$xreturn."<br>";
		$this->closeFile();
		return $xreturn;
    }
	...
	##############################  µÃÇ¨ÊÍºä¿Åì·ÕèÁÕ»Ñ­ËÒ
		function pdf_parser_error($filename) {
        $this->filename = $filename;
        ...
        $this->c =& new pdf_context($this->f);
        // Read xref-Data
       	$xreturn = $this->pdf_read_error($this->xref, $this->pdf_find_xref());
		//echo "xv : ".$xreturn."<br>";
		$this->closeFile();
		return $xreturn;
    }
#######################   end µÃÇ¨ÊÍºä¿Åì·ÕèÁÕ»Ñ­ËÒ µéÍ§ repare
    ...
    function pdf_find_xref() {
        ...
        if (!preg_match('/\s*(\d+).*$/s', $data, $matches)) {
           ...
           // $this->error('Unable to find pointer to xref table');##   µÃÇ¨ÊÍº¡Ã³Õä¿ÅìàÊÕÂ
    	}
     ...
     */
	 function xpdf_read_xref(&$result, $offset) {
        fseek($this->f, $o_pos = $offset-20); // set some bytes backwards to fetch errorious docs
            
        $data = fread($this->f, 100000);
       // echo $data;
        $xrefPos = strpos($data, 'Encrypt');
        if (!$xrefPos === false) {
        	 return "error";
        }else{
			return "ok";
	 ...
	 function pdf_read_error(&$result, $offset) {
        fseek($this->f, $o_pos = $offset-20); // set some bytes backwards to fetch errorious docs
    ...
    }//end 	 function pdf_read_error(&$result, $offset) {
