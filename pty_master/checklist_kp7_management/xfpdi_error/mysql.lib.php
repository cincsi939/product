...
{ 
    ...
    var $Error = ""; 
	var $Table = TABLE_NAME;  // ชื่อตารางที่ทำกาำรเก็บ log ตรวจสอบข้อมูล
...
function next_record() 
	...
	####  function เก็บข้อมูล log การบันทึกข้อมูล
	function savelog($filename,$msg){
		$tbl = $this->Table;
		$xidcard = substr($filename,0,-4);
		$sql="INSERT INTO $tbl SET idcard='$xidcard',msg_error='$msg',timeupdate=NOW();"; 
        $this->query($sql); 
        $result=@mysql_fetch_array($this->Query_ID); 
        return $result[0]; 
	}//end 	function savelog($filename,$msg){
