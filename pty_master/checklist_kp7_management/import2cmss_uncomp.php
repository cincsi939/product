...
left Join edubkk_master.view_general AS t2 ON t1.idcard = t2.CZ_ID AND t1.siteid = t2.siteid
WHERE t1.siteid =  '$xsiteid' AND t1.profile_id =  '$profile_id' and t2.CZ_ID IS NULL";
		//echo $sql;die;
...
### end ทำการเทียบข้อมูลใน ตาราง checkl_data กับฐานข้อมูล cmss ถ้าข้อมูลส่วนเกินให้ย้ายและเก็บไปไว้ใน temp_general ก่อนเนื่องจากเป็นข้อมูลที่เคยคีย์อยู่แล้้ว
	//echo "AAAA";die;
		...
		echo "<script>alert('นำเข้าข้อมูลเรียบร้อยแล้ว');location.href='import2cmss_uncomp.php?action=$action&xsiteid=$xsiteid&secname=$secname&flag_crontab=1&profile_id=$profile_id&import_id=$last_id';</script>";
		exit();
         ...
         <td width="50%"><table width="100%" border="0" cellspacing="0" cellpadding="0">
           <tr>
             <td bgcolor="#000000"><table width="100%" border="0" cellspacing="1" cellpadding="2">
               <tr>
                 <td colspan="2" align="center" bgcolor="#CAD5FF"><strong>หมวดรายการนำข้อมูลสู่ระบบ cmss ในส่วนเอกสารไม่สมบูรณ์หรือยังไม่มีใน cmss</strong></td>
                 </tr>
               <tr>
                 ...
                 <td align="center" bgcolor="#000000"><table width="100%" border="0" cellspacing="1" cellpadding="3">
                   <tr>
                     <td colspan="3" align="center" bgcolor="#CAD5FF"><strong>สรุปการนำข้อมูลจาก checklist เข้าสู่ระบบ cmss<br>
                       (ระบบจะทำการนำเข้าข้อมูลบัญชี จ.18 ด้วยในกรณีที่เป็นเขตใหม่)
                     </strong></td>
                     </tr>
 ...
 <tr>
  ...
  <tr>
	...
	<thead> 
      ...
      <tr>
        <th width="3%" align="center" bgcolor="#A8B9FF"><strong>ลำดับ</strong></th>
        <th width="31%" align="center" bgcolor="#A8B9FF">หน่วยงาน/โรงเรียน</th>
        <th width="11%" align="center" bgcolor="#A8B9FF">จำนวนบุคลากร(คน)</th>
        <th width="12%" align="center" bgcolor="#A8B9FF"><strong>จำนวนหน้า<br>
          เอกสาร(แผ่น)</strong></th>
        <th width="10%" align="center" bgcolor="#A8B9FF"><strong>จำนวน<br>
          รูป(รูป)</strong></th>
        <th width="10%" align="center" bgcolor="#A8B9FF">จำนวนไฟล์ pdf<br>
          ที่อยู่ในะรบบ(คน)</th>
      </tr>
      </thead>    
	<tbody>   
		...
		while($rs = mysql_fetch_assoc($result)){
			 if ($i++ %  2){ $bg = "#F0F0F0";}else{$bg = "#FFFFFF";}
				  ...
				  $count_impP = $rs['NumAll'];// จำนวนทั้งหมด			
				  	...
				  	#$diff_p1 = $count_impP-$arr_numschool[$rs[id]];
