...
$path_pdf = "../../../".PATH_KP7_REFDOC_ORG_FILE."/";
$kp7_original = "../../../".PATH_KP7_REFDOC_ORG_FILE."/";
...
include("function_check_xrefdoc.php");
...
function ApproveScanFile($idcard,$approve="",$num_actual=""){
	...
	$sql_update = "UPDATE tbl_checklist_assign_detail SET approve='$approve',num_actual='$num_actual' WHERE idcard ='$idcard' AND profile_id='$profile_id'";
	...
	function CountPersonPdf(){
		...
		sum(if(num_actual > 0,1,0)) as NumPdf,
		sum(if(num_actual > 0 and num_actual <> num_ref,1,0)) as NumPageFail,
		...
		$arr[$rs[siteid]]['NumPdf'] = $rs[NumPdf]; // จำนวนไฟล์ pdf
		$arr[$rs[siteid]]['NumPerson'] = $rs[NumPerson]; // จำนวนบุคลากรทั้งหมด
		$arr[$rs[siteid]]['NumPageFail'] = $rs[NumPageFail]; // จำนวนหน้าที่ไม่สมบูรณ์
		$arr[$rs[siteid]]['NumTrue'] = $rs[NumTrue]; // จำนวนเอกสารที่พร้อมแสกน
	...
	function AddLogPdf($get_idcard,$get_siteid,$get_action){
		global $dbname_temp;
		$sql = "INSERT INTO tbl_log_upload_pdf SET idcard='$get_idcard',siteid='$get_siteid',action='$get_action',type_ref='Y'";
		mysql_db_query($dbname_temp,$sql);
	}
...
<table width="100%" border="1" cellpadding="3" cellspacing="1" bordercolor="#000000" class="tbl3">
      <tr>
        <td height="24" colspan="11" align="center" bgcolor="#a3b2cc"><strong><!--<a href="?action=getdata&type=pdfall">ประมวลผลทั้งหมด</a>-->โปรแกรมนำเข้าไฟล์เอกสารก่อตั้งสิทธิ์</strong></td>
        </tr>
      <tr>
        <td rowspan="2" align="center" bgcolor="#a3b2cc"><strong>ลำดับ</strong></td>
        <td rowspan="2" align="center" bgcolor="#a3b2cc"><strong>สำนักการเขต</strong></td>
...
ไม่ตรงกัน(คน)</strong></td>
  ...
  $arrXref = NumXrefError();
	...
	WHERE eduarea_config.group_type =  'keydata' AND  eduarea_config.profile_id='$profile_id'  order by cast(eduarea.secid as SIGNED) ASC";
...
?>
  <tr bgcolor="#<?=$bgcolor1?>">
    ...
    <td width="8%" align="center"><a href="report_log_upload_ref.php?action=&xsiteid=<?=$rs[secid]?>&profile_id=<?=$profile_id?>"><img src="../../images_sys/button_select.png" width="14" height="13" / border="0" title="แสดงข้อมูล log กาีร upload ไฟล์ pdf"></a></td>
    </tr>
   <? 
...
### log pdf main ก่อนทำการเก็บ รายละเอียด
	if(count($file_pdf) > 1){
		$lastlog_id = SaveLogUploadPdf($xsecid,'Y');
		$xsfile = 1;
	}else{
		$xsfile = 0;
	...
	$path_n = "../../../".PATH_KP7_REFDOC_FILE."/$xsecid/";
	if(!is_dir($path_n)){
		xRmkdir($path_n,0777);
	}
	//die;
	
	$path_old = "../../../".PATH_KP7_REFDOC_FILE."/fileall/";
	$path_filekp7 = "../../../".PATH_KP7_REFDOC_ORG_FILE."/$xsecid/";
	$path_kp7 = "../../../".PATH_KP7_REFDOC_FILE."/$xsecid/";
	...
	//echo "จำนวนทั้งหมด".count($file_pdf)."<br>";
		...
		$hashfile = 0;
			foreach($file_pdf as $k => $v){
				$v = str_replace('R','',$v); ## ตัดตัว R ออก
				...
				#echo "$hashfile_new == $hashfile_old";
				#echo "file ::".$path_old.$v.$pdf;die();
				
				if($hashfile_new == $hashfile_old){
					if(is_file($path_old.$v.$pdf)){
												
						chmod($path_old.$v.$pdf,0777);
						SaveLogUploadPdfDetail($lastlog_id,$rs_c[idcard],$rs_c[siteid],"9",'Y');// ทำการเก็บ logfile ที่เป็น hashfile เดิม 
					...
					#	echo " 3 : ".$file_source."<hr>";
						...
						if((CheckXrefPdf($rs_c[idcard],$rs_c[siteid],'R') == "ok") and $xEncrytp == "ok" and $xPdfError == "ok"){ // ตรวจสอบหา xref
							### เก็บ log
						...
						if(file_exists($file_dest)){
								if(checkMod($rs_c[idcard],$rs_c[siteid],"".PATH_KP7_REFDOC_FILE."") == "0"){
									unlink($file_dest);	
									SaveLogUnlinkFile($rs_c[idcard],$rs_c[siteid],"script_manage_filepdfrefdoc.php",$file_dest,$profile_id);
								}//end if(checkMod($rs_c[idcard],$rs_c[siteid],"".PATH_KP7_REFDOC_FILE."") == "0"){
							...
							SaveLogUploadPdfDetail($lastlog_id,$rs_c[idcard],$rs_c[siteid],"1",'Y');// ทำการเก็บ logfile
							...
							if(is_file($file_dest)){
								...
								if(file_exists($kp7file_dest)){
									if(checkMod($rs_c[idcard],$rs_c[siteid],"kp7file") == "0"){
											unlink($kp7file_dest);
											SaveLogUnlinkFile($rs_c[idcard],$rs_c[siteid],"script_manage_filepdfrefdoc.php",$kp7file_dest,$profile_id);
								...
								if($rs_cf[numid] > 0){
										...
										$sql_re = "REPLACE INTO log_pdf(idcard,secid,date_c,date_m,status_file,type_ref)VALUES('$rs_c[idcard]','$rs_c[siteid]','$temp_date','$temp_date','$status_file','Y')";	
								...
								//echo " จวนแผ่นที่ตรวจสอบ   :: $temp_page  == $rs_c[num_ref]<br>";
								//if($temp_page == $rs_c[num_ref]){
								$sql_update = "UPDATE tbl_checklist_kp7 SET num_actual='$temp_page',hash_kp7file='".$strHash."'  WHERE idcard='$rs_c[idcard]' AND siteid='$rs_c[siteid]' AND profile_id='$profile_id'  and status_file='1'";
								//echo "$sql_update<br>";
								mysql_db_query($dbname_temp,$sql_update);
								AddLogPdf($rs_c[idcard],$rs_c[siteid],"Upload Complete");
								//}//end if($temp_page == $rs_c[num_ref]){
								...
								@unlink($file_source);
								SaveLogUnlinkFile($rs_c[idcard],$rs_c[siteid],"script_manage_filepdfrefdoc.php",$file_source,$profile_id);
							}else{
						...
						$j++;
							...
							CopyFilePdfBackup($lastlog_id,$rs_c[idcard],$rs_c[siteid]); // ทำการ backup file เดิมก่อนการ copy ไฟล์ไปทับ
							if($xEncrytp == "error"){
									SaveLogUploadPdfDetail($lastlog_id,$rs_c[idcard],$rs_c[siteid],"2",'Y');// ทำการเก็บ logfile	ไฟล์มีปัญหาเข้ารหัสก่อนนำข้อมูลขึ้นไป
							}else if($xPdfError == "error"){
									SaveLogUploadPdfDetail($lastlog_id,$rs_c[idcard],$rs_c[siteid],"3",'Y');// ทำการเก็บ logfile	 ไฟล์มีปัญหา ต้อง repare
							}else{
									SaveLogUploadPdfDetail($lastlog_id,$rs_c[idcard],$rs_c[siteid],"0",'Y');// ทำการเก็บ logfile	// ไฟล์มีปัญหา หา xref ไม่เจอ
							}
	...
	if($xsfile == 1){
		...
		echo "<script>alert('ประมวลผลข้อมูลเรียบร้อยแล้วจำนวน  $j  ไฟล์ $msg_error_queue $msg_hashfile  ระบบจะแสดง log ข้อมูลในการ upload'); location.href='report_log_upload_ref.php?action=&upload_id=$lastlog_id&xsiteid=$xsecid&profile_id=$profile_id';</script>";
	}else{
...
if($action == "view_all" or $action == "view_in" or $action == "view_discount" or $action == "view_page_fail" or $action == "view_True" or $action == "view_False"){
	...
	if($action == "view_all"){
			...
			$sql = "SELECT CAST(schoolid as SIGNED) as id,idcard,siteid,prename_th,name_th,surname_th,position_now,schoolid,num_actual,num_ref  FROM tbl_checklist_kp7 WHERE siteid='$sentsecid' AND profile_id='$profile_id'  ORDER BY id ASC";
	}else if($action == "view_in"){
			$xtitle = "รายงานจำนวนบุคลากรที่มีไฟล์ pdf แล้ว ".show_area($sentsecid);	
			$sql = "SELECT CAST(schoolid as SIGNED) as id,idcard,siteid,prename_th,name_th,surname_th,position_now,schoolid,num_actual,num_ref  FROM tbl_checklist_kp7 WHERE siteid='$sentsecid' AND num_actual > 0 AND num_actual IS NOT NULL AND profile_id='$profile_id'  ORDER BY id ASC";
	}else if($action == "view_discount"){
			$xtitle = "รายงานจำนวนบุคลากรที่ยังไม่มีไฟล์ pdf ".show_area($sentsecid);		
			$sql = "SELECT CAST(schoolid as SIGNED) as id,idcard,siteid,prename_th,name_th,surname_th,position_now,schoolid,num_actual,num_ref FROM tbl_checklist_kp7 WHERE siteid='$sentsecid' AND (num_actual = 0 OR num_actual IS NULL) AND profile_id='$profile_id' and status_numfile='1' and status_check_file='YES' and status_file='1' and (mainpage IS NULL  or mainpage='' or mainpage='1') and  status_id_false='0'  ORDER BY id ASC";
			...
			$sql = "SELECT CAST(schoolid as SIGNED) as id,idcard,siteid,prename_th,name_th,surname_th,position_now,schoolid,num_actual,num_ref FROM tbl_checklist_kp7 WHERE siteid='$sentsecid' AND (num_actual > 0 and  num_actual  <> num_ref ) AND profile_id='$profile_id'   ORDER BY id ASC";	
			//echo $sql;
	}else if($action == "view_True"){
			$xtitle = "รายงานจำนวนบุคลากรที่เอกสารพร้อมแสกน ".show_area($sentsecid);		
			$sql = "SELECT CAST(schoolid as SIGNED) as id,idcard,siteid,prename_th,name_th,surname_th,position_now,schoolid,num_actual,num_ref FROM tbl_checklist_kp7 WHERE siteid='$sentsecid' AND  profile_id='$profile_id'  AND status_numfile='1' and status_check_file='YES' and status_file='1' and (mainpage IS NULL  or mainpage='' or mainpage='1') and  status_id_false='0'  ORDER BY id ASC";	
    ...
    <td bgcolor="#000000"><table width="100%" border="0" cellspacing="1" cellpadding="3">
      ...
      <tr>
        <td width="3%" rowspan="2" align="center" bgcolor="#a3b2cc"><strong>ลำ<br />
          ดับ</strong></td>
        <td width="12%" rowspan="2" align="center" bgcolor="#a3b2cc"><strong><?=TEXT_IDCARD?></strong></td>
      ...
      <tr>
        ...
        if($numrow < 0){
			...
			while($rs = mysql_fetch_assoc($result)){
			 ...
			 $file_pdf = "../../../".PATH_KP7_REFDOC_ORG_FILE."/$rs[siteid]/$rs[idcard]"."R.pdf";
			 	if(is_file($file_pdf)){
      ...
      <tr bgcolor="<?=$bg?>">
        ...
        <td align="center"><?=$rs[num_ref]?></td>
        <td align="center"><?=$rs[num_actual]?></td> 
        <? if($action == "view_page_fail"){ ?>
  ...
  <tr>
    <td bgcolor="#000000"><table width="100%" border="0" cellspacing="1" cellpadding="3">
      ...
      <tr>
        ...
        <td width="20%" align="center" bgcolor="#a3b2cc"><strong><?=TEXT_IDCARD?></strong></td>
      ...
      <?
		...
		while($rs = mysql_fetch_assoc($result)){
			 if ($i++ %  2){ $bg = "#F0F0F0";}else{$bg = "#FFFFFF";}
			 $file_pdf = "../../../".PATH_KP7_REFDOC_ORG_FILE."/fileall/$rs[idcard]"."R.pdf";
			 	if(is_file($file_pdf)){
