...
global $x,$y,$row1_x,$row1_y,$col_width,$col_height;
			$col_width = array(7,25,35,40,52,15,16);
			$col_height = 7;
			...
			$x += $col_width[2];
			$this->SetXY($x ,$y);
			$this->Cell($col_width[3],($col_height),"ตำแหน่ง",1,0,'C');
			
		
			$x += $col_width[3];
			$this->SetXY($x ,$y);
			$this->Cell($col_width[4],($col_height),"หน่วยงาน",1,0,'C');


			$x += $col_width[4];
			$this->SetXY($x ,$y);
			$this->Cell(($col_width[5]),($col_height),"อายุราชการ",1,0,'C');

			$x += $col_width[5];
			$this->SetXY($x ,$y);
			$this->Cell($col_width[6],($col_height),"จำนวน(แผ่น)",1,0,'C');
//
//			
//			$x = 10;
//			$this->SetXY($x ,$y);
