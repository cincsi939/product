...
include("checklist.inc_checkpdf.php");
//						$xtemp = CheckFilePDF("3730300850181.pdf");
//							if($temp_page == "error"){
//										echo "error";
//								}else{
//										echo "ok";
...
if($action == "copyfile"){
	...
	echo "<script>alert('คัดลอกไฟล์ได้  $j  รายการไม่สามารถคัดลอกได้ $k รายการ'); location.href='script_manage_checkfilepdf.php?action=';</script>";	
...
if($action == "getdata"){
	...
	$pdf = ".pdf";
				...
				$rs_c = mysql_fetch_assoc($result_c);
					...
					$n++;
						$file_dest = $path_n.$v.$pdf;
								...
								$temp_page = CheckFilePDF($file_dest);
								if($temp_page == "error"){
										$sql = "REPLACE INTO log_check_pdf SET idcard='$rs_c[idcard]', siteid='$rs_c[siteid]'";
										mysql_db_query("competency_system",$sql);
								}else{
										$sql_del = "DELETE FROM log_check_pdf WHERE idcard='$rs_c[idcard]' AND siteid='$rs_c[siteid]' ";	
		...
		echo "<h3><a href='script_manage_checkfilepdf.php'>กลับหน้าหลัก</a></h3>";
