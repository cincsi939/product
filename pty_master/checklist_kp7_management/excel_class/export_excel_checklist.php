...
LogImpExpExcel($xsiteid,$profile_id,"ดาวน์โหลดข้อมูลexcel กรณีเข้าเขตเก่า");
...
$worksheet1->set_column("N:N", 15);//รหัสโรงเรียน
...
$h_BNK_grey1  =& $workbook->addformat(array('align' => 'left','font'=>'CordiaUPC','color'=>'black','size'=>14,'fg_color'=>0x3c,'bold'=>1)); // พีื้นสีน้ำตาล
#มีเส้นขอบ left
...
$h_BNK_grey1_border  =& $workbook->addformat(array('align' => 'left','font'=>'CordiaUPC','color'=>'black','size'=>14,'fg_color'=>0x3c,'bold'=>1,'right'=>1,'bottom'=>1,'top'=>1,'left'=>1)); // พีื้นสีน้ำตาล
# มีเส้นขอบตรง center
...
$line6 = 6;
$line7 = 7;
$worksheet1->write($line0, 0," ", $h_LT); // ลำดับ	
$worksheet1->write($line0, 1," ", $h_T); // เลขบัตรประชาชน
$worksheet1->write($line0, 2," ", $h_T);// คำนำหน้าชื่อ	
$worksheet1->write($line0, 3," ", $h_T);//ชื่อ
$worksheet1->write($line0, 4," ", $h_T); // นามสกุล	
$worksheet1->write($line0, 5," ", $h_T); // ตำแหน่งปัจจุบัน
$worksheet1->write($line0, 6," ", $h_T); // โรงเรียน
$worksheet1->write($line0, 7," ", $h_T);// รหัสโรงเรียน
$worksheet1->write($line0, 8,"", $h_T); // อำเภอ
...
$worksheet1->write($line1, 5,"ตำแหน่งปัจจุบัน", $h_LR); // ตำแหน่งปัจจุบัน
...
$worksheet1->write($line2, 5," ", $h_BTM);// ตำแหน่งปัจจุบัน
$worksheet1->write($line2, 6," ", $h_BTM);
$worksheet1->write($line2, 7," ", $h_BTM);
$worksheet1->write($line2, 8,"", $h_BTM);
$worksheet1->write($line2, 9,"(แผ่น)", $h_BTM);
$worksheet1->write($line2, 10,"(รูป)", $h_BTM);
$worksheet1->write($line2, 11,"0=ไม่มีปก)", $h_BTM);
$worksheet1->write($line2, 12," ", $h_BTM);
$worksheet1->write($line2, 13,"2= ลบรายชื่อออก", $h_BTM);
...
$worksheet1->write($line7,16,"สีน้ำตาล คือ เอกสารค้างรับ", $h_BNK_grey1);
######  start data
...
if($rs[ty] == "1"){# สีชมพู
	...
	$worksheet1->write($int_line, 5," $rs[position_now]", $sy_left);
	$worksheet1->write($int_line, 6," $rs[office]", $sy_left);
	$worksheet1->write($int_line, 7,"$rs[id]", $sy_left);     
	$worksheet1->write($int_line, 8,"$ampur", $sy_left);
	$worksheet1->write($int_line, 9,"$rs[page_num]", $sy_left);
	$worksheet1->write($int_line, 10,"$rs[pic_num]", $sy_left);
	$worksheet1->write($int_line, 11,"$rs[mainpage]", $sy_left);
	$worksheet1->write($int_line, 12,"$rs[staff_upload_pic]", $sy_left);
	$worksheet1->write($int_line, 13,"$status_ch", $sy_left);
	$worksheet1->write($int_line, 14," ", $sy_left);
