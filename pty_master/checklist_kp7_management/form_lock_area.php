...
//	global $dbname_temp;	
//	$sql_check = "SELECT COUNT(siteid) AS numsite FROM tbl_status_lock_site WHERE siteid='$get_siteid' ";
//	$result_check = mysql_db_query($dbname_temp,$sql_check);
//	$rs_ch = mysql_fetch_assoc($result_check);
//	if($rs_ch[numsite] < 1){
//			$sql_save = "INSERT INTO tbl_status_lock_site SET siteid='$get_siteid'";
...
if($_SERVER['REQUEST_METHOD'] == "POST"){
		...
		$sql_del = "DELETE FROM tbl_status_lock_site WHERE profile_id='$profile_id' $conv ";
		...
		if(count($ch_area) > 0){
				foreach($ch_area as $k => $v){
					$sql_1 = "SELECT count(siteid) as num1 FROM tbl_status_lock_site WHERE siteid='$v' AND profile_id='$profile_id'";
					$result_1 = mysql_db_query($dbname_temp,$sql_1);
					$rs_1 = mysql_fetch_assoc($result_1);
					if($rs_1[num1] > 0){
						$sql_in = "INSERT INTO tbl_status_lock_site SET siteid='$v', status_lock='1',profile_id='$profile_id',staff_conf='".$_SESSION['session_staffid']."'";
						mysql_db_query($dbname_temp,$sql_in);
					}else{
						$sql_in = "INSERT INTO tbl_status_lock_site SET siteid='$v', status_lock='1',profile_id='$profile_id',timeupdate=NOW() ,staff_conf='".$_SESSION['session_staffid']."'";
						mysql_db_query($dbname_temp,$sql_in);	
					}//end if($rs_1[num1] > 0){
				}
		}

			echo "<script>location.href='../hr3/tool_competency/change_idcard/script_send_mail_confarea.php?profile_id=$profile_id';</script>";
		exit;		
      ...
      <table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
          <td bgcolor="#000000"><table width="100%" border="0" cellspacing="1" cellpadding="3">
            <tr>
              <td colspan="4" align="left" bgcolor="#9FB3FF"><strong>เครื่องมือในการยืนยันจำนวนบุคลากรในเขต</strong></td>
              </tr>
            <tr>
              ...
              <td width="15%" align="center" bgcolor="#9FB3FF"><strong>เลือกเขต</strong></td>
              </tr>
            <?
...
WHERE eduarea_config.group_type =  'keydata'  AND profile_id='$profile_id' ORDER BY idsite,eduarea.secname ASC";
				...
				while($rs = mysql_fetch_assoc($result)){
				 ...
				 $sql_count1 = "SELECT COUNT(siteid) AS num1 FROM tbl_status_lock_site WHERE siteid='$rs[secid]' AND profile_id='$profile_id'";
