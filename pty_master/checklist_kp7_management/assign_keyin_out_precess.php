...
$dbname_callcenter_entry = DB_USERENTRY;
$arr_logExp = array();
function assignKeyinExperience($ticketid=""){
		global $dbname, $dbname_callcenter_entry, $arr_logExp; 
		$sql_assign = "SELECT idcard FROM `tbl_assign_key` WHERE `ticketid` LIKE '".$ticketid."' ";
		$query_assign = mysql_db_query($dbname_callcenter_entry, $sql_assign);
...
function assignKeyinAVG($staffid=""){
	global $dbname_callcenter_entry; 
	...
	$sql_avg = "
		...
		";
	$query_avg = mysql_db_query($dbname_callcenter_entry, $sql_avg);
...
*/		
		...
		$query_assignTicket = mysql_db_query($dbname_callcenter_entry, $sql_assignTicket);
			...
			if($cal_rate > 0){
				...
				foreach($arr_logExp[$row_assignTicket['ticketid']] as $idcard=>$val){
					...
					if(($cal_ticketOut<$keyAVG || $cal_ticketbefore<$keyAVG) and $i < 30){
						...
						echo "$dbname_callcenter_entry ::".$sql_insert."";
						mysql_db_query($dbname_callcenter_entry, $sql_insert);
						echo "<br/>";
						 $sql_del = "DELETE FORM tbl_assign_key WHERE ticketid='".$row_assignTicket['ticketid']."' AND idcard='".$idcard."' ";
						 echo $sql_del;
						#mysql_db_query($dbname_callcenter_entry, $sql_del);
						echo "<br/>";
					}
				}
			}
