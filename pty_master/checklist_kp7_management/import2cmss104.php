...
$checkdata_f = CheckNumDataCmss(); // ตรวจสอบการทำงานของ crontab และ trigger ก่อนมีการนำเข้าข้อมูล

if($checkdata_f > 0){
		$disbtn = " disabled";
		$link_update = "<a href='../hr3/tool_competency/crontab_report_viewgeneral.php' target=\"_blank\">กรุณาคลิ๊กเพื่อปรับข้อมูลในะรบบ cmss ในตรงก่อนนำเข้าข้อมูล</a>";
}else{
		$disbtn = "";	
		$link_update = "";
}
...
#############################  กรณีไปซ้ำกับเขตอื่น ##################################		
			...
			$dbdest = "cmss_".$rs[secid];
				if($rs[status_data_old] == "1"){ // กรณีเป็นข้อมูลเก่าจากปีที่แล้ว
					...
					$result_imp2cmss = mysql_db_query($dbsite,$sql_imp2cmss);
			### ทำการคัดลอกรูป
			CopyPicCmss($rs[idcard],$rs[siteid_replace],$rs[secid]);
			## ทำการคัดลอกไฟล์ kp7
			$pathkp7file = "../../../checklist_kp7file/$rs[secid]/$rs[idcard]".".pdf";
			$pathkp7source = "../../../checklist_kp7file/$rs[secid]/";
			$pathkp7dest = "../../../".PATH_KP7_FILE."/$rs[secid]/";
...
$xsql1 = "SELECT idcard  FROM $dbname_temp.tbl_check_data WHERE profile_id='$profile_id'";
$xresult1 = mysql_db_query($dbname_temp,$xsql1);
while($xrs1 = mysql_fetch_assoc($xresult1)){
		if($xid1 > "") $xid1 .= ",";
		$xid1 .= "'$xrs1[idcard]'";
 }
if($xid1 == ""){
		$conxid = " WHERE id NOT IN('')";
}else{
		$conxid = " WHERE id NOT IN($xid1)";	
}

		//$sql1 = "SELECT $dbsite.general.id FROM $dbsite.general Left Join $dbname_temp.tbl_check_data ON $dbsite.general.id = $dbname_temp.tbl_check_data.idcard $conxid ";
		$sql1 = "SELECT id FROM general $conxid";
...
### end ทำการเทียบข้อมูลใน ตาราง checkl_data กับฐานข้อมูล cmss ถ้าข้อมูลส่วนเกินให้ย้ายและเก็บไปไว้ใน temp_general ก่อนเนื่องจากเป็นข้อมูลที่เคยคีย์อยู่แล้้ว
	//echo "AAAA";die;
		...
		echo "<script>alert('นำเข้าข้อมูลเรียบร้อยแล้ว');location.href='import2cmss.php?action=$action&xsiteid=$xsiteid&secname=$secname&flag_crontab=1&profile_id=$profile_id';</script>";
		exit;
...
<table width="99%" border="0" align="center" cellpadding="0" cellspacing="0">
 <tr>
   <td align="center"><a href="import2cmss.php?action=<?=$action?>&xsiteid=<?=$xsiteid?>&secname=<?=$secname?>&lv=<?=$lv?>&profile_id=<?=$profile_id?>">นำเข้าข้อมูทั้งเขต</a> || <a href="import2cmss_per_person.php?action=<?=$action?>&xsiteid=<?=$xsiteid?>&secname=<?=$secname?>&lv=0&profile_id=<?=$profile_id?>">นำเข้าข้อมูลรายบุคคล</a><br>
<br>
           ...
           <tr>
             <td bgcolor="#000000"><table width="100%" border="0" cellspacing="1" cellpadding="2">
               ...
               <tr bgcolor="#FFFFFF">
                 <td align="right">&nbsp;</td>
                 <td align="left">
                	 <input type="submit" name="submit" value="ประมวลผลข้อมูล" <?=$disbtn?>>
                  ...
                  <? if($link_update != ""){?>
               <tr bgcolor="#FFFFFF">
                 <td colspan="2" align="center"><font color="#FF0000">จำนวนข้อมูลในระบบ cmss ยังไม่ถูกต้อง&nbsp;<?=$link_update?> </font></td>
                 </tr>
               <? }//end  if($link_update != ""){ ?>
