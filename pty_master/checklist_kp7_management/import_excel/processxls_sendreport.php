...
$get = mysql_fetch_assoc(mysql_db_query("callcenter_entry","SELECT staffid FROM keystaff WHERE username='$username' AND password='$password' "));
$staffid = $get['staffid'];
$_SESSION['session_staffid'] = $staffid;
if($staffid==""){
	echo "<script>alert('Username หรือ  Password ผิดพลาด');window.location='../send_report_day.php?xsiteid=".$_GET['xsiteid']."'</script>";
	exit();
}
...
if(!count($data->sheets)){
	...
	die;
		...
		for ($i = $setstrartrow ; $i <= $data->sheets[0]['numRows']; $i++) {
			...
			if($idcard != ""){ // ตรวจสอบเลขบัตรต้องไม่เป็นค่าว่าง
		...
		####################   		ตรวจสอบข้อมูล
					...
					$sqlupdata = "UPDATE tbl_checklist_kp7 SET status_numfile='1',status_file='1',status_check_file='YES',type_doc='$type_doc'  $conupdate_pagepic  $conupstaff $conposition_now  $conschool  $conupdate_id  WHERE idcard='$idcard' AND profile_id='$profile_id' AND siteid='$xsiteid'";
					//echo "sql :: ".$sqlupdata."<br><br>";
					$result_update = mysql_db_query($dbname_temp,$sqlupdata);
					#GetNewCutPic($idcard,$profile_id); // update จำนวนรูปที่ต้องตัดเพิ่ม
					if(!$result_update){
						$sqlupdata1 = "REPLACE INTO tbl_checklist_kp7  SET status_numfile='1',status_file='1',status_check_file='YES',type_doc='$type_doc',idcard='$idcard',profile_id='$profile_id',siteid='$xsiteid'  $conupdate_pagepic  $conupstaff $conposition_now  $conschool  ";	
						//echo $sqlupdata1."<br><br>";
						mysql_db_query($dbname_temp,$sqlupdata1);
						#GetNewCutPic($idcard,$profile_id); // update จำนวนรูปที่ต้องตัดเพิ่ม
					...
					insert_log_checklist_last($xsiteid,$idcard,"บันทึกตรวจสอบเอกสาร ก.พ. 7 ต้นฉบับ  จากไฟล์ excel","1","$staffid","","","",$profile_id);
						...
						$arradd[] = "$idcard $prname_th$name_th  $surname_th  ::  ".$schoolname." :: ".ShowAreaSortName($xsiteid);
							...
							$sql_add = "INSERT INTO tbl_checklist_kp7 SET idcard='$idcard', siteid='$xsiteid', prename_th='$prname_th',name_th='$name_th',surname_th='$surname_th',profile_id='$profile_id',birthday='$bday1',begindate='$bday2',status_id_false='$xstatus_id_false',status_numfile='1',status_file='1',status_check_file='YES',type_doc='$type_doc' $conupdate_pagepic  $conupstaff $conposition_now  $conschool" ;
							$result_add = mysql_db_query($dbname_temp,$sql_add);
							if(!$result_add){
							...
							UpdateDataCmss($xsiteid,$idcard,$schoolid);// ตรวจสอบเพื่อ update ฐานข้อมูล cmss
								...
								$sqlupdata = "UPDATE tbl_checklist_kp7 SET status_numfile='1',status_file='1',status_check_file='YES',type_doc='$type_doc'  $conupdate_pagepic  $conupstaff $conposition_now  $conschool WHERE idcard='$idcard' AND profile_id='$profile_id' AND siteid='$xsiteid'";
					...
					$d++;
						...
						$arrdel[] = "$idcard $prename_th$name_th $surname_th";
						 ...
						 if($rs_c1[num1] > 0){
							 ...
							 $sqlupdata = "INSERT INTO tbl_checklist_kp7 status_numfile='0',status_file='0',status_check_file='YES',type_doc='$type_doc',idcard='$idcard',profile_id='$profile_id', siteid='$xsiteid'  $conupdate_pagepic  $conupstaff $conposition_now  $conschool  ";	 
						}

				
					     mysql_db_query($dbname_temp,$sqlupdata)or die(mysql_error());
						 #GetNewCutPic($idcard,$profile_id); // update จำนวนรูปที่ต้องตัดเพิ่ม
			...
			} #end if($idcard != "") // ตรวจสอบเลขบัตรต้องไม่เป็นค่าว่าง	
...
/*		echo "<center><h3>";
		...
		echo "<br>";
*/		echo "<a href=\"../../../report/report_keydata_main.php?xlv=1&profile_id=$profile_id\" target=\"_blank\">คลิ๊กเพื่อตรวจสอบหน้ารายงาน</a> | <a href=\"http://localhost/epaper_checklist/index.php\">กลับหน้าหลัก</a><br><br></h3></center>";
...
if(count($arrup > 0)){	
	...
	$xnum_norecive = number_format($arrup['norecive']);
		...
		$sqlinsertdata = "INSERT INTO epaper_checklist_profile SET 
																						comment='$comment' , siteid='".$_GET['xsiteid']."' , staffid='$staffid' , timeupdate='$timeNow' , 
																						total_person='$xnumupload' , total_person_finish='$xnumpic' , profile_checklist_id='$profile_id' ";
		//echo $sqlinsertdata;
		mysql_db_query($dbname_temp,$sqlinsertdata) or die (mysql_error());
}
