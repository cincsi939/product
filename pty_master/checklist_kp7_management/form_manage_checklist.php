...
/*if($xmsg_err1 != "" or $msg_err1 != ""){
	...
	echo "<script>alert('ไม่สามารถบันทึกได้เนื่องจากเลขบัตรไปซ้ำกับข้อมูลที่มีอยู่ในระบบ');location.href='form_manage_checklist.php?action=$action&xsiteid=$xsiteid&sentsecid=$sentsecid&schoolid=$schoolid&idcard=$idcard&prename_th=$prename_th&name_th=$name_th&surname_th=$surname_th&birthday=$birthday&begindate=$begindate&position_now=$position_now';</script>";
	exit();
...
//echo "xstatus_id_false :: $xstatus_id_false";die;
	...
	if($action == "ADD"){
		if($xmsg_err1 != ""){
	insert_log_import($sentsecid,$idcard,"ไม่สามารถบันทึกได้เนื่องจากเลขบัตรซ้ำในระบบ","1");
	echo "<script>alert(\"ไม่สามารถบันทึกได้เนื่องจากเลขบัตรไปซ้ำกับข้อมูลที่มีอยู่ในระบบ $txt_msg\");location.href='form_manage_checklist.php?action=$action&xsiteid=$xsiteid&sentsecid=$sentsecid&schoolid=$schoolid&idcard=$idcard&prename_th=$prename_th&name_th=$name_th&surname_th=$surname_th&birthday=$birthday&begindate=$begindate&position_now=$position_now&profile_id=$profile_id';</script>";
	exit();
		}else if($msg_err1 != ""){
			$sql_select = "SELECT * FROM tbl_checklist_kp7 WHERE idcard='$xidcard' AND siteid='$sentsecid' AND profile_id='$profile_id'";
		...
		$sql_check = "SELECT * FROM tbl_checklist_kp7 WHERE idcard='$xidcard' AND siteid='$sentsecid' AND profile_id='$profile_id'";
...
/*					if($extra == "1"){
				...
				echo "<script>alert(\"บันทึกข้อมูลเรียบร้อยแล้ว แต่ $txt_msg\");location.href='check_kp7_area.php?action=&xsiteid=$xsiteid&sentsecid=$sentsecid&schoolid=$schoolid&profile_id=$profile_id';</script>";
				exit();
...
/*					if($extra == "1"){
								...
								echo "<script>alert('บันทึกข้อมูลเรียบร้อยแล้ว');location.href='check_kp7_area.php?action=&xsiteid=$xsiteid&sentsecid=$sentsecid&schoolid=$schoolid&profile_id=$profile_id';</script>";
								exit();
				//	}//end if($extra == "1"){
			}
	}else{
	insert_log_import($sentsecid,$idcard,"ไม่สามารถบันทึกได้เนื่องจากเลขบัตรซ้ำในระบบ","1");
			echo "<script>alert('ไม่สามารถบันทึกได้เนื่องจากข้อมูลซ้ำกันภายในระบบตรวจสอบข้อมูลต้นฉบับ');location.href='form_manage_checklist.php?action=$action&xsiteid=$xsiteid&sentsecid=$sentsecid&schoolid=$schoolid&idcard=$idcard&prename_th=$prename_th&name_th=$name_th&surname_th=$surname_th&birthday=$birthday&begindate=$begindate&position_now=$position_now&profile_id=$profile_id';</script>";
	exit();
	}//end if($result){
...
?>
<HTML><HEAD><TITLE>ระบบตรวจสอบเอกสาร ก.พ.7 ต้นฉบับ</TITLE>
      ...
      <table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
          <td bgcolor="#000000"><table width="100%" border="0" cellspacing="1" cellpadding="3">
            ...
            <tr>
              <td width="16%" align="right" bgcolor="#FFFFFF"><strong>เลขบัตรประจำตัวประชาชน : </strong></td>
            ...
            <tr>
              ...
              <td align="left" bgcolor="#FFFFFF"><em><font color="#990000">การเปลี่ยนข้อมูลในช่อง &quot;เลขบัตรประจำตัวประชาชน&quot; มีผลเฉพาะข้อมูลใน checklist เท่านั้นซึ่งหากข้อมูลมีความสัมพันธ์กับส่วนอื่นเช่นไฟล์ ก.พ.7 และข้อมูลในระบบ cmss ให้ไปแก้ที่ระบบเปลี่ยนเลขบัตรประจำตัวประชาชน</font></em></td>
            </tr>
            <tr>
            ...
            <tr>
              ...
              <td align="left" bgcolor="#FFFFFF"><INPUT name="birthday" onFocus="blur();" value="<? if($xbirthday!='00/00/0000'){echo $xbirthday;}?>" size="15" readOnly>
            ...
            <tr>
              ...
              <td align="left" bgcolor="#FFFFFF"><INPUT name="begindate" onFocus="blur();" value="<? if($xbegindate!='00/00/0000'){ echo $xbegindate; }?>" size="15" readOnly>
            ...
            <tr>
              ...
              <td align="left" bgcolor="#FFFFFF"><label>
				...
				<?
					if($extra == "1"){ // กรณีต้องการแก้ไขหน่วยงาน
					  ...
					  echo "<option value='0'> - กรุณาระบุหน่วยงานสังกัด</option>";
						$sql_school = "SELECT * FROM allschool WHERE siteid='$sentsecid' and activate_status =  'open' ORDER BY office ASC";  	
						...
						while($rs_sch = mysql_fetch_assoc($result_school)){
							...
							echo "<option value='$rs_sch[id]' $sel>$rs_sch[prefix_name]$rs_sch[office] </option>";
						}
               		echo "</select>";
                ...
                <? if($extra != "1"){?>
              <a href="form_manage_checklist.php?action=EDIT&sentsecid=<?=$sentsecid?>&idcard=<?=$idcard?>&fullname=<?=$fullname?>&search=<?=$search?>&key_name=<?=$key_name?>&key_surname=<?=$key_surname?>&key_idcard=<?=$key_idcard?>&schoolid=<?=$schoolid?>&xsiteid=<?=$xsiteid?>&extra=1&profile_id=<?=$profile_id?>">คลิ๊กเพื่อแก้ไขหน่วยงาน</a>
              <?
				}else{
			  ?>
               <a href="form_manage_checklist.php?action=EDIT&sentsecid=<?=$sentsecid?>&idcard=<?=$idcard?>&fullname=<?=$fullname?>&search=<?=$search?>&key_name=<?=$key_name?>&key_surname=<?=$key_surname?>&key_idcard=<?=$key_idcard?>&schoolid=<?=$schoolid?>&xsiteid=<?=$xsiteid?>&profile_id=<?=$profile_id?>">ยกเลิกแก้ไขหน่วยงาน</a>
              <?
				}//end 
			  ?>
             </td>
