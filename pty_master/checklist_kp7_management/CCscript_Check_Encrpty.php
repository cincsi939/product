...
while($rst = mysql_fetch_assoc($rs_temp)){
	$xsecid = $rst[siteid];
	$file_pdf = read_file_folder("checklist_kp7file",$xsecid);
	$count = 1;
				...
				$rs_c = mysql_fetch_assoc($result_c);
					...
					$n++;
						$file_dest = $path_n.$v.$pdf;
								...
								$temp_page = CheckFileEncrypt($file_dest);
								if($temp_page == "error"){
										$sql = "REPLACE INTO log_check_pdf_encrytp SET idcard='$rs_c[idcard]', siteid='$rs_c[siteid]'";
										mysql_db_query("competency_system",$sql);
								}else{
										$sql_del = "DELETE FROM log_check_pdf_encrytp WHERE idcard='$rs_c[idcard]' AND siteid='$rs_c[siteid]' ";	
