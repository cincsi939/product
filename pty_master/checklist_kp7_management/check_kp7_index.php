...
inner Join $tbl_allschool as t2 ON edubkk_checklist.tbl_checklist_kp7.schoolid = t2.id
Left Join edubkk_checklist.temp_order_school ON t2.id = edubkk_checklist.temp_order_school.schoolid
WHERE
...
ข้าราชการครูและบุคลากรทางการศึกษา (
    ...
    <? show_area($xsiteid) ?></strong></td>
           ...
           <tr>
             ...
             <td width="11%" align="center" bgcolor="#EEEEEE"><strong>
               <?=CountSchool($xsiteid,$profile_id);?>
             </strong></td>
           ...
           <tr>
             ...
             <td colspan="2" align="center" bgcolor="#CCCCCC"><strong>ไม่มีสังกัด</strong></td>
				...
				$sql_school_num = "SELECT count(t1.idcard) as num_school FROM edubkk_checklist.tbl_checklist_kp7 AS t1 Left Join edubkk_master.allschool AS t2 ON t1.schoolid = t2.id AND t1.siteid = t2.siteid WHERE t1.siteid =  '$xsiteid' AND t1.profile_id =  '$profile_id' and t2.id is null group by t1.siteid";
...
<? } ######### END   }else{ if  ($lv == 1 ){   ?>    	 
 ...
 <tr>
    <td align="right"><table width="100%" border="0" cellspacing="0" cellpadding="4">
      <tr>
        <td width="16%" align="left"><a href="search_person_area.php?profile_id=<?=$profile_id?>"><img src="../../images_sys/searchb.gif" width="26" height="22" border="0" title="คลิ๊กเพื่อค้นหาข้อมูลบุคลากรเพื่อบันทึกผลการตรวจสอบเอกสาร">ค้นหาบุคลากร</a></td>
        <td width="60%">&nbsp;</td>
        <td width="4%">&nbsp;</td>
        <td width="20%" align="right"><strong>รายงาน ณ วันที่
  ...
  <tr>
	...
	<thead> 
      <tr>
        <th width="5%" rowspan="2" align="center" bgcolor="#A8B9FF"><strong>ลำดับ</strong><br>
          <?  if($lv == "1"){//if(CheckLockArea($xsiteid,$profile_id) > 0){ echo "<em>Lock</em>"; }else{?><input type="button" name="btnA" value="เพิ่มข้อมูล" onClick="location.href='form_manage_checklist.php?action=ADD&sentsecid=<?=$xsiteid?>&extra=1&profile_id=<?=$profile_id?>'" style="cursor:hand"><? //}  
		  } //end //end <? if(CheckLockArea($sentsecid) > 0){ ?></th>
        <? if($lv == "1"){?>
        <th width="4%" rowspan="2" align="center" bgcolor="#A8B9FF"><strong>รหัส<br>
          หน่วยงาน</strong></th>
        <? } //end ?>
        <th width="18%" rowspan="2" align="center" bgcolor="#A8B9FF"><strong><?=$xtitle?></strong>&nbsp;<a href="index_excel_form_noarea.php?profile_id=<?=$profile_id?>" target="_blank"><img src="../../images_sys/xls_logo.gif" width="16" height="16" border="0" title="คลิ๊กเพื่อส่งออกหรือนำเข้าไฟล์ excel กรณีไม่ได้มีข้อมูลตั้งต้นจาก pobec"></a></th>
        <th width="6%" rowspan="2" align="center" bgcolor="#A8B9FF"><strong></strong>อัตราจริง<br>
          รวม(คน)</th>
        <th width="6%" rowspan="2" align="center" bgcolor="#A8B9FF">
          <p>ได้รับ<br>
            เอกสาร<br>
            (คน)</p></th>
        <td colspan="2" align="center" bgcolor="#A8B9FF"><strong>ค้างรับ (คน)</strong></td>
        ...
        <th width="6%" rowspan="2" align="center" bgcolor="#A8B9FF">จำนวน<br>
          (แผ่น)</th>
        <th width="5%" rowspan="2" align="center" bgcolor="#A8B9FF"><strong><br>
          รูป</strong></th>
      </tr>
      <tr>
        <th width="7%" align="center" bgcolor="#A8B9FF">เลขบัตร<br>
สมบูรณ์</th>
        <th width="7%" align="center" bgcolor="#A8B9FF">เลขบัตร<br>
ไม่สมบูรณ์</th>
        ...
        <th width="7%" align="center" bgcolor="#A8B9FF">เลขบัตร<br>
          ไม่สมบูรณ์</th>
        <th width="7%" align="center" bgcolor="#A8B9FF">ขาดเอกสาร<br>
          ประกอบ</th>
        <th width="7%" align="center" bgcolor="#A8B9FF"><strong>ไม่สม<br>
          บูรณ์</strong></th>
      </tr> 
	</thead>    
	<tbody>   
...
FROM
$tbl_allschool as t1
left Join edubkk_checklist.temp_order_school as t2 ON t1.id = t2.schoolid
WHERE
t1.siteid=  '$xsiteid'
...
$result = mysql_db_query($dbname_temp,$sql);
		...
		while($rs = mysql_fetch_assoc($result)){
			 if ($i++ %  2){ $bg = "#F0F0F0";}else{$bg = "#FFFFFF";}
			  ...
			  if($lv == ""){
				  ...
				  $count_norecive_idfalse = $arrN[$id_org]['numnorecive_idfalse'];// ค้างรับ (คน) เลขบัตรไม่สมบูรณ์
			...
			#########  link แสดงรายการข้อมูลที่เป็นค่าว่างไม่สามารถนำเข้าระบบได้ #################
			if($arr_datanull[$rs[secid]] > 0){
				$link_datanull = "<a href='report_databefore_import.php?xsiteid=$rs[secid]&profile_id=$profile_id&schoolid=' target=\"_blank\"><img src=\"../../images_sys/attention_s.png\" width=\"18\" height=\"18\" border=\"0\" title=\"คลิ๊กเพื่อแสดงรายการข้อมูลที่เป็นค่าว่าง\"></a>";
			}else{
				$link_datanull = "";	
			}
				 ...
				 // echo "<pre>";
				  ...
				  $count_norecive_idfalse = $arrnum[$id_org]['numnorecive_idfalse'];// ค้างรับ (คน) เลขบัตรไม่สมบูรณ์
				 ...
				 if($arr_datanull[$rs[secid]] > 0){
				$link_datanull = "<a href='report_databefore_import.php?xsiteid=$rs[siteid]&profile_id=$profile_id&schoolid=$id_org' target=\"_blank\"><img src=\"../../images_sys/attention_s.png\" width=\"18\" height=\"18\" border=\"0\" title=\"คลิ๊กเพื่อแสดงรายการข้อมูลที่เป็นค่าว่าง\"></a>";
			}else{
				$link_datanull = "";	
			}
      ...
      <tr bgcolor="<?=$xbg?>">
		...
		<? } //end ?>
        <td align="left"><?=$xalink?><? if($lv == "1"){  echo $old_org ;}?>&nbsp;&nbsp;<? if($lv != "1"){?><a href="index_excel_form.php?profile_id=<?=$profile_id?>&xsiteid=<?=$sxsiteid?>" target="_blank"><img src="../../images_sys/xls_logo.gif" width="16" height="16" border="0" title="คลิ๊กเพื่อส่งออกหรือนำเข้าไฟล์ excel"></a><? }//end if($lv == "1"){  
		echo "&nbsp;".$link_datanull;
		?></td>
 ...
 <tfoot>
      <tr class="txthead">
        ...
        <td align="center" bgcolor="#CCCCCC"><strong>
         ...
         <td align="center" bgcolor="#CCCCCC"><strong> <?=number_format($sum_norecive_idfalse_all);?></strong></td>
  ...
  <tr>
    <td align="left" valign="middle"><em>หมายเหตุ <font color="#FF0000">*</font> หมายถึงเขตที่ตรวจสอบจำนวนถูกต้องแล้ว<br>
    บรรทัดที่เป็น สีส้ม คือหน่วนงานที่ปิดไปแล้ว</em></td>
  </tr>
</table>
