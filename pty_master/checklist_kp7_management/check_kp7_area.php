...
if($action == "process"){
	...
	if($flag_sup == "1"){
			$sql_unlock = "UPDATE tbl_assign_key SET unlock_idcardkey='1',approve='0' WHERE idcard='$idcard' AND siteid='$sentsecid' AND profile_id='$profile_id'";
			mysql_db_query($dbnameuse,$sql_unlock);
	}
...
//print_r($_POST);
	...
	while($rs = mysql_fetch_assoc($result)){
		if($$rs[field_name] == "1"){
			...
			//echo $sql_update_problem."<br>";
			mysql_db_query($dbname_temp,$sql_update_problem);
				foreach($arr_doc_comment as $k => $v){
...
/*	echo "<pre>";
	...
	if(count($commentT) > 0 or count($commentT1) > 0){
			foreach($commentT as $kx1 => $vx1){
				$val_comment = $vx1." ".$commentT1[$kx1];
					$update_con .= " ,$kx1='$val_comment' ";		
			}//end foreach($commentT as $kx1 => $vx1){
	}else{*/
			$menu_comp = GetFieldComment();
			//echo "<pre>";
			//print_r($menu_comp);
			if(count($menu_comp) > 0){
				foreach($menu_comp as $key_field => $val_field){
						$val_comment = $commentT[$val_field]."".$commentT1[$val_field];
						$update_con .= ",$val_field='$val_comment'";
				}
			}//end if(count($arrmenu) > 0){
	...
	//echo $dbname_temp." <br>".$sql_update;die;
	$result_update = mysql_db_query($dbname_temp,$sql_update)or die(mysql_error()." LINE ::".__LINE__."<br>$sql_update");
	### update รูปที่ต้องตัดเพิ่ม
	//GetNewCutPic($idcard,$profile_id);
	chkUpdateNewCut_profile($idcard,$profile_id,$sentsecid);
...
function check_radio_all(txtpotion){
	var arrmenu = new Array('','general_status','graduate_status','salary_status','seminar_status','sheet_status','getroyal_status','special_status','goodman_status','absent_status','nosalary_status','prohibit_status','specialduty_status','other_status');
		...
		//var arrmenu = new Array('','general_status','salary_status');
		//		for(i=1;i<3;i++){
//					document.getElementById(arrmenu[i]+"2").checked=true;
//					 sw_table(0,i)
//				}//end for(i=0;i<count(arrment);i++){
					document.getElementById("general_status2").checked=true;
					sw_table(0,1);
					document.getElementById("salary_status2").checked=true;
					sw_table(0,3);
		}
...
<!--        <tr>
		...
		<?
			...
			####  สถานะเอกสารสมบูรณ์
			if($rsm1[status_active] == "1"){
				 if($rs_e["$fieldname"] == "1"){
					$xcheck = "checked='checked' ";
				}else{
					$xcheck = "";	
				}//end if($rsm1[status_active] == "1"){
			}else{
				$xcheck = "checked='checked'";	
				 $disp1 = "style='display:'";
			}// end 	if($rsm1[status_active] == "1"){
			...
			### สถานะเอกสารไม่สมบูรณ์
			if($rs_e["$fieldname"] == "0"){
				$xcheck1 = "checked='checked'";	
			}else{
				$xcheck1 = "";	
			}//end 	if($rs_e["$fieldname"] == "0"){
			
				
		###########  ตรวจสอบ การ check ค่าข้อมูล
        ...
        <tr>
          ...
          <td align="left" bgcolor="#FFFFFF"><table width="100%" border="0" cellspacing="0" cellpadding="0">
            <tr>
              <td align="left"><input name="<?=$fieldname?>" id="<?=$fieldname."1"?>" type="radio" value="1" <?=$xcheck?> onClick="return sw_table(this.value,'<?=$rsm1[menu_id]?>')">
          สมบูรณ์ 
          <? if($rsm1[status_active] == "1"){?>
          <input name="<?=$fieldname?>" type="radio" id="<?=$fieldname."2"?>"  value="0" <?=$xcheck1?> onClick="return sw_table(this.value,'<?=$rsm1[menu_id]?>');">
          ไม่สมบูรณ์
          <?
		  }//end  if($rsm1[status_active] == "1"){
		  ?>
          ...
          <table width="100%" border="0" cellspacing="0" cellpadding="0" id="xtblName<?=$rsm1[menu_id]?>" <?=$disp1?>>
            <tr>
              <td>
              <?
              	if(count($arr_doc_comment) > 0){
						foreach($arr_doc_comment as $kcoment => $vcoment){
							$sql_11 = "SELECT * FROM tbl_checklist_problem_detail WHERE idcard='$idcard' AND problem_id='$kcoment' AND menu_id='$rsm1[menu_id]' AND profile_id='$profile_id' and status_problem='0'";
							$result_11 = mysql_db_query($dbname_temp,$sql_11);
							$rs_11 = mysql_fetch_assoc($result_11);
							if ($n++ %  2){ 	$bg = "#F0F0F0"; 	}else{ $bg = "#FFFFFF";	}
							if($rs_11[problem_id] != ""){ $chk = "checked='checked'";}else{ $chk = ""; }

							?>
							 <input type="checkbox" name="check_problem[<?=$idcard?>][<?=$rsm1[menu_id]?>][<?=$kcoment?>]" id="check_problem<?=$rsm1[menu_id]?><?=$kcoment?>" value="<?=$kcoment?>" <?=$chk?>><?=$vcoment?>
							<?	
						}//end foreach($arr_doc_comment as $kcoment => $vcoment){
				}//end  	if(count($arr_doc_comment) > 0){
			  ?>
               </td>
            </tr>
          </table>
  ...
  <tr>
	...
	?>
        ...
        <tr>
          ...
          <input type="hidden" name="flag_sup" value="<?=$flag_sup?>">
