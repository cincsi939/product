...
$flag_pass = 1; # เปิดให้นำเข้าไปก่อน
...
if($action == "process"){
	...
	if(count($arr_id) > 0){
		...
		#$age_gov = $obj_checkdata->GetAge($rs[begindate]);
			...
			if(Check_IDCard($rs[idcard]) or $flag_pass == 1){ // เลขบัตรถูกต้องตามกรมการปกครอง
					if($rs[status_transferkey] == "1"){
						#echo "$rs[idcard] => 0000 => $rs[siteid] => 0000 => $rs[schoolid] => 11 =>Tranfer From 0000 To Cmss<hr>";
							$result_tranfer = transfer_data($rs[idcard],"0000",$rs[siteid],"0000",$rs[schoolid],"11","Tranfer From 0000 To Cmss");// ย้ายข้อมูลจาก cmss 0000
						...
						//if($age >= 18 and $age <= 60 and $rs[sex] != "" and $rs[position_now] != "" and ($rs[begindate]!='0000-00-00' and trim($rs[begindate])!='')){
							...
							$obj_imp->SaveQueueLog($rs[idcard],$rs[siteid],$profile_id,'0000','ข้อมูลตั้งต้นใน checklist ไม่สมบูรณ์','0','0','0',$rsr[idc],$rsr[prename_th],$rsr[name_th],$rsr[surname_th],$rsr[position_now]);
...
WHERE idcard='$rs[idcard]' AND siteid='$rs[siteid]' AND profile_id='$profile_id' order by precen_compare desc limit 1";
	$result_rep = mysql_db_query($dbname_temp,$sql_rep) or die(mysql_error()."".__LINE__);
	$rsr = mysql_fetch_assoc($result_rep);
							$obj_imp->SaveQueueLog($rs[idcard],$rs[siteid],$profile_id,'0000','ข้อมูลเลขประจำตัวประชาชนไม่ถูกต้อง','0','0','9',$rsr[idc],$rsr[prename_th],$rsr[name_th],$rsr[surname_th],$rsr[position_now]);
			...
			echo "<script>alert(\"ข้อมูลทั้งหมด $intall รายการ สามารถประมวลผลได้ $int1 รายการ ไม่สามารถประมวลผลได้ $int2 รายการ\");location.href='report_analysis.php?xsiteid=$xsiteid&profile_id=$profile_id';</script>";	exit;
  ...
  <table width="100%" border="0" cellspacing="0" cellpadding="0">
    <tr>
      <td bgcolor="#000000"><table width="100%" border="0" cellspacing="1" cellpadding="3">
        <tr>
          <td colspan="11" align="left" bgcolor="#CCCCCC"><strong><a href="import_analysis.php?xsiteid=<?=$xsiteid?>&profile_id=<?=$profile_id?>">กลับหน้าหลัก</a> ||รายงานข้อมูลข้าราชการครูและบุคลากรทางการศึกษาที่พร้อมนำเข้าข้อมูล Cmss เพื่อบันทึกข้อมูล</strong></td>
          </tr>
        <tr>
          <td width="3%" align="center" bgcolor="#CCCCCC"><strong>ลำดับ</strong></td>
          <td width="9%" align="center" bgcolor="#CCCCCC"><strong>เลขประจำตัวประชาชน</strong></td>
          <td width="8%" align="center" bgcolor="#CCCCCC"><strong>ชื่อ นามสกุล</strong></td>
          <td width="8%" align="center" bgcolor="#CCCCCC"><strong>วันเดือนปีเกิด</strong></td>
          <td width="8%" align="center" bgcolor="#CCCCCC"><strong>วันเริ่มปฏิบัติ<br>
            ราชการ</strong></td>
          <td width="12%" align="center" bgcolor="#CCCCCC"><strong>ตำแหน่ง</strong></td>
          <td width="4%" align="center" bgcolor="#CCCCCC"><strong>เพศ</strong></td>
          <td width="8%" align="center" bgcolor="#CCCCCC"><strong>หน่วยงานสังกัด</strong></td>
          <td width="7%" align="center" bgcolor="#CCCCCC"><strong>หมายเหตุ</strong></td>
          <td width="23%" align="center" bgcolor="#CCCCCC"><strong>รายการข้อมูลที่คล้ายกัน</strong></td>
...
";
	...
	while($rs = mysql_fetch_assoc($result)){
		 ...
		 if($rs[status_transferkey] == "1"){
			$msg_comment = "<font color=\"#990000\">ย้ายข้อมูลจากฐานกลางเข้าสู่ cmss<font>";	 
			$file_site = "0000";
		}else if($rs[flag_insert] == "1"){
			$msg_comment = "<font color=\"#006600\">เพิ่มข้อมูลเข้าระบบ cmss</font>";
			$file_site = $xsiteid;
		}else{
			$msg_comment = "";
			$file_site = $xsiteid;
        ...
        <tr bgcolor="<?=$bg1?>">
          ...
          <table width="100%" border="0" cellspacing="1" cellpadding="3">
            <tr>
              <td align="center" bgcolor="#CCCCCC"><strong>เลขบัตรฯ</strong></td>
              <td align="center" bgcolor="#CCCCCC"><strong>ชื่อ นามสกุล</strong></td>
              <td align="center" bgcolor="#CCCCCC"><strong>วดป เกิด</strong></td>
              <td align="center" bgcolor="#CCCCCC"><strong>วดป ปฏิบัติ</strong></td>
              <td align="center" bgcolor="#CCCCCC"><strong>ตำแหน่ง</strong></td>
...
queue_tranfer_data_similar AS t1 WHERE t1.idcard='$rs[idcard]' AND t1.siteid='$xsiteid' AND t1.profile_id='$profile_id'";
		...
		?>
          ...
          <input type="submit" name="button" id="button" value="ประมวลผลข้อมูลสู่ cmss" height="50">
          </td>
        </tr>
           <tr>
             <td colspan="10" align="center" bgcolor="#CCCCCC">&nbsp;</td>
