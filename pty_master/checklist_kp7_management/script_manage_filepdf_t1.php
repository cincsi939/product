...
### log pdf main ก่อนทำการเก็บ รายละเอียด
	...
	if($xsfile == 1){
		echo "<script>alert('ประมวลผลข้อมูลเรียบร้อยแล้วจำนวน  $j  ไฟล์ระบบจะแสดง log ข้อมูลในการ upload'); location.href='report_log_upload.php?action=&upload_id=$lastlog_id&xsiteid=$xsecid&profile_id=$profile_id';</script>";
	}else{
...
order by time_update DESC";
	...
	while($rs_a = mysql_fetch_assoc($result_a)){
        ...
        <? } //end if($action == "view_page_fail"){?>
        <td align="center"><? echo $img_pdf;?>
        
        <?
        echo "<a href='$file_pdf' target='_blank'>$file_pdf</a>";
