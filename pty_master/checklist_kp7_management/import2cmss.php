...
header("Location: import_analysis.php?action=&xsiteid=$xsiteid&secname=$secname&lv=0&profile_id=$profile_id");die;
...
if($_SERVER['REQUEST_METHOD'] == "POST"){
	...
	$sql_status_field = "REPLACE INTO tbl_status_field_import(secid,prename_th,name_th,surname_th,birthday,begindate,position_now,schoolid,status_import_data,sex,profile_id)VALUES('$xsiteid','$Fprename_th','$Fname_th','$Fsurname_th','$Fbirthday','$Fbegindate','$Fposition_now','$Fschoolid','$status_data','$sex','$profile_id')";
@mysql_db_query($dbname_temp,$sql_status_field);
	## end บันทึกข้อมูลรายการข้อมูลที่จะนำเข้าระบบ
	
	
		$sql = "SELECT * FROM tbl_checklist_kp7 WHERE siteid='$xsiteid' AND profile_id='$profile_id'";
		//echo $sql;die;
		...
		###  log การนำเข้าข้อมูล #####
				if(array_key_exists("$siteid_replace",$arrsitecon)){
		 		...
		 		InsertLogImpDetail($last_id,$rs[idcard],$rs[siteid],$profile_id,$rs[prename_th],$rs[name_th],$rs[surname_th],$rs[schoolid],$rs[position_now],$siteid_replace,$status_replace_sitecon);
								...
								### เก็บ log ข้อมูลในกรณีข้อมูลไม่ตรงกับระบบ
				...
				if(count($arr_field) > 0){
					foreach($arr_field as $key => $val){
							if(($rs[$val['field_name']] != $arr_compare[$val['field_compare']]) and $arr_compare[$val['field_compare']] != ""){
									...
									$obj->SaveLogCompare($last_id,$rs[idcard],$rs[siteid],$key,$arr_compare[$val['field_compare']],$rs[$val['field_name']]);
									$num_error++;
							}
...
###  field ที่จะทำการ protection ข้อมูลการนำเข้า
...
left join log_import_cmss_confirm as t3 ON t2.idcard=t3.idcard and t2.import_id=t3.import_id and t2.siteid=t3.siteid
WHERE
...
t1.schoolid <>  '0' AND
t1.profile_id =  '$profile_id' AND t2.status_replace_sitecon='0' 
AND t2.import_id='$last_id' 
and t3.idcard IS NULL
GROUP BY t1.idcard";
$result = mysql_db_query($dbname_temp,$sql);
while($rs = mysql_fetch_assoc($result)){
		...
		if($rs[status_id_replace] == "0"){ // คือข้อมูลปกติสามารถนำเข้าไปได้เลย
			SaveLogImpotCheckListToCmss($rs[idcard],$rs[secid],$rs[profile_id],"1");// เก็บ log การนำเข้าข้อมูล
				...
				if(CheckDataInDb($rs[secid],$rs[idcard]) > 0){ // กรณีมีอยู่แล้ว update
					...
					//echo "insert";
					######  ตรวจสอบข้อมูลใน cmss000 เพื่อย้ายข้อมูลกลับมา
...
### end ทำการเทียบข้อมูลใน ตาราง checkl_data กับฐานข้อมูล cmss ถ้าข้อมูลส่วนเกินให้ย้ายและเก็บไปไว้ใน temp_general ก่อนเนื่องจากเป็นข้อมูลที่เคยคีย์อยู่แล้้ว
	//echo "AAAA";die;
		...
		echo "<script>alert('นำเข้าข้อมูลเรียบร้อยแล้ว');location.href='import2cmss.php?action=$action&xsiteid=$xsiteid&secname=$secname&flag_crontab=1&profile_id=$profile_id&getlast_id=$last_id&import_id=$last_id';</script>";	exit;
         ...
         <td width="50%"><table width="100%" border="0" cellspacing="0" cellpadding="0">
           <tr>
             <td bgcolor="#000000"><table width="100%" border="0" cellspacing="1" cellpadding="2">
               <tr>
                 <td colspan="2" align="center" bgcolor="#CAD5FF"><strong>หมวดรายการนำข้อมูลสู่ระบบ cmss ทั้งหมดในเขต</strong></td>
                 </tr>
               <tr>
                 ...
                 <td align="left" bgcolor="#FFFFFF"><table width="100%" border="0" cellspacing="1" cellpadding="3">
                   ...
                   <tr>
                     <td width="11%" align="center" bgcolor="#FFFFFF">
                       <input type="checkbox" name="Fprename_th" value="1"  checked="checked" <?=$xreadonly?>>
                    </td>
                     <td width="89%" align="left" bgcolor="#FFFFFF">คำนำหน้าชื่อ</td>
                   </tr>
                   <tr>
                     <td align="center" bgcolor="#FFFFFF">
                       <input type="checkbox" name="Fname_th" value="1" checked="checked" <?=$xreadonly?>>
                     </td>
                     <td align="left" bgcolor="#FFFFFF">ชื่อ</td>
                   </tr>
                   <tr>
                     <td align="center" bgcolor="#FFFFFF">
                       <input type="checkbox" name="Fsurname_th" value="1" checked="checked" <?=$xreadonly?>>
                     </td>
                     <td align="left" bgcolor="#FFFFFF">นามสกุล</td>
                   </tr>
                   <tr>
                     <td align="center" bgcolor="#FFFFFF"><input type="checkbox" name="Fsex" id="Fsex" value="1" checked="checked" <?=$xreadonly?>>
                      </td>
                     <td align="left" bgcolor="#FFFFFF">เพศ</td>
                   </tr>
                   <tr>
                     <td align="center" bgcolor="#FFFFFF">
                       <input type="checkbox" name="Fbirthday" value="1" checked="checked" <?=$xreadonly?>>
                  </td>
                     <td align="left" bgcolor="#FFFFFF">วันเดือนปีเกิด</td>
                   </tr>
                   <tr>
                     <td align="center" bgcolor="#FFFFFF">
                       <input type="checkbox" name="Fbegindate" value="1" checked="checked" <?=$xreadonly?>>
                    </td>
                     <td align="left" bgcolor="#FFFFFF">วันเริ่มปฏิบัติราชการ</td>
                   </tr>
                   <tr>
                     <td align="center" bgcolor="#FFFFFF"><label>
                       <input type="checkbox" name="Fposition_now" value="1" checked="checked" <?=$xreadonly?>>
                     </label></td>
                     <td align="left" bgcolor="#FFFFFF">ตำแหน่งปัจจุบัน</td>
                   </tr>
                   <tr>
                     <td align="center" bgcolor="#FFFFFF">
                       <input type="checkbox" name="Fschoolid" value="1" checked="checked" <?=$xreadonly?>>
                    </td>
                 ...
                 <td align="center" bgcolor="#000000"><table width="100%" border="0" cellspacing="1" cellpadding="3">
                  ...
                  <? if(CheckNumJ18($xsiteid) > 0){ ?>
                   ...
                   <tr>
                     ...
                     <td align="center" bgcolor="#FFFFFF">
                     <?
                     	$sql_rep = "SELECT COUNT(t1.idcard) AS num1 FROM log_import_cmss_detail AS t1 WHERE t1.siteid='$xsiteid' AND t1.profile_id='$profile_id' AND t1.status_replace_sitecon='1' AND import_id='$getlast_id'";
						$result_rep = mysql_db_query($dbname_temp,$sql_rep);
						$rsrep = mysql_fetch_assoc($result_rep);
						if($rsrep[num1] > 0){
								echo "<a href='report_import_replace.php?action=&xsiteid=$xsiteid&profile_id=$profile_id&import_id=$getlast_id' target='_blank'>".number_format($rsrep[num1])."</a>";
						}else{
								echo "0";	
						}
					 ?>
                   ...
                   <?
                   	$num_confirm = $obj->CountNumListConfirm($import_id,$xsiteid);
					if($num_confirm > 0){
					?>
                   
                 <tr>
                   <td align="left" bgcolor="#FF9900"><strong>6. จำนวนบุคลากรที่ไม่สามารถนำเข้าได้เนื่องจาก<br>
                     ข้อมูลที่จะนำเข้าไม่ตรงกับข้อมูลในระบบ
                   cmss</strong></td>
                   <td align="center" bgcolor="#FF9900"><strong><? echo "<a href=\"import_confirm_data.php?import_id=$import_id&xsiteid=$xsiteid\" target=\"_blank\">".number_format($num_confirm)."</a>";?></strong></td>
                   <td align="center" bgcolor="#FF9900"><strong>คน</strong></td>
                 </tr>
                 <?
					}// end if($num_confirm > 0){
				 ?>
                 </table></td>
               </tr>
             </table></td>
                 ...
                 <td bgcolor="#000000"><table width="100%" border="0" cellspacing="1" cellpadding="3">
                   <tr>
                     <td colspan="2" align="center" bgcolor="#CAD5FF"><strong>รายงานสรุป
                       <?=show_area($xsiteid);?>
                       <br>
                       (ระบบจะทำการนำเข้าข้อมูลบัญชี จ.18 ด้วยในกรณีที่เป็นเขตใหม่)                     </strong></td>
                   </tr>
                   <tr>
				   ...
				   ?>
                   <tr>
                     <td align="left" bgcolor="#FF9900"><strong>จำนวนข้อมูลที่ไม่สามารถนำเข้าได้เนื่องจากข้อมูลที่จะนำเข้า<br>
                       ไม่ตรองกับข้อมูลในระบบ cmss</strong></td>
                     <td align="center" bgcolor="#FF9900"><strong><? echo "<a href=\"import_confirm_data.php?import_id=$import_id&xsiteid=$xsiteid\" target=\"_blank\">".number_format($num_confirm)."</a>";?></strong></td>
                   </tr>
                   <?
					}//end 	if($num_confirm > 0){
