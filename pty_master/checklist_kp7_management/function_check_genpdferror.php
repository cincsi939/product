<?
	...
	$path_fileall = "../../../checklist_kp7file/fileall/";
	$folder_bk_pdf = "../../../backup_kp7file/";
	$path_filesorce = "../../../checklist_kp7file/";
	$path_filesorce_new = "../../../".PATH_KP7_FILE."/";
...
function xCheckXrefPdf($xfile){
		global $db_temp,$path_fileall;
		$file_pdf = $xfile; // ที่อยู่ไฟล์
	...
	function CheckXrefPdf($get_idcard,$get_siteid){
		global $db_temp,$path_fileall;
...
function SaveLogUploadPdf($get_siteid=""){
	global $db_temp,$folder_bk_pdf;
	$xip = xget_real_ip();
	$date_upload = date("Y-m-d H:i:s");
	$date_bk = date("YmdHis");
	$sql = "INSERT INTO log_upload_pdf SET staffid='".$_SESSION['session_staffid']."',ip_server='$xip', date_upload='".$date_upload."',folder_bk='$date_bk',siteid='$get_siteid'";
	$result = mysql_db_query($db_temp,$sql);
...
function CopyFilePdfBackup($get_upload_id,$idcard,$get_siteid){
...
##############  function log_detail
function SaveLogUploadPdfDetail($upload_id,$get_idcard,$get_siteid,$status_file){
	global $db_temp;
	$sql = "INSERT INTO log_upload_pdf_detail SET upload_id='$upload_id',idcard='$get_idcard',siteid='$get_siteid',status_file='$status_file',timeupdate=NOW() ";
	mysql_db_query($db_temp,$sql);
