...
*/
function read_file_folder($get_secid){
		$Dir_Part="../../../checklist_kp7file/$get_secid/";
...
?>
  <tr bgcolor="<?=$bgcolor1?>">
    ...
    <td width="13%" align="center"><a href="?action=getdata&type=pdf&secid=<?=$rs[secid]?>">ประมวลผลรายเขต</a></td>
    <td width="9%" align="center"><? if($arr1[NumPageFail] > 0){ echo "<a href='?action=view_page_fail&sentsecid=$rs[secid]&amount=$arr1[NumPageFail]' target='_blank'>".number_format($arr1[NumPageFail])."</a>";}else{ echo "0";}?></td>
    <td width="18%" align="center"><? if($arr1[NumPerson] > 0){ echo "<a href='?action=view_all&sentsecid=$rs[secid]&amount=$arr1[NumPerson]' target='_blank'>".number_format($arr1[NumPerson])."</a>";}else{ echo "0";}?></td>
...
if($action == "getdata"){
	####  sript log pdf
	if($type == "pdf"){
	$file_pdf = read_file_folder($secid);
	
//echo "<pre>";
//print_r($file_pdf);die;
	
	$path_n = "../../../".PATH_KP7_FILE."/$secid/";
	if(!is_dir($path_n)){
		xRmkdir($path_n);
	}
	$path_old = "../../../checklist_kp7file/$secid/";
	$pdf = ".pdf";
		if(count($file_pdf) > 0){
		$j=0;
	...
	echo "<script>alert('script เก็บ log บันทึกข้อมูลเรียบร้อยแล้ว  $j  รายการ'); location.href='script_manage_filepdf_kp7file.php?action=';</script>";
	}// end if($type == "pdf"){
	#### end sript log pdf
#####  ประมวลผลทั้งหมด
	if($type == "pdfall"){
		$get_file = "fileall";
		$file_pdf = read_file_folder($get_file);
