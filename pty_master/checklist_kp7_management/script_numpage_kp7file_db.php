...
function ReadFileFolderx($get_site=""){
		if($get_site == ""){
...
<table width="100%" border="0" cellpadding="3" cellspacing="1" bordercolor="#000000">
      <tr>
        <td height="24" colspan="9" align="center" bgcolor="#a3b2cc"><strong><!--<a href="?action=getdata&type=pdfall">ประมวลผลทั้งหมด</a>-->โปรแกรมนำเข้าไฟล์ทะเบียนประวัิติต้นฉบับ</strong></td>
        </tr>
      <tr>
...
ไม่ตรงกัน(คน)</strong></td>
    <td height="24" colspan="5" align="center" bgcolor="#a3b2cc"><strong>จำนวนรายการ</strong></td>
  </tr>
  <tr>
...
?>
  <tr bgcolor="#<?=$bgcolor1?>">
    ...
    <td width="20%"><?=str_replace("สำนักงานเขตพื้นที่การศึกษา","สพท.",$rs[secname])?>&nbsp;[<?=$rs[secid]?>]</td>
    <td width="7%" align="center"><a href="?action=getdata&type=pdf&xsecid=<?=$rs[secid]?>&profile_id=<?=$profile_id?>">upload</a></td>
    <td width="11%" align="center"><? if($arr1[NumPageFail] > 0){ echo "<a href='?action=view_page_fail&sentsecid=$rs[secid]&amount=$arr1[NumPageFail]&profile_id=$profile_id' target='_blank'>".number_format($arr1[NumPageFail])."</a>";}else{ echo "0";}?></td>
    <td width="17%" align="center"><? if($arr1[NumPerson] > 0){ echo "<a href='?action=view_all&sentsecid=$rs[secid]&amount=$arr1[NumPerson]&profile_id=$profile_id' target='_blank'>".number_format($arr1[NumPerson])."</a>";}else{ echo "0";}?></td>
    <td width="12%" align="center"><? if($arr1[NumPdf] > 0){ echo "<a href='?action=view_in&sentsecid=$rs[secid]&amount=$arr1[NumPdf]&profile_id=$profile_id' target='_blank'>".number_format($arr1[NumPdf])."</a>";}else{ echo "0";}?></td>
    <td width="12%" align="center"><? if($DisCount > 0){ echo "<a href='?action=view_discount&sentsecid=$rs[secid]&amount=$DisCount&profile_id=$profile_id' target='_blank'>".number_format($DisCount)."</a>";}else{ echo "0";}?></td>
    <td width="10%" align="center"><? if($numxref > 0){ echo "<a href='?action=view_error&xsiteid=$rs[secid]&profile_id=$profile_id' target='_blank'>$numxref</a>";}else{ echo "0";}?></td>
    <td width="7%" align="center"><a href="report_log_upload.php?action=&xsiteid=<?=$rs[secid]?>&profile_id=<?=$profile_id?>"><img src="../../images_sys/button_select.png" width="14" height="13" / border="0" title="แสดงข้อมูล log กาีร upload ไฟล์ pdf"></a></td>
    </tr>
   <? 
...
### log pdf main ก่อนทำการเก็บ รายละเอียด
	...
	//echo "จำนวนทั้งหมด".count($file_pdf)."<br>";
				...
				$sql_c= "SELECT * FROM tbl_checklist_kp7 WHERE  profile_id='$profile_id'  and page_upload > 0 ";
				//echo $sql_c."<br>";
				
				$result_c = mysql_db_query($dbname_temp,$sql_c);
				$i=0;
				while($rs_c = mysql_fetch_assoc($result_c)){
						$path_filekp7 = "../../../".PATH_KP7_FILE."/$rs_c[siteid]/";
						$pdf = ".pdf";
				
								$kp7file_dest = $path_filekp7.$rs_c[idcard].$pdf;
								
								if(!is_file($kp7file_dest)){
									$i++;
									$sql_update = "UPDATE tbl_checklist_kp7 SET page_upload='' WHERE idcard='$rs_c[idcard]' AND siteid='$rs_c[siteid]' AND profile_id='$profile_id' ";
									echo $i." :: $sql_update<br>";
									mysql_db_query($dbname_temp,$sql_update);
								}//end 
								
								
					}//end	while($rs_c = mysql_fetch_assoc($result_c)){
