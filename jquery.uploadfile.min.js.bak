    ...
    b.fn.uploadFile = function(f) {
        ...
        function r(K, E, x) {
            for (var F = 0; F < x.length; F++) {
                ...
                H.filename.html(J);
                var y = b("<form style='display:block; position:absolute;left: 150px;' class='" + E.formGroup + "' method='" + K.method + "' action='" + K.url + "' enctype='" + K.enctype + "' id='form_" + K.req_id + "_" + K.doc_id + "_" + n.fileCounter + "''></form>");
                y.appendTo("body");
                var A = [];
                A.push(x[F].name);
                w(y, G, H, A, E, x[F]);
        ...
        function j(y, x) {
			...
			for (var i =0; i<=6; i++ )
			{
				var y = b(".ajax-file-upload-statusbar").length;
			
			var datavar1 =["แบบลงทะเบียนเพื่อขอรับสิทธิฯ (ดร.01)", "แบบรับรองสถานะของครัวเรือน (ดร.02)", "สำเนาบัตรประชาชนของหญิงตั้งครรภ์/มารดา 1 ฉบับ", "เอกสารการฝากครรภ์หรือสมุดบันทึกสุขภาพแม่และเด็กมาแสดง(เจ้าหน้าที่บันทึกสถานที่ฝากครรภ์และกำหนดคลอด)", "สำเนาสูติบัตรเด็ก 1 ฉบับ (ยื่นหลังจากคลอดบุตรแล้ว)","หนังสือมอบอำนาจ (ดร.05)"];
            this.statusbar = b("<div class='ajax-file-upload-statusbar'><div style='text-align:top;'>คำอธิบาย : <input type='text' style='width:300px;' class='caption_input' id='in1' onKeyup=\"$('#form_" + x.req_id + "_" + x.doc_id + "_" + n.fileCounter + "').attr('action',$('#form_" + x.req_id + "_" + x.doc_id + "_" + n.fileCounter + "').attr('action')+'&caption='+$(this).val()); \" value='"+datavar1[i]+"'></div></div>").width(400);
