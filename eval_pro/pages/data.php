...
$data;
$query = "SELECT * FROM personal_point_2557 LIMIT 20";
if (isset($_GET['update']))
{
	//$wp=$_GET['WorkingPeple'];
	//$WPeple=json_decode($wp, true);
	//$wPe = json_decode($wp, true);
	//$wPeple=iconv("TIS-620","UTF-8",$wp);
	//print_r($obj);
	//echo $obj;
	//$obj = json_decode($wPeple, true);
	//print_r($obj);
	// UPDATE COMMAND 
	//echo $obj;
	//console.log($obj);
	...
	//$WPeple=json_decode($update_query);
	//$WPeple=json_decode($update_query, true);
	 $result = mysql_query($update_query) or die("SQL Error 1: " . mysql_error());
	//$WPeple=json_decode($result, true);
     echo $result;
}
else
{
